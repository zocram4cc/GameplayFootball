...
- **Dribbles, tricks and feints** (403 PES clips). They belong to
  `ballcontrol`, which is GF's core locomotion-with-ball type, filtered hard
  on direction and requeued constantly. Bulk-importing them risks the thing
  the rest of this work is meant to protect, so they are converted
  (`data/imports/pes21/gameplay/feint/`) but not installed.
- **Enriched movement** (748 locomotion clips) — same reasoning, more so:
  `movement` is where the autogenerated blend set lives, and stock locomotion
  is tuned.
- **An animation inspector** — a debug view with one actor cycling imported
  clips side by side with their stock counterparts would make this far faster
  to judge than watching matches.
