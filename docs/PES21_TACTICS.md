...
| Playing styles / cards | **partial** — 12 traits; 10 have real mechanical effects (poacher X-target on the offside line, space-rating weight, shielding radius, knuckleball spin, first-time-shot power, shot appetite/range for Fox in the Box, Long Range Shooter and Anchorman). **Prolific Winger and Box-to-Box are declared, named, assigned — and read by nothing.** Deterministic per-player assignment gated by role, GKs excluded | `src/data/playertraits.hpp/.cpp`, consumed in `elizacontroller.cpp:966,982,1173`, `teamAIcontroller.cpp:599`, `humanoid.cpp:716,722`, `playerdata.cpp:142,156-158` |
| Position-compatibility gate for styles | **absent** — a trait fires wherever the player is deployed. `PlayerTraits::SuitsRole` exists but gates *assignment* only, never activation | `src/data/playertraits.cpp:132` |
