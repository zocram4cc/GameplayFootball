...
TeamData::TeamData(int teamDatabaseID) : databaseID(teamDatabaseID) {
  ...
  while (iter != tree.children.end()) {
    for (int num = 0; num < playerNum; num++) {
      if ((*iter).first == "p" + int_to_str(num + 1)) {
        formation[num].databasePosition =
            GetVectorFromString((*iter).second.children.find("position")->second.value);
  ...
  while (changed && iterations < maxIterations) {
    ...
    for (int p1 = 0; p1 < playerNum - 1; p1++) {
      if (formation[p1].role == e_PlayerRole_GK)
        continue;

      for (int p2 = p1 + 1; p2 < playerNum; p2++) {
        if (formation[p2].role == e_PlayerRole_GK)
          continue;
        ...
        if (diff.GetLength() < minDistanceFraction) {
          ...
          offset[p1] +=
              diff.GetNormalized(Vector3(0, 1, 0)) * minDistanceFraction * distanceFactor * 0.5f;
          offset[p2] -=
              diff.GetNormalized(Vector3(0, 1, 0)) * minDistanceFraction * distanceFactor * 0.5f;
        }
      }
  ...
  if (national)
    order = "nationalformationorder";

  result =
      GetDB()->Query("select id from players where team_id = " + int_to_str(teamDatabaseID) +
                     " or nationalteam_id = " + int_to_str(teamDatabaseID) + " order by " + order);
...
void TeamData::SwitchPlayers(int databaseID1, int databaseID2) {
  ...
  for (int i = 0; i < (signed int)playerData.size(); i++) {
    if (playerData.at(i)->GetDatabaseID() == databaseID1)
      index1 = i;
    if (playerData.at(i)->GetDatabaseID() == databaseID2)
      index2 = i;
  }
  assert(index1 != -1);
  assert(index2 != -1);
