...
void HumanController::RequestCommand(PlayerCommandQueue& commandQueue) {
  ...
  if (actionMode == 2 &&
      (functionType == e_FunctionType_ShortPass || functionType == e_FunctionType_LongPass ||
       functionType == e_FunctionType_HighPass || functionType == e_FunctionType_Shot) &&
  ...
  if (actionMode == 1 &&
      (functionType == e_FunctionType_Sliding || functionType == e_FunctionType_Interfere) &&
      !CastPlayer()->TouchPending()) {
    actionMode = 0;
    gauge_ms = 0;
    actionBufferTime_ms = 0;
  ...
  if (actionMode == 2) {
    if (!hid->GetButton(actionButton) ||
        (hid->GetButton(actionButton) &&
         gauge_ms >
             500) ||  // allow anim to kick in before queue is complete (before button is released),
                      // it will usually touch ball after the remaining time anyway, so we still
                      // have time to add more power, yet still respond as fast as possible
        (!CastPlayer()->HasPossession() && !match->IsInSetPiece() && actionBufferTime_ms > 0)) {
      int baseTime_ms =
          60;  // substract a little because we can't really press a button shorter than this
      ...
      if (match->IsInSetPiece() && team->GetController()->GetPieceTaker() == player &&
          team->GetController()->GetSetPieceType() == e_SetPiece_KickOff) {
        ...
        AI_GetPass(CastPlayer(), command.desiredFunctionType, command.touchInfo.inputDirection,
                   command.touchInfo.inputPower, command.touchInfo.autoDirectionBias,
                   command.touchInfo.autoPowerBias, command.touchInfo.desiredDirection,
                   command.touchInfo.desiredPower, command.touchInfo.targetPlayer,
                   command.touchInfo.forcedTargetPlayer);
        ...
        command.touchInfo.autoDirectionBias = GetConfiguration()->GetReal(
            "gameplay_shortpass_autodirection", _default_ShortPass_AutoDirection);
        command.touchInfo.autoPowerBias = GetConfiguration()->GetReal(
            "gameplay_shortpass_autopower", _default_ShortPass_AutoPower);
        AI_GetPass(CastPlayer(), command.desiredFunctionType, command.touchInfo.inputDirection,
                   command.touchInfo.inputPower, command.touchInfo.autoDirectionBias,
                   command.touchInfo.autoPowerBias, command.touchInfo.desiredDirection,
                   command.touchInfo.desiredPower, command.touchInfo.targetPlayer);
        ...
        command.touchInfo.autoPowerBias =
        ...
        if (GetHIDevice()->GetDeviceType() == e_HIDeviceType_Keyboard)
          command.touchInfo.autoDirectionBias = 1.0f;
        command.touchInfo.desiredDirection = AI_GetShotDirection(
            CastPlayer(), command.touchInfo.inputDirection, command.touchInfo.autoDirectionBias);
  ...
  if ((match->IsInSetPiece() && team->GetController()->GetPieceTaker() == player &&
    ...
    // if (team->GetController()->GetPieceTaker() != player) printf("waiting for teammate to take
    // set piece!\n");
    return;
  }

  // delay direction input until we have chosen a steady direction.
  // this is because humans can only move an analog stick so fast, and we don't want requeues to
  // happen mid-analogstick-movement.
  Vector3 inputDirectionSaveNonsteady = inputDirection;
  // SetGreenDebugPilon(player->GetPosition() + inputDirection * (inputVelocityFloat * 0.5f +
  // 0.5f));
  ...
  if (match->IsInPlay() && !match->IsInSetPiece()) {
    ...
    if (actionMode == 2) {
      float dot = CastPlayer()->GetDirectionVec().GetDotProduct(inputDirection) * 0.5f +
                  0.5f;  // todo: test
      if (CastPlayer()->GetEnumVelocity() != e_Velocity_Idle) {
        inputDirection =
            (CastPlayer()->GetDirectionVec() * 1.0f + inputDirection * 0.0f)
                .GetNormalized(inputDirection);  // want inputdirection to be biggest, so we won't
                                                 // stubbornly fail to do 180s
      }
    ...
    // sidestep dribble disabled for now, too quirky: if (hid->GetButton(e_ButtonFunction_Dribble))
    // keepCurrentBodyDirection = true;
    _BallControlCommand(commandQueue, idleTurnToOpponentGoal, knockOn, true,
  ...
  if (commandQueue.size() > 0) {
    ...
    assert(command.desiredFunctionType ==
...
void HumanController::Process() {
  // just doesn't work so well (fixes the 'humans can't change stick pos instantly' problem, but
  // introduces too much lag). maybe revisit/update later
  ...
  if (enableSteadyDirectionSystem) {
    if (angle < 0.01f * pi || angle > 0.65f * pi ||
        (match->GetActualTime_ms() - lastSteadyDirectionSnapshotTime_ms) > 100) {
  ...
  if (actionMode == 0 &&
    ...
    if (match->GetDesignatedPossessionPlayer() == player) {
      possessionContext = 1.0f;  // new (keeper was allowed doing slidings before free kick
                                 // sometimes lol, sign something was wrong)
    } else {
      // in situations where we aren't the designated player, we sometimes still want to do ball
      // stuff, because we could try to extend our leg to pass, for example
      if (hid->GetButton(e_ButtonFunction_ShortPass))
        possessionContext += 0.15f;  // todo: bug: the logic of these weighings are based on the
                                     // default 'pes' button settings.. need to take into account
                                     // the defensive function of the buttons as well
      if (hid->GetButton(e_ButtonFunction_LongPass))
        possessionContext += 0.15f;
      ...
      if (match->GetBall()->Predict(0).coords[2] > 1.5f)
        possessionContext += 0.2f;
    }
    ...
    if (possessionContext < 0.0f) {
      ...
      if (hid->GetButton(e_ButtonFunction_Pressure) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_Pressure) && allowPressure) {
        actionMode = 1;
      ...
      if (hid->GetButton(e_ButtonFunction_Sliding) &&
          ...
          allowSliding) {  // we don't want high passes to turn into slidings
        actionMode = 1;
        actionButton = e_ButtonFunction_Sliding;
      }

      if (hid->GetButton(e_ButtonFunction_TeamPressure) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_TeamPressure) && allowTeamPressure) {
        actionMode = 1;
        actionButton = e_ButtonFunction_TeamPressure;
      }

      if (hid->GetButton(e_ButtonFunction_KeeperRush) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_KeeperRush) && allowKeeperRush) {
        actionMode = 1;
      ...
      if (team->GetController()->GetPieceTaker() == player &&
          team->GetController()->GetSetPieceType() == e_SetPiece_ThrowIn) {
        allowHighPass = false;
        allowShot = false;
      }

      if (hid->GetButton(e_ButtonFunction_ShortPass) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_ShortPass) && allowShortPass) {
        actionMode = 2;
        actionButton = e_ButtonFunction_ShortPass;
      }

      if (hid->GetButton(e_ButtonFunction_LongPass) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_LongPass) && allowLongPass) {
        actionMode = 2;
        actionButton = e_ButtonFunction_LongPass;
      }

      if (hid->GetButton(e_ButtonFunction_HighPass) &&
          !hid->GetPreviousButtonState(e_ButtonFunction_HighPass) && allowHighPass) {
        actionMode = 2;
