...
Vector3 CalculateBestAchievableTarget(Player* player, const Vector3& pos1, float time1_sec,
                                      const Vector3& pos2, float time2_sec) {
  ...
  for (float percentage = 0.0f; percentage <= 1.0f; percentage += stepSize) {
    ...
    int maxTime_ms = -1;
    unsigned int timeNeeded_ms =
        AI_GetTimeNeededForDistance_ms(player->GetPosition(), player->GetMovement(), checkPos,
                                       player->GetMaxVelocity(), false /*precise*/, maxTime_ms,
                                       false /*debug*/)
            .optimistic_ms;
...
void GoalieDefaultStrategy::RequestInput(const MentalImage* mentalImage, Vector3& direction,
  ...
  if (ballPos.coords[0] * team->GetSide() > 0) {  // optimization
    ...
    if (!IsBallBoundForGoal()) {
      ...
      // todo: maybe in some situations, we should prefer walking as max velo, because then the
      // correct body directino can be applied (walking backwards, for example), which are better
      // for deflect anims
      maxVelocity = sprintVelocity;  // walkVelocity;

      // first, make line from ballPos to one post, then one to the other post, then calculate the
      // line in between. this line is the line we want our goalie to be on: it splits the goal in
      // to equal halves (in the 'ball view projection', that is)
      ...
      Vector3 middle =
          toPost1.GetRotated2D(angle * 0.5f).GetNormalized(Vector3(team->GetSide(), 0, 0));
      Line ballToGoal;
      ...
      // (or rather, near the backline - keeping ON the backline is dangerous; some anims may only
      // touch the ball when it's already behind the line, which is pretty useless :p)
      Line backLine;
      ...
      bool applyRushOut =
          team->GetController()->GetEndApplyKeeperRush_ms() > match->GetActualTime_ms();
      if (applyRushOut)
        awayFromGoalBias = 1.0f;

      float distance = std::max(ballToGoal.GetLength() - 0.5f,
                                0.0f);  // keep distance from target, we don't want to overshoot
      awayFromGoalOffset_m = clamp(distance * awayFromGoalBias, awayFromGoalOffset_m,
                                   pitchHalfW);  // when ball is farther away, goalie stays farther
                                                 // away from goal (to make runs when necessary)

      // offset from goal line
      targetPos = ballToGoal.GetVertex(1) +
                  (ballToGoal.GetVertex(0) - ballToGoal.GetVertex(1)).GetNormalized(0) *
                      awayFromGoalOffset_m;
      // SetGreenDebugPilon(targetPos);

      /* disabled: just rush to ball even if we probably can't make it there. this stuff doesn't
         work good enough (yet?)
            // time we assume it will take for the ball to arrive at some point (in other words: how
         fast the attacker gets the ball there) float time1_sec = (targetPos -
         ballToGoal.GetVertex(0)).GetLength() / sprintVelocity; float time2_sec =
         (ballToGoal.GetVertex(1) - ballToGoal.GetVertex(0)).GetLength() / sprintVelocity;

            //SetYellowDebugPilon(targetPos);
            targetPos = CalculateBestAchievableTarget(CastPlayer(), targetPos, time1_sec,
         ballToGoal.GetVertex(1), time2_sec); SetRedDebugPilon(targetPos + Vector3(0, 0, 0.1f));
      */
      ...
      if ((targetPos - goalPos).GetLength() < (player->GetPosition() - goalPos).GetLength() &&
          distanceToBallToGoalLine < 1.0f && u > 0.0f)
        maxVelocity = walkVelocity;

      targetPos.coords[0] = clamp(targetPos.coords[0], -pitchHalfW + 0.2f,
                                  pitchHalfW - 0.2f);  // not very useful to stand behind backline
      ...
      Vector3 ballOverGoalLinePos =
          Vector3(pitchHalfW * team->GetSide(), ballBoundForGoal_ycoord, 0);
      ballOverGoalLinePos +=
          (ballToGoal.GetVertex(0) - ballOverGoalLinePos).GetNormalized(0) * minGoalLineDist;
      ballToGoal.SetVertex(1, ballOverGoalLinePos);
      float u = 0.0f;
      float distance =
          ballToGoal.GetDistanceToPoint(player->GetPosition() + player->GetMovement() * 0.05f, u);

      float u_at_1sec = 0.0f;
      float distance_at_1sec =
          ballToGoal.GetDistanceToPoint(mentalImage->GetBallPrediction(1010).Get2D(), u_at_1sec);
      ...
      if (should_gk_run_towards_the_goal) {
        ...
        targetPos =
            ballToGoal.GetVertex(0) + (ballToGoal.GetVertex(1) - ballToGoal.GetVertex(0)) * u;
...
void GoalieDefaultStrategy::CalculateIfBallIsBoundForGoal(const MentalImage* mentalImage) {
  ...
  if (mentalImage->GetBallPrediction(4000).coords[0] * side > pitchHalfW &&
    ...
    /* 3d version
        ...
        //line.SetVertex(1, mentalImage->GetBallPrediction(0) + match->GetBall()->GetMovement() *
       800); Triangle goal1; goal1.SetVertex(0, Vector3((pitchHalfW - 0.0) * side,  3.7 * panic,
       0)); goal1.SetVertex(1, Vector3((pitchHalfW - 0.0) * side, -3.7 * panic, 0));
