...
DefaultMidfieldStrategy::DefaultMidfieldStrategy(ElizaController* controller)
    : Strategy(controller) {
...
void DefaultMidfieldStrategy::RequestInput(const MentalImage* mentalImage, Vector3& direction,
                                           float& velocity) {
  ...
  if (defensiveComponents) {
    ...
    controller->AddDefensiveComponent(
        desiredPosition,
        pow(clamp(1.5f - mindset - controller->GetFadingTeamPossessionAmount(), 0.0f, 1.0f), 0.7f));
