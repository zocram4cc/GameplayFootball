...
void FillTemporalHumanoidNodes(boost::intrusive_ptr<Node> targetNode,
  ...
  temporalHumanoidNode.position.SetValue(targetNode->GetPosition(),
                                         EnvironmentManager::GetInstance().GetTime_ms());
  temporalHumanoidNode.orientation.SetValue(targetNode->GetRotation(),
                                            EnvironmentManager::GetInstance().GetTime_ms());
...
HumanoidBase::HumanoidBase(PlayerBase* player, Match* match,
    ...
    : fullbodyTargetNode(fullbodyTargetNode),
      ...
      buf_bodyUpdatePhaseOffset(bodyUpdatePhaseOffset) {
  ...
  skin = ResourceManagerPool::GetInstance()
  ...
  boost::intrusive_ptr<Node> bla2(
      new Node(*fullbodySourceNode.get(), int_to_str(player->GetID()), GetScene3D()));
  fullbodyNode = bla2;
  ...
  // players on the default body fall back to the shared PES base face
  // (media/objects/players/faceweights.txt + expressions/, generated by
  // tools/pes21_import/pes_base_body.py)
  ...
  boost::intrusive_ptr<Resource<GeometryData>> geometry =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch("media/objects/players/hairstyles/" + player->GetPlayerData()->GetHairStyle() +
                      ".ase",
                  true, true);
  hairStyle = boost::static_pointer_cast<Geometry>(
      ObjectFactory::GetInstance().CreateObject("hairstyle", e_ObjectType_Geometry));
...
HumanoidBase::~HumanoidBase() {
  ...
  // the other full body mesh ref is connected to a geometry object which has taken over ownership
...
void HumanoidBase::PrepareFullbodyModel(std::map<Vector3, Vector3>& colorCoords) {
  ...
  for (unsigned int subgeom = 0; subgeom < fullbodySubgeomCount; subgeom++) {
    ...
    assert((unsigned int)uniqueMesh.size ==
           uniqueVertices.size() * GetTriangleMeshElementCount() * 3);
    ...
    assert(materializedTriangleMeshes.at(subgeom).verticesDataSize /
    ...
    memcpy(materializedTriangleMeshes.at(subgeom).vertices, uniqueMesh.data,
    ...
    /*
        ...
        for (int i = 0; i < elementOffset; i += 3) {
          printf("%f, ", materializedTriangleMeshes.at(subgeom).vertices[uniqueIndices[i / 3] * 3 +
       uniqueElementOffset * 2]);
        }
        printf("\n");
        printf("\n");
    */
...
void HumanoidBase::UpdateFullbodyNodes() {
  ...
  if (NeedsModelUpdate()) {
    ...
    hairStyle->SetPosition(hairStyle->GetPosition() + (fullbodyOffset - previousFullbodyOffset),
  ...
  */
  if (hairstyleActive) {
    ...
    hairStyle->SetPosition(Vector3(1000, 1000, -1000), false);
    hairStyle->RecursiveUpdateSpatialData(e_SpatialDataType_Both);
  }
...
void HumanoidBase::Process() {
  ...
  if (startPos.coords[2] != 0.f) {
    ...
    Log(e_FatalError, "HumanoidBase", "Process",
  ...
  // radian beginAngle = currentAnim->rotationSmuggle.begin;// * (1.0f - frameBias); // more
  // influence in the beginning; act more like it was 0 later on. (yes, next one is a bias within a
  // bias) *edit: disabled, looks better without currentAnim->rotationSmuggleOffset = beginAngle *
  // (1.0f - frameBias) +
...
void HumanoidBase::PreparePutBuffers(unsigned long snapshotTime_ms) {
  // offsets
  CalculateGeomOffsets();  // todo: in a perfect world, we don't want to do cpu intensive and/or
                           // stuff that uses a lot of mutex locking in this here function

  buf_animApplyBuffer = animApplyBuffer;
  buf_animApplyBuffer.snapshotTime_ms = snapshotTime_ms;
  /*
  // some temporal buffers fail when switching anims - can't interpolate between these values from
  previous and new anim if (currentAnim->frameNum == 0) {
  ...
  */
  ...
  if (!player->GetExternalController() && !match->GetPause()) {
    ...
    if ((spatialState.position - focusPos).GetLength() > 14.0f)
      buf_LowDetailMode = true;
  }
}
...
void HumanoidBase::Put() {
  ...
  //  the apply function doesn't know better than that it is displaying snapshot times, so continue
  //  this hoax into the timeDiff_ms value. then, the temporalsmoother will convert it to 'realtime'
  //  once again
  unsigned long timeDiff_ms =
      fetchedbuf_animApplyBuffer.snapshotTime_ms - fetchedbuf_previousSnapshotTime_ms;
  // printf("diff: %lu\n", timeDiff_ms);
  ...
  for (unsigned int i = 0; i < buf_TemporalHumanoidNodes.size(); i++) {
    // first, restore the previous non-temporal-smoothed values, so the Apply() function can use
    // them for smoothing
    buf_TemporalHumanoidNodes.at(i).actualNode->SetPosition(
        buf_TemporalHumanoidNodes.at(i).cachedPosition, false);
    buf_TemporalHumanoidNodes.at(i).actualNode->SetRotation(
        buf_TemporalHumanoidNodes.at(i).cachedOrientation, false);
  }
  ...
  // thus; read out the current values we've just set, insert them in the temporal smoother, and get
  // the historic spatial data instead (GetValue).
  for (unsigned int i = 0; i < buf_TemporalHumanoidNodes.size(); i++) {
    // save the non-historic version in cachedNode
    buf_TemporalHumanoidNodes.at(i).cachedPosition =
        buf_TemporalHumanoidNodes.at(i).actualNode->GetPosition();
    buf_TemporalHumanoidNodes.at(i).cachedOrientation =
        buf_TemporalHumanoidNodes.at(i).actualNode->GetRotation();

    // sudden realisation: by only SetValue'ing here instead of in prepareputbuffers, aren't we
    // missing out on valuable interpolateable data?
    buf_TemporalHumanoidNodes.at(i).position.SetValue(
        buf_TemporalHumanoidNodes.at(i).actualNode->GetPosition(),
        fetchedbuf_animApplyBuffer.snapshotTime_ms);
    buf_TemporalHumanoidNodes.at(i).orientation.SetValue(
        buf_TemporalHumanoidNodes.at(i).actualNode->GetRotation(),
        fetchedbuf_animApplyBuffer.snapshotTime_ms);

    buf_TemporalHumanoidNodes.at(i).actualNode->SetPosition(
        buf_TemporalHumanoidNodes.at(i).position.GetValue(match->GetPreviousPutTime_ms()), false);
    buf_TemporalHumanoidNodes.at(i).actualNode->SetRotation(
        buf_TemporalHumanoidNodes.at(i).orientation.GetValue(match->GetPreviousPutTime_ms()),
        false);
  }
...
void HumanoidBase::CalculateGeomOffsets() {
  ...
  if (currentAnim->functionType == e_FunctionType_Movement && match->IsInPlay()) {
    ...
    Vector3 toBall = (currentMentalImage->GetBallPrediction(10) -
  spatialState.position).GetNormalized(spatialState.directionVec);
...
int HumanoidBase::GetIdleMovementAnimID() {
  ...
  if (Verbose())
    if (dataSet.size() == 0)
      printf("no animations to begin with\n");
  ...
  SetBodyDirectionSimilarityPredicate(
      spatialState.position + Vector3(0, -10, 0).GetRotated2D(spatialState.angle));  // lookat
#ifdef dataSetSortable
  dataSet.sort([this](int a, int b) { return CompareBodyDirectionSimilarity(a, b); });
#else
  std::stable_sort(dataSet.begin(), dataSet.end(),
                   [this](int a, int b) { return CompareBodyDirectionSimilarity(a, b); });
#endif
...
void HumanoidBase::OffsetPosition(const Vector3& offset) {
  ...
  if (decayingPositionOffset.GetLength() > 0.1f)
    decayingPositionOffset = decayingPositionOffset.GetNormalized() * 0.1f;
  ...
  if (currentAnim->anim->GetVariable("incoming_special_state").compare("") == 0 &&
    ...
    if (this->interruptAnim == e_InterruptAnim_None &&
        (currentAnim->functionType != e_FunctionType_Trip ||
         (currentAnim->anim->GetVariable("triptype").compare("1") == 0 && tripType > 1)) &&
...
bool HumanoidBase::_HighOrBouncyBall() const {
  ...
  if (ballHeight1 > 0.3f || ballHeight2 > 0.3f) {
    highBall = true;
  } else if (ballBounce > 1.0f) {  // low balls are also treated as 'high ball' when there's a lot
                                   // of bounce going on (hard to control)
    highBall = true;
  }
  return highBall;
}
  ...
  while (iter != dataSet.end()) {
    ...
    if (strict) {
      if (atoi(anim->GetVariable("quadrant_id").c_str()) == bestQuadrantID) {
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement") == 0)
        // printf("keeping %s\n", anim->GetName().c_str());
        iter++;
      } else {
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement") == 0)
      ...
      if (anim->GetVariable("lastditch").compare("true") !=
          0) {  // last ditch anims may always change velo
        if (abs(GetVelocityID(quadrant.velocity, true) -
                GetVelocityID(bestQuadrant.velocity, true)) > allowedVelocitySteps)
          predicate = false;
      }
      ...
      if (predicate) {
        ...
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement") == 0)
        // printf("deleting %s (quadrant.angle %f - bestQuadrant.angle %f)\n",
...
void HumanoidBase::_KeepBestBodyDirectionAnims(DataSet& dataSet, const PlayerCommand& command,
  ...
  while (iter != dataSet.end()) {
    ...
    if (fabs(animLookAngle - bestLookAngle) <= adaptedAllowedAngle) {
      // if (player->GetDebug() && bestAnim->GetAnimType().compare("ballcontrol") == 0)
      // printf("keeping %s (%f, %f (%f), %f)\n", anim->GetName().c_str(), animOutgoingBodyAngle,
      // animOutgoingAngle, anim->GetOutgoingAngle(), animLookAngle);
      iter++;
    } else {
      // if (player->GetDebug() && bestAnim->GetAnimType().compare("ballcontrol") == 0)
      // printf("deleting %s (%f, %f (%f), %f)\n", anim->GetName().c_str(), animOutgoingBodyAngle,
      // animOutgoingAngle, anim->GetOutgoingAngle(), animLookAngle);
      iter = dataSet.erase(iter);
    }
  }
...
/* deprecated
void HumanoidBase::GetRatedDataSet(RatedDataSet &ratedDataSet, const DataSet &dataSet, const
IdealAnimDescription &idealAnimDescription, float weight_foot, float weight_incomingVelocity, float
weight_outgoingMovement, float weight_incomingBodyDirection, float weight_outgoingBodyDirection,
float weight_baseAnim, float weight_side, float penalty_nonLinearVelocity, float
penalty_nonLinearBodyDirection) { DataSet::const_iterator iter = dataSet.begin();
  ...
  while (iter != dataSet.end()) {
    ...
    if (weight_foot > 0.0f) {
      ...
      if (FloatToEnumVelocity(anim->GetIncomingVelocity()) == e_Velocity_Idle) rating_foot = 1.0f;
// actually: disable weighting?
    }
    ...
    if (weight_incomingVelocity > 0.0f) {
      int currentVelocityID =
GetVelocityID(FloatToEnumVelocity(idealAnimDescription.incomingVelocityFloat));
      ...
      rating_incomingVelocity = 1.0f - pow(NormalizedClamp(fabs(anim_incomingVelocityID -
currentVelocityID), 0, 3), 0.5f);

      // also add a penalty for anim incoming velocities which aren't between actual incoming and
anim outgoing
      ...
      if (anim_incomingVelocityID > std::max(currentVelocityID, anim_outgoingVelocityID))
rating_nonLinearVelocity -= penalty_nonLinearVelocity; if (anim_incomingVelocityID <
std::min(currentVelocityID, anim_outgoingVelocityID)) rating_nonLinearVelocity -=
penalty_nonLinearVelocity;
    }
    ...
    if (weight_outgoingMovement > 0.0f) {
      ...
      rating_outgoingMovement = 1.0f - pow(NormalizedClamp((idealAnimDescription.outgoingMovementRel
- animOutgoingMovement).GetLength(), 0.0f, sprintVelocity), 0.5f);

      // accel/decel anims are kinda chaotic so make them less likeable (HAX!)
      // if (FloatToEnumVelocity(anim->GetIncomingVelocity()) !=
FloatToEnumVelocity(anim->GetOutgoingVelocity())) rating_outgoingMovement *= 0.1f;
    }
    ...
    if (weight_incomingBodyDirection > 0.0f) {
      rating_incomingBodyDirection = 1.0f -
pow(fabs(ForceIntoAllowedBodyDirectionVec(anim->GetIncomingBodyDirection()).GetAngle2D(ForceIntoAllowedBodyDirectionVec(idealAnimDescription.incomingBodyDirectionRel)))
/ pi, 0.5f); if (FloatToEnumVelocity(anim->GetIncomingVelocity()) == e_Velocity_Idle)
rating_incomingBodyDirection = 1.0f;

      bool nonLinearCondition = false;
      // if anim incoming body dir != between (including) query incoming and anim outgoing dir, then
this anim may look annoying
      // how do we check this?
      // 1. if we look at the smallest angles between (anim incoming -> query incoming) and (anim
incoming -> anim outgoing), one has to be positive, the other negative.
      // 2. the (absolute) angles added up have to be < pi radians. else, we could be on the 'other
side' of the 'virtual half circle' and still have the former condition met radian marginRadians =
0.06f * pi; // anims can deviate a few degrees from the desired (quantized) directions Vector3
incomingBodyDirRel = anim->GetIncomingBodyDirection(); Vector3 outgoingBodyDirRel = Vector3(0, -1,
0).GetRotated2D(anim->GetOutgoingBodyAngle() + anim->GetOutgoingAngle()); radian shortestAngle1 =
incomingBodyDirRel.GetAngle2D(outgoingBodyDirRel); radian shortestAngle2 =
incomingBodyDirRel.GetAngle2D(idealAnimDescription.incomingBodyDirectionRel); if ((shortestAngle1 >
marginRadians && shortestAngle2 >  marginRadians) || (shortestAngle1 < -marginRadians &&
shortestAngle2 < -marginRadians)) { nonLinearCondition = true;
      }
      if (nonLinearCondition) {
        if (fabs(shortestAngle1) + fabs(shortestAngle2) > pi + marginRadians) nonLinearCondition =
true; else nonLinearCondition = false;
      }
    ...
    if (weight_outgoingBodyDirection > 0.0f) {
      ...
      relDesiredBodyDirection = ((idealAnimDescription.desiredLookAtAbs -
spatialState.position).Get2D().GetRotated2D(-spatialState.angle) - anim->GetTranslation() *
translationFactor).GetNormalized(Vector3(0, -1, 0));
      //relDesiredBodyDirection = ForceIntoAllowedBodyDirectionVec(relDesiredBodyDirection);
      //rating_outgoingBodyDirection = 1.0f -
pow(fabs(anim->GetOutgoingBodyDirection().GetRotated2D(anim->GetOutgoingAngle()).GetAngle2D(relDesiredBodyDirection))
/ pi, 0.5f); rating_outgoingBodyDirection = 1.0f -
pow(fabs(ForceIntoAllowedBodyDirectionVec(anim->GetOutgoingBodyDirection()).GetRotated2D(ForceIntoPreferredDirectionAngle(anim->GetOutgoingAngle())).GetAngle2D(relDesiredBodyDirection))
/ pi, 0.5f);
    }
    ...
    if (weight_side > 0.0f && weight_outgoingBodyDirection > 0.0f) {
      ...
      Vector3 animOutgoingBodyDir =
anim->GetOutgoingDirection().GetRotated2D(anim->GetOutgoingBodyAngle()); radian animRotationAngle =
animOutgoingBodyDir.GetAngle2D(animIncomingBodyDir);
      // desired direction
      ...
      if (fabs(animRotationAngle) > 0.3f * pi && fabs(desiredRotationAngle) > 0.3f * pi &&
fabs(animRotationAngle - desiredRotationAngle) > 0.06f) { rating_side = (signSide(animRotationAngle)
== signSide(desiredRotationAngle)) ? 1.0f : 0.0f; } else rating_side = 1.0f;
    }
...
bool HumanoidBase::SelectAnim(
  ...
  if (currentAnim->anim->GetVariable("outgoing_special_state") != "")
  ...
  if (dataSet.size() == 0) {
    if (command.desiredFunctionType == e_FunctionType_Movement) {
      ...
      if (Verbose())
        printf("no movement animations to begin with (humanoidbase)\n");
      dataSet.push_back(
  ...
  if (command.useDesiredMovement) {
    ...
    SetMovementSimilarityPredicate(relDesiredDirection,
                                   FloatToEnumVelocity(adaptedDesiredVelocityFloat));
  ...
  if (command.desiredFunctionType == e_FunctionType_Movement ||
    ...
    if (command.useDesiredLookAt)
      desiredBodyDirectionRel = ((command.desiredLookAt - spatialState.position)
                                     .Get2D()
                                     .GetRotated2D(-spatialState.angle) -
                                 nextAnim->GetTranslation())
  ...
  // check if we really want to requeue - only requeue movement to movement, for example, when we
         ...
         (FloatToEnumVelocity(currentAnim->anim->GetOutgoingVelocity()) == e_Velocity_Idle &&
          fabs((ForceIntoPreferredDirectionAngle(currentAnim->anim->GetOutgoingAngle()) -
                ForceIntoPreferredDirectionAngle(
                    anims->GetAnim(selectedAnimID)->GetOutgoingAngle()))) < 0.20f * pi))) {
      selectedAnimID = -1;
      if (player->GetDebug())
        printf("rejecting requeue anim for leading into the same quadrant\n");
    }
  }
    ...
    *previousAnim = *currentAnim;
    ...
    currentAnim->rotationSmuggle.begin =
        clamp(ModulateIntoRange(-pi, pi,
              ...
              -bodyRotationSmoothingMaxAngle, bodyRotationSmoothingMaxAngle);
...
void HumanoidBase::CalculatePredictedSituation(Vector3& predictedPos, radian& predictedAngle) {
  if (currentAnim->positions.size() > (unsigned int)currentAnim->frameNum) {
    ...
    predictedPos = spatialState.position +
                   currentAnim->positions.at(currentAnim->anim->GetEffectiveFrameCount()) +
                   currentAnim->actionSmuggle + currentAnim->actionSmuggleSustain +
                   currentAnim->movementSmuggle;
  } else {
    predictedPos = spatialState.position +
                   currentAnim->anim->GetTranslation().Get2D().GetRotated2D(spatialState.angle) +
                   currentAnim->actionSmuggle + currentAnim->actionSmuggleSustain +
                   currentAnim->movementSmuggle;
  }

  predictedAngle =
      spatialState.angle + currentAnim->anim->GetOutgoingAngle() + currentAnim->rotationSmuggle.end;
...
void HumanoidBase::CalculateSpatialState() {
  Vector3 position;
  if (currentAnim->positions.size() > (unsigned int)currentAnim->frameNum) {
    position = startPos + currentAnim->positions.at(currentAnim->frameNum) +
               currentAnim->actionSmuggleOffset + currentAnim->actionSmuggleSustainOffset +
               currentAnim->movementSmuggleOffset;
  } else {
    Quaternion orientation;
    currentAnim->anim->GetKeyFrame("player", currentAnim->frameNum, orientation, position);
    position.coords[2] = 0.0f;
    position = startPos + position.GetRotated2D(startAngle) + currentAnim->actionSmuggleOffset +
               currentAnim->actionSmuggleSustainOffset + currentAnim->movementSmuggleOffset;
  }
  ...
  spatialState.physicsMovement =
      spatialState.actualMovement - (spatialState.actionSmuggleMovement) -
  ...
  Vector3 quatDirection;
  quatDirection = bodyOrientation;

  Vector3 bodyDirectionVec =
      Vector3(0, -1, 0).GetRotated2D(z + startAngle + currentAnim->rotationSmuggleOffset);
  ...
  if (spatialState.enumVelocity != e_Velocity_Idle) {
    ...
    // adaptedBodyDirectionVec = (adaptedBodyDirectionVec * 0.95f + Vector3(0, -1, 0) *
    // 0.05f).GetNormalized(0); ForceIntoAllowedBodyDirectionVec(Vector3(0, -1, 0)).Print();

    bool preferCorrectVeloOverCorrectAngle =
        true;  // todo: for false, should also alter .movement, right?
    ...
    if (spatialState.enumVelocity == e_Velocity_Sprint && fabs(bodyAngleRel) >= 0.125f * pi) {
      if (preferCorrectVeloOverCorrectAngle) {
        ...
        adaptedBodyDirectionVec =
            Vector3(0, -1, 0).GetRotated2D(0.12f * pi * signSide(bodyAngleRel));
      } else {
      ...
      if (preferCorrectVeloOverCorrectAngle) {
        // on impossible combinations of velocity and body angle, decrease body angle
        adaptedBodyDirectionVec =
            Vector3(0, -1, 0).GetRotated2D(0.495f * pi * signSide(bodyAngleRel));
      } else {
        // on impossible combinations of velocity and body angle, decrease velocity
  ...
  spatialState.relBodyAngleNonquantized =
      spatialState.relBodyDirectionVecNonquantized.GetAngle2D(Vector3(0, -1, 0));
  spatialState.bodyDirectionVec = spatialState.relBodyDirectionVec.GetRotated2D(
      spatialState.angle);  // rotate back, we now have it forced into allowed angle
...
bool HumanoidBase::CompareFootSimilarity(int animIndex1, int animIndex2) const {
  int one = 1;
  int two = 1;
  if (anims->GetAnim(animIndex1)->GetCurrentFoot() == predicate_DesiredFoot)
    one = 0;
  if (anims->GetAnim(animIndex2)->GetCurrentFoot() == predicate_DesiredFoot)
    two = 0;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) == e_Velocity_Idle)
    one = 0;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetIncomingVelocity()) == e_Velocity_Idle)
    two = 0;
  return one < two;
}
...
bool HumanoidBase::CompareIncomingVelocitySimilarity(int animIndex1, int animIndex2) const {
  /* old version
  float rating1 = fabs(clamp(RangeVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) -
  EnumToFloatVelocity(predicate_IncomingVelocity), -sprintVelocity, sprintVelocity)); float rating2
  = fabs(clamp(RangeVelocity(anims->GetAnim(animIndex2)->GetIncomingVelocity()) -
  EnumToFloatVelocity(predicate_IncomingVelocity), -sprintVelocity, sprintVelocity));
  */

  int currentVelocityID = GetVelocityID(predicate_IncomingVelocity);

  // rate difference anim incoming / actual incoming
  int anim1_incomingVelocityID =
      GetVelocityID(FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()));
  int anim2_incomingVelocityID =
      GetVelocityID(FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetIncomingVelocity()));
  ...
  // also add a penalty for anim incoming velocities which aren't between actual incoming and anim
  // outgoing
  int anim1_outgoingVelocityID =
      GetVelocityID(FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetOutgoingVelocity()));
  int anim2_outgoingVelocityID =
      GetVelocityID(FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetOutgoingVelocity()));
  if (anim1_incomingVelocityID > std::max(currentVelocityID, anim1_outgoingVelocityID))
    rating1 += 0.5f;
  if (anim1_incomingVelocityID < std::min(currentVelocityID, anim1_outgoingVelocityID))
    rating1 += 0.5f;
  if (anim2_incomingVelocityID > std::max(currentVelocityID, anim2_outgoingVelocityID))
    rating2 += 0.5f;
  if (anim2_incomingVelocityID < std::min(currentVelocityID, anim2_outgoingVelocityID))
    rating2 += 0.5f;
...
void HumanoidBase::SetMovementSimilarityPredicate(const Vector3& relDesiredDirection,
  ...
  // if (relDesiredDirection.GetDotProduct(Vector3(0, -1, 0)) < 0) predicate_DesiredVelocity =
  // e_Velocity_Idle;
  //  this isn't working all too well: if targetmovement is set to 0 (aka corneringbias towards 1),
  //  both dribble @ 0 deg and dribble @ 90 deg will be the same distance (from 0), so still no
  //  preference for braking straight
  predicate_CorneringBias =
      CalculateBiasForFastCornering(Vector3(0, -1.0f * spatialState.floatVelocity, 0),
                                    relDesiredDirection * EnumToFloatVelocity(desiredVelocity),
...
float HumanoidBase::GetMovementSimilarity(int animIndex, const Vector3& relDesiredDirection,
  ...
  Vector3 outgoingDirection =
      ForceIntoPreferredDirectionVec(anims->GetAnim(animIndex)->GetOutgoingDirection());
  ...
  // anims that end at lower velocities have an advantage: they don't get dragged into the
  // currentmovement that much thus: have a bias that is higher at higher outgoing velocities, which
  // means anim outgoingmovement gets more % of current movement and less % of their own disabled
  // for now - the new physics system disregards most of the anims movement anyway :) *enabled
  // again: altered the physics system so that it will regard anim movement more, so this became
  // useful again for proper hard cornering
  ...
  // desiredMovement = desiredMovement.GetNormalized(0) * clamp(desiredMovement.GetLength() - 0.8f,
  ...
  value -= fabs(relDesiredDirection.GetDotProduct(outgoingDirection)) *
           4.0f;  // prefer straight lines (towards/away from desired outgoing)

  /* disabled quantization:
    // maximum quantization (20 degree angle @ dribble velocity, smallest distance to be measured)
    // faulty calculation (cornering distance instead of straight line) for optimization, makes
    little difference anyway for 20 degrees float maxQuant = (dribbleVelocity * 2.0f * pi) / (360.0f
    / 20.0f);// * (1.0 - velocityBias * 0.5) // == ~1.22 at the moment of writing
    //float maxQuant = sqrt(2.0f * pow(dribbleVelocity, 2.0f) * (1.0f - cos(pi / 180.0f * 20.0f)));
    // the actual correct calculation (straight line)
  ...
  */
  ...
  // if (fabs(outgoingDirection.GetAngle2D() - relDesiredDirection.GetAngle2D()) > pi) value +=
  // 100000;
...
bool HumanoidBase::CompareMovementSimilarity(int animIndex1, int animIndex2) const {
  float rating1 = GetMovementSimilarity(animIndex1, predicate_RelDesiredDirection,
                                        predicate_DesiredVelocity, predicate_CorneringBias);
  float rating2 = GetMovementSimilarity(animIndex2, predicate_RelDesiredDirection,
                                        predicate_DesiredVelocity, predicate_CorneringBias);
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareDirectionSimilarity(int animIndex1, int animIndex2) const {
  float rating1 = fabs(
      predicate_RelDesiredDirection.GetAngle2D(anims->GetAnim(animIndex1)->GetOutgoingDirection()));
  float rating2 = fabs(
      predicate_RelDesiredDirection.GetAngle2D(anims->GetAnim(animIndex2)->GetOutgoingDirection()));
  return rating1 < rating2;
}

bool HumanoidBase::CompareOutgoingVelocitySimilarity(int animIndex1, int animIndex2) const {
  float rating1 = fabs(clamp(RangeVelocity(anims->GetAnim(animIndex1)->GetOutgoingVelocity()) -
                                 EnumToFloatVelocity(predicate_DesiredVelocity),
                             -sprintVelocity, sprintVelocity));
  float rating2 = fabs(clamp(RangeVelocity(anims->GetAnim(animIndex2)->GetOutgoingVelocity()) -
                                 EnumToFloatVelocity(predicate_DesiredVelocity),
                             -sprintVelocity, sprintVelocity));
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareIncomingBodyDirectionSimilarity(int animIndex1, int animIndex2) const {
  float rating1 =
      fabs(ForceIntoAllowedBodyDirectionVec(anims->GetAnim(animIndex1)->GetIncomingBodyDirection())
               .GetAngle2D(ForceIntoAllowedBodyDirectionVec(predicate_RelIncomingBodyDirection))) /
      pi;
  float rating2 =
      fabs(ForceIntoAllowedBodyDirectionVec(anims->GetAnim(animIndex2)->GetIncomingBodyDirection())
               .GetAngle2D(ForceIntoAllowedBodyDirectionVec(predicate_RelIncomingBodyDirection))) /
      pi;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) == e_Velocity_Idle)
    rating1 = 0;  //-1;
...
bool HumanoidBase::CompareBodyDirectionSimilarity(int animIndex1, int animIndex2) const {
  ...
  Vector3 relDesiredBodyDirection1 =
      ((predicate_LookAt - spatialState.position).GetRotated2D(-spatialState.angle) -
       a1->GetTranslation() * translationFactor)
          .GetNormalized(Vector3(0, -1, 0));
  Vector3 relDesiredBodyDirection2 =
      ((predicate_LookAt - spatialState.position).GetRotated2D(-spatialState.angle) -
       a2->GetTranslation() * translationFactor)
          .GetNormalized(Vector3(0, -1, 0));

  /* this is quirky somehow; when accelerating, sometimes the humanoid will actually go 135 when it
  shouldn't. could debug it, but then again, maybe KISS is a better idea; this whole setup seems a
  bit overly complicated. probably should look for solution elsewhere
  ...
  // doesn't work when ending idle
    ...
    if (anims->GetAnim(animIndex1)->GetOutgoingVelocity() > idleDribbleSwitch &&
      ...
      relDesiredBodyDirection1 = Vector3(0, -1, 0).GetRotated2D(d1 * (1.0f - origAngleBias) + a1 *
  origAngleBias);
      ...
      relDesiredBodyDirection2 = Vector3(0, -1, 0).GetRotated2D(d2 * (1.0f - origAngleBias) + a2 *
  origAngleBias);
    }
  }
  */

  // this version corrects for rotation smuggle; we will probably end up walking in a direction a
  // bit rotated towards the user desired direction, instead of pure anim direction. we need some
  // heuristic to use that fact, because else, we may end up looking in the wrong direction somewhat
  // radian outgoingAngle1 = FixAngle(((a1->GetOutgoingDirection() + predicate_RelDesiredDirection *
  // 0.2f).GetNormalized()).GetAngle2D()); radian outgoingAngle2 =
  // FixAngle(((a2->GetOutgoingDirection() + predicate_RelDesiredDirection *
  // 0.2f).GetNormalized()).GetAngle2D());
  radian maxAngleSmuggle =
      0.1f *
      pi;  // mind you, for anims ending idle, this shouldn't be >= 0.125f * pi, because then we end
           // up allowing multiple outgoing angles. after all, outgoing direction will be skewed by
           // this value as max, which will make outgoing body direction follow in the same dir but
           // since idle anims don't have outgoing body directions, it will just change their
           // outgoing directions and they will be pointing in exactly the same direction
  radian outgoingAngle1 =
      a1->GetOutgoingDirection()
          .GetRotated2D(clamp(predicate_RelDesiredDirection.GetAngle2D(a1->GetOutgoingDirection()),
                              -maxAngleSmuggle, maxAngleSmuggle))
          .GetAngle2D(Vector3(0, -1, 0));
  radian outgoingAngle2 =
      a2->GetOutgoingDirection()
          .GetRotated2D(clamp(predicate_RelDesiredDirection.GetAngle2D(a2->GetOutgoingDirection()),
                              -maxAngleSmuggle, maxAngleSmuggle))
          .GetAngle2D(Vector3(0, -1, 0));
  Vector3 predictedOutgoingBodyDirection1 =
      a1->GetOutgoingBodyDirection().GetRotated2D(outgoingAngle1);
  Vector3 predictedOutgoingBodyDirection2 =
      a2->GetOutgoingBodyDirection().GetRotated2D(outgoingAngle2);
  ...
  // penalty for body angles (as opposed to straight forward), to get a slight preference for
  // forward angles
...
bool HumanoidBase::CompareTripDirectionSimilarity(int animIndex1, int animIndex2) const {
  float rating1 = -GetVectorFromString(anims->GetAnim(animIndex1)->GetVariable("bumpdirection"))
                       .GetDotProduct(predicate_RelDesiredTripDirection);
  float rating2 = -GetVectorFromString(anims->GetAnim(animIndex2)->GetVariable("bumpdirection"))
                       .GetDotProduct(predicate_RelDesiredTripDirection);
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareBallDirectionSimilarity(int animIndex1, int animIndex2) const {
  float rating1 = -GetVectorFromString(anims->GetAnim(animIndex1)->GetVariable("balldirection"))
                       .Get2D()
                       .GetDotProduct(predicate_RelDesiredBallDirection);
  float rating2 = -GetVectorFromString(anims->GetAnim(animIndex2)->GetVariable("balldirection"))
                       .Get2D()
                       .GetDotProduct(predicate_RelDesiredBallDirection);
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareNumericVariable(int animIndex1, int animIndex2) const {
  return fabs(atof(anims->GetAnim(animIndex1)
                       ->GetVariable(predicate_NumericVariableName.c_str())
                       .c_str()) -
              predicate_NumericVariableValue) <
         fabs(atof(anims->GetAnim(animIndex2)
                       ->GetVariable(predicate_NumericVariableName.c_str())
                       .c_str()) -
              predicate_NumericVariableValue);
}
...
Vector3 HumanoidBase::CalculatePhysicsVector(Animation* anim, bool useDesiredMovement,
  ...
  if (anim->GetVariable("incoming_special_state").compare("") != 0 ||
      anim->GetVariable("outgoing_special_state").compare("") != 0)
    outgoingSwitchBias = 1.0f;

  Vector3 animIncomingMovement = Vector3(0, -1, 0).GetRotated2D(spatialState.angle) *
                                 RangeVelocity(anim->GetIncomingVelocity());
  Vector3 adaptedCurrentMovement = animIncomingMovement * incomingSwitchBias +
                                   spatialState.movement * (1.0f - incomingSwitchBias);
  ...
  Vector3 velocifiedDesiredMovement =
      (useDesiredMovement) ? desiredMovement : predictedOutgoingMovement;

  assert(desiredMovement.coords[2] == 0.0f);

  Vector3 adaptedDesiredMovement = predictedOutgoingMovement * outgoingSwitchBias +
                                   velocifiedDesiredMovement * (1.0f - outgoingSwitchBias);
  ...
  float difficultyPenaltyFactor =
      std::pow(clamp((difficultyFactor - 0.0f) *
                         (1.0f - (stat_agility * 0.2f + stat_acceleration * 0.2f)) * 2.0f,
                     0.0f, 1.0f),
               0.7f);
  ...
  float powerFactor =
      1.0f -
      clamp(
          std::pow(player->GetLastTouchBias(1000), 0.8f) * (0.8f - stat_dribble * 0.3f), 0.0f,
          0.4f);  // todo: put lasttouchbias thing in loop, so it'll change over time (in that loop)
  // moved to per ms timeloop penalty
  powerFactor *= 1.0f - clamp(decayingPositionOffset.GetLength() *
                                      (10.0f - player->GetStat("physical_balance") * 5.0f) -
                                  0.1f,
                              0.0f, 0.3f);
  ...
  if (touch) {
    float bonus =
        1.0f - std::pow(NormalizedClamp(
                            (adaptedCurrentMovement + predictedOutgoingMovement).GetLength() * 0.5f,
                            0, sprintVelocity),
                        0.8f) *
                   0.8f;
    bonus *=
        0.6f + 0.4f * player->GetStat("technical_ballcontrol");  // todo: shouldn't this be agility?
  ...
  if (mod_AllowRotation && physicsBias > 0.0f) {
    ...
    if (FloatToEnumVelocity(predictedOutgoingMovement.GetLength()) == e_Velocity_Idle)
      animOutgoingVector = anim->GetOutgoingDirection().GetRotated2D(spatialState.angle);
    Vector3 desiredVector = adaptedDesiredMovement.GetNormalized(0);
    if (FloatToEnumVelocity(adaptedDesiredMovement.GetLength()) == e_Velocity_Idle)
    ...
    if (fabs(toDesiredAngle) <= 0.5f * pi ||
      ...
      if (fabs(animChange) > 0.06f * pi) {
        ...
        if (signSide(toDesiredAngle) == sign) {
          toDesiredAngle_capped =
              clamp(toDesiredAngle, -maxAngleMod_overAnimAngle, maxAngleMod_overAnimAngle);
        } else {
          toDesiredAngle_capped =
              clamp(toDesiredAngle, -maxAngleMod_underAnimAngle, maxAngleMod_underAnimAngle);
        }
      } else {
        // straight ahead anim, has no specific side.
        toDesiredAngle_capped =
            clamp(toDesiredAngle, -maxAngleMod_straightAnimAngle, maxAngleMod_straightAnimAngle);
      }
      // printf("animChange: %f radians, to desired angle: %f radians, sign: %i, sign side of to
      // desired angle: %i, resulting capped angle: %f radians\n", animChange, toDesiredAngle, sign,
      // signSide(toDesiredAngle), toDesiredAngle_capped);
    }
  }
  ...
  if (mod_CorneringBraking) {
    ...
    brakeBias *=
        std::pow(NormalizedClamp(spatialState.floatVelocity, idleVelocity, sprintVelocity - 0.5f),
                 0.8f);  // 0.5f);
    float maxVelo =
        sprintVelocity *
        ((1.0f - brakeBias) +
         ((1.0f -
           std::pow(fabs(animOutgoingMovement.GetNormalized(0).GetAngle2D(Vector3(0, -1, 0)) / pi),
                    0.5f)) *
          brakeBias));
  ...
  for (int time_ms = 0; time_ms < anim->GetFrameCount() * 10; time_ms += timeStep_ms) {
    // start with +1, because we want to influence the first frame as well
    // as for finishing, finish with frameBias = 1.0, even if the last frame is 'spiritually' the
    // one-to-last, since the first frame of the next anim is actually 'same-tempered' as the
    // current anim's last frame. however, it works best to have all values 'done' at this
    // one-to-last frame, so the next anim can read out these correct (new starting) values.
    ...
    if (mod_PointinessCurve && physicsBias > 0.0f &&
      ...
      if (touch && time_ms < animTouchFrame * 10)
        lagExp = std::max(
            lagExp,
            0.7f);  // else, we could 'miss' the ball because we're already turned around too much
    ...
    Vector3 animMovement =
        CalculateMovementAtFrame(origPositionCache,
                                 anim->GetEffectiveFrameCount() * adaptedFrameBias, 1)
            .GetRotated2D(spatialState.angle);
    ...
    float maxSlower = 1.6f;  // rationale: don't want to end up below dribbleVelocity -
                             // idleDribbleSwitch (= change velocity)
    if (touch)
      maxSlower = 1.2f;
    float maxFaster = 0.0f;
    if (touch)
      maxFaster = 0.0f;
    if (temporalMovement.GetLength() > adaptedAnimVelo)
      maxFaster = std::min(
          0.0f + 1.0f * (1.0f - frameBias),
          std::max(maxFaster,
                   temporalMovement.GetLength() -
                       adaptedAnimVelo));  // ..already going faster.. well okay, allow this
    if (maxFaster > 0)
    ...
    adaptedAnimVelo =
        clamp(desiredVelocity, adaptedAnimVelo - maxSlower, adaptedAnimVelo + maxFaster);
    adaptedAnimMovement = adaptedAnimMovement.GetNormalized(0) * adaptedAnimVelo;

    if (mod_CorneringBraking) {
      float frameBiasedMaximumOutgoingVelocity =
          sprintVelocity * (1.0f - frameBias) + maximumOutgoingVelocity * frameBias;
    ...
    if (mod_MaximumAccelDecel) {
      // this is basically meant to enforce transitions to be smoother, disallowing bizarre steps.
      // however, with low enough max values, it can also serve as a physics slowness thing. in that
      // regard, the maxaccel part is somewhat similar to the air resistance mod below. they can
      // live together; this one can serve as a constant maximum, and the air resistance as a
      // velocity-based maximum. update: decided to not let them live together, this should now
      // purely be used for capping transition speed
      ...
      if (veloChangeMPS < -maxDecelMPS || veloChangeMPS > maxAccelMPS) {
        adaptedAnimVelo = currentVelo + clamp(veloChangeMPS, -maxDecelMPS, maxAccelMPS) *
                                            ((float)timeStep_ms * 0.001f);
        adaptedAnimMovement = adaptedAnimMovement.GetNormalized(0) * adaptedAnimVelo;
      }
    }
    ...
    // angle
    resultingPhysicsMovement = resultingPhysicsMovement.GetRotated2D(
        toDesiredAngle_capped * frameBias);  // * pow(frameBias, 0.6f));

    // --- stay true to anim?
    // -----------------------------------------------------------------------------------------------------------------------

    resultingPhysicsMovement =
        resultingPhysicsMovement * physicsBias + animMovement * (1.0f - physicsBias);
    ...
    if (mod_BrakeOnTouch) {
      ...
      // (precalc at touchframe, because temporalMovement will change because of this, so if we
      // don't precalc then changing numBrakeFrames will change the amount of effect)
      int numBrakeFrames = 15;
      if (touch && time_ms >= animTouchFrame * 10 &&
          time_ms < (animTouchFrame + numBrakeFrames) * 10) {
        int brakeFramesInto = (time_ms - (animTouchFrame * 10)) / 10;
        float brakeFrameFactor = std::pow(1.0f - (brakeFramesInto / (float)numBrakeFrames), 0.5f);

        float touchBrakeFactor = 0.3f;

        float touchDifficultyFactor =
            clamp((difficultyFactor + 0.7f) * (1.0f - stat_dribble * 0.4f), 0.0, 1.0f);
        touchDifficultyFactor *=
            1.0f - std::pow(fabs(anim->GetOutgoingAngle()) / pi,
                            0.75f);  // don't help with braking (when going nearer 180 deg)

        float veloFactor =
            NormalizedClamp(temporalMovement.GetLength(), walkVelocity, sprintVelocity);

        penaltyBreakFactor =
            touchDifficultyFactor * veloFactor * brakeFrameFactor * touchBrakeFactor;
      }
    }

    /* part of new method, but needs some debugging/unittesting
    // increase over multiple frames for smoother effect, and so we get a proper effect even if
    maxChange isn't very high int numBrakeFrames = 5; if (touch && time_ms >= animTouchFrame * 10 &&
    time_ms < (animTouchFrame + numBrakeFrames) * 10) { int framesInto = (time_ms - (animTouchFrame
    * 10)) / 10; toDesired += -temporalMovement.GetNormalized(0) * (ballTouchSlowdownAmount /
    (float)(numBrakeFrames - framesInto));
      // if (player->GetDebug()) {
      //   printf("divisor: %i, frame %i, time: %i, vector: ", numBrakeFrames, animTouchFrame,
    time_ms);
    ...
    */
    ...
    if (mod_MaxCornering) {
      ...
      if (temporalMovement.GetLength() > startVelo && predictedMovement.GetLength() > startVelo) {
        radian angle =
            predictedMovement.GetNormalized().GetAngle2D(temporalMovement.GetNormalized());
        ...
        float veloFactor =
            std::pow(NormalizedClamp(temporalMovement.GetLength(), 0, sprintVelocity), 1.0f);
        ...
        if (fabs(angle) > maxAngle) {
          ...
          if (mode == 0) {
            Vector3 restrictedPredictedMovement =
                predictedMovement.GetRotated2D((fabs(angle) - maxAngle) * -signSide(angle));
    ...
    if (mod_MaxChange) {
      ...
      // if (touch && time_ms >= animTouchFrame * 10) {
      //   int influenceFrames = 16; // number of frames to slow down on each 'side' of the
      //   balltouch
      //   //float frameBias = NormalizedClamp(fabs((animTouchFrame * 10) - time_ms), 0,
      //   influenceFrames * 10) * 0.7f + 0.3f; float frameBias = NormalizedClamp(time_ms -
      //   (animTouchFrame * 10), 0, influenceFrames * 10) * 1.0f;// + 0.1f; frameBias =
      //   curve(frameBias, 1.0f); maxChange *= clamp(frameBias, 0.1f, 1.0f);
      // }
    ...
    if (mod_AirResistance && animType != "sliding" && animType != "deflect") {
      ...
      if ((temporalMovement + toDesired).GetLength() > falloffStartVelo) {
        ...
        // wolfram alpha to show difference in sin before/after exp order: 10 * (sin(((1.0 - x
        // ^ 2.0) - 0.5) * pi) * 0.5 + 0.5), 10 * ((1.0 - (sin((x - 0.5) * pi) * 0.5 + 0.5)) ^ 2.0)
        // | from x = 0.0 to 1.0

        float veloAirResistanceFactor =
            clamp(std::pow(clamp((temporalMovement.GetLength() - falloffStartVelo) /
        ...
        // veloAirResistanceFactor = 1.0f - ((1.0f - veloAirResistanceFactor) * pow(sin(frameBias *
        // pi), 2.0f));

        // circular version
        Vector3 forwardVector;
        if ((temporalMovement + toDesired).GetLength() >
            temporalMovement.GetLength()) {  // outside the 'velocity circle'
        ...
        float maxAccelerationMPS =
            accelPower * (1.0f - veloAirResistanceFactor) * (stat_acceleration * 0.3f + 0.7f);
    ...
    if (time_ms >= (anim->GetFrameCount() - 2) * 10) {
      ...
      if (!hardQuantize && anim->GetVariable("outgoing_special_state").compare("") != 0)
        hardQuantize = true;

      if (!hardQuantize) {
        // soft version
        if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) == e_Velocity_Idle &&
            FloatToEnumVelocity(tmpTemporalMovement.GetLength()) != e_Velocity_Idle)
          tmpTemporalMovement.NormalizeTo(idleDribbleSwitch - 0.01f);
        else if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) != e_Velocity_Idle &&
                 FloatToEnumVelocity(tmpTemporalMovement.GetLength()) == e_Velocity_Idle)
          tmpTemporalMovement = anim->GetOutgoingMovement()
                                    .GetRotated2D(spatialState.angle)
                                    .GetNormalizedTo(idleDribbleSwitch + 0.01f);
      } else {
        // hard version
        if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) == e_Velocity_Idle &&
            FloatToEnumVelocity(tmpTemporalMovement.GetLength()) != e_Velocity_Idle)
          tmpTemporalMovement = 0;
        else if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) != e_Velocity_Idle &&
                 FloatToEnumVelocity(tmpTemporalMovement.GetLength()) == e_Velocity_Idle)
          tmpTemporalMovement = anim->GetOutgoingMovement()
                                    .GetRotated2D(spatialState.angle)
                                    .GetNormalizedTo(dribbleVelocity);
      }
    }
  ...
  if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) != e_Velocity_Idle &&
    ...
    rotationOffset_ret =
        resultingMovement.GetRotated2D(-spatialState.angle).GetAngle2D(anim->GetOutgoingMovement());
  } else {
  ...
  if (mod_CheatBodyDirection && useDesiredBodyDirection && animType == "movement") {
    ...
    radian predictedAngleRel =
        anim->GetOutgoingAngle() + anim->GetOutgoingBodyAngle() + rotationOffset_ret;
    radian desiredRotationOffset =
        desiredBodyDirectionRel.GetRotated2D(-predictedAngleRel).GetAngle2D(Vector3(0, -1, 0));

    if (fabs(desiredRotationOffset) < 0.5f * pi) {  // else: too much

      float outgoingVelocityFactorInv =
          1.0f -
          NormalizedClamp(resultingMovement.GetLength(), idleDribbleSwitch, sprintVelocity - 1.0f) *
              1.0f;
      float animLengthFactor = NormalizedClamp(anim->GetFrameCount(), 0, 25);
      radian maximizedRotationOffset =
          clamp(desiredRotationOffset,
      ...
      //   positions_ret.back() + Vector3(0, -1, 0).GetRotated2D(spatialState.angle +
      //   predictedAngleRel + rotationOffset) * 3.1f); SetRedDebugPilon(spatialState.position +
      //   positions_ret.back() + Vector3(0, -1, 0).GetRotated2D(spatialState.angle +
      //   predictedAngleRel + rotationOffset + maximizedRotationOffset) * 3.2f);
      // }
      ...
      // mee bezig. zie ook eerder in deze functie, moet ook aangepast, maximum angle shizzle, wil
      // niet over 180 graden if (fabs(predictedAngleRel + rotationOffset) > 0.75f * pi &&
      // SignSide(maximizedRotationOffset) == SignSide(predictedAngleRel)) {
      // }
    }
  }
