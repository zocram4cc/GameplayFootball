  ...
  */
  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    ...
    if ((omitLuxuryAnims && files.at(i).find("luxury") != std::string::npos) ||
      ...
      // Every clip is held twice, as authored and mirrored. This parsed the
      // file once per copy: the collection is 453 MB of .anim text, so half a
      // gigabyte was read and parsed a second time to produce something that is
      // just the first one reflected. Parse once and mirror a copy instead -
      // the autogenerated clips above already build their variants this way.
      //
      // Both are still mirrored and prepared exactly as before: the copy is
      // taken from the freshly parsed clip, before _PrepareAnim touches either.
      ...
      // The copy constructor deliberately does not carry extensions across, so
      // the mirrored clip is given its own.
