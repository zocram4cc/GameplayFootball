  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    ...
    if ((omitLuxuryAnims && files.at(i).find("luxury") != std::string::npos) ||
      ...
      for (int mirror = 0; mirror < 2; mirror++) {
        ...
        // duplicate dribble anims with > 45 degree body directions (either in or out) and convert
        duplicate to walking speed
        // this is because of the decision to allow 135 degree body directions ('walking backward')
        on walking velocities.
        // more correct (to get proper leg movement for walking velocities) would be to create
        separate anims for these, but i'm feeling lazy if
        (animation->GetAnimType().compare("movement") == 0) { if
        (fabs(animation->GetIncomingBodyAngle()) > 0.5 * pi ||
        fabs(animation->GetOutgoingBodyAngle()) > 0.5 * pi) { if
        (FloatToEnumVelocity(animation->GetIncomingVelocity()) == e_Velocity_Dribble ||
        FloatToEnumVelocity(animation->GetOutgoingVelocity()) == e_Velocity_Dribble) {

              Animation *animation2 = new Animation();
              std::shared_ptr<FootballAnimationExtension> extension(new
        FootballAnimationExtension(animation)); animation2->AddExtension("football", extension);
