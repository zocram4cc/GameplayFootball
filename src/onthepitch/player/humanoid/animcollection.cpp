...
void FillNodeMap(boost::intrusive_ptr<Node> targetNode,
  ...
  nodeMap.insert(
      std::pair<std::string, boost::intrusive_ptr<Node>>(targetNode->GetName(), targetNode));
...
AnimCollection::AnimCollection(std::shared_ptr<Scene3D> scene3D) : scene3D(scene3D) {
  defString[0] = "";
  ...
  // idle - dribble - walk - run, in combination with angles: 0 - 20 - 45 - 90 - 135 - 180 (and
  // their negatives, where applicable) so every anim falls into one of these quadrants.
...
void GenerateAutoAnims(const std::vector<Animation*>& templates,
  ...
  for (unsigned int t1 = 0; t1 < templates.size(); t1++) {
    for (unsigned int t2 = 0; t2 < templates.size(); t2++) {
      ...
      for (unsigned int direction = 0; direction < 9; direction++) {
        ...
        // max deceleration
        // use dot product to make sure "running 180 degrees" isn't considered "keeping the same
        // velocity" and therefore legal.
        float dot = Vector3(0, -1, 0).GetDotProduct(Vector3(0, -1, 0).GetRotated2D(
            angle));  // todo: this is optimizable (simple angle -> dot)
        int veloIDDiff_dotted = round(outgoingVeloID * dot - incomingVeloID);
        if (veloIDDiff_dotted < -3)
          legalAnim = false;

        if (incomingVeloID == 3 && outgoingVeloID == 1 &&
            (fabs(outgoingBodyAngleT2) > 0.25f * pi + margin || fabs(angle) > 0.25f * pi + margin))
          legalAnim = false;  // no sprint to dribble with backwards body angle
        // if (incomingVeloID == 3 && outgoingVeloID == 0 && fabs(angle) > 0.25f * pi) legalAnim =
        // false; // no sprint to idle with big angle

        // experimental block
        if (incomingVeloID > 0 && outgoingVeloID > 0 && fabs(angle) > 0.50f * pi + margin)
          legalAnim = false;
        // if (incomingVeloID > 0 && outgoingVeloID == 0 && fabs(outgoingBodyAngleT2) > 0.50f * pi +
        // margin) legalAnim = false;
        //  if (incomingVeloID == 3 && outgoingVeloID == 2 && (fabs(outgoingBodyAngleT2) > 0.3f * pi
        //  || fabs(angle) > 0.25f * pi)) legalAnim = false;
        // zzif (incomingVeloID == 3 && outgoingVeloID == 0) legalAnim = false;
        // if (incomingVeloID == 3 && outgoingVeloID == 2 && fabs(angle) > 0.25f * pi) legalAnim =
        // false; if (fabs(outgoingBodyAngleT2 - incomingBodyAngleT1 + angle) > 0.5f * pi) legalAnim
        // = false;

        // if (incomingVeloID + outgoingVeloID > 5 && fabs(angle) > 0.25f * pi) legalAnim = false;
        // // sprint -> sprint if (incomingVeloID + outgoingVeloID > 4 && fabs(angle) > 0.5f * pi)
        // legalAnim = false; // walk -> sprint && sprint -> walk
        if (incomingVeloID + outgoingVeloID > 5 && fabs(angle) > 0.25f * pi + margin)
          legalAnim = false;  // sprint -> sprint
        if (incomingVeloID + outgoingVeloID > 4 && fabs(angle) > 0.25f * pi + margin)
          legalAnim = false;  // walk -> sprint && sprint -> walk
        // if (incomingVeloID + outgoingVeloID > 3 && fabs(angle) > 0.50f * pi + margin) legalAnim =
        // false; // walk -> walk && sprint -> dribble && dribble -> sprint if (incomingVeloID +
        // outgoingVeloID > 2 && fabs(angle) > 0.75f * pi + margin) legalAnim = false; // dribble ->
        // walk && walk -> dribble if (incomingVeloID > 0 && outgoingVeloID > 0 && fabs(angle) >
        // 0.5f * pi + margin) legalAnim = false; // dribble -> walk && walk -> dribble

        radian bodyAngleDelta =
            ModulateIntoRange(-pi, pi, outgoingBodyAngleT2 - incomingBodyAngleT1);
        if (fabs(angle + bodyAngleDelta) > 1.0f * pi + margin)
          legalAnim = false;  // don't rotate over 180 degrees (we've got an anim for that the
                              // shorter away around anyway)
        // if (fabs(angle + bodyAngleDelta) > 0.5f * pi + margin) legalAnim = false; // just don't
        // turn too fast

        // if (fabs(bodyAngleDelta) > 0.9f * pi) legalAnim = false; // body rotation limit: if we
        // allow 180 degrees, the slerp doesn't know whether it should go CW or CCW (seems to be
        // fixed by the ModulateIntoRange above - why?) if (fabs(veloIDDiff) > 2 && fabs(angle +
        // bodyAngleDelta) > 0.25f * pi) legalAnim = false; // don't brake AND turn all too much
        //|__[R]__ autogen [v2 b-135] => [v2 b45 a-179]_mirror
        // if (incomingVeloID + outgoingVeloID > 3 && fabs(angle) > 0.50f * pi) legalAnim = false;

        /* too uncontrollable
                Vector3 movementChangeVec = (Vector3(0, -1, 0) * incomingVelocityT1) - (Vector3(0,
           -1, 0).GetRotated2D(angle) * outgoingVelocityT2); Vector3 bodyDirChangeVec = (Vector3(0,
           -1, 0).GetRotated2D(incomingBodyAngleT1)) - (Vector3(0, -1, 0).GetRotated2D(angle +
           outgoingBodyAngleT2)); Vector3 totalChangeVec = movementChangeVec + bodyDirChangeVec
           * 1.0f; if (totalChangeVec.GetLength() > sprintVelocity) legalAnim = false;
        */
        ...
        if (legalAnim == true) {
          ...
          gen->SetName("autogen [v" +
                       int_to_str(GetVelocityID(FloatToEnumVelocity(incomingVelocityT1))) + " b" +
                       int_to_str(incomingBodyAngleT1 / pi * 180) + "] => [v" +
                       int_to_str(GetVelocityID(FloatToEnumVelocity(outgoingVelocityT2))) + " b" +
                       int_to_str(outgoingBodyAngleT2 / pi * 180) + " a" +
          ...
          for (unsigned int n = 0; n < nodeAnimsT1.size(); n++) {
            ...
            Vector3 movementChangeMPS =
                (outgoingMovement - anim1->GetIncomingMovement()) * (100.0f / frameCount);
            ...
            while (animKeyIter != animationT1.end()) {
              keyFrames.push_back(animKeyIter->first);
              animKeyIter++;
            }
            ...
            if (n == 0) {
              // make sure there's 2 position keyframes close to each other at the start and at the
              // end, so we will have the right ingoing and outgoing velocities
            ...
            while (keyIter != keyFrames.end()) {
              ...
              float origBias =
                  clamp(frame - 1.0f, 0.0f, frameCount - 3.0f) /
                  ((float)frameCount - 3.0f);  // version that ignores first and last 2 frames, so
                                               // incoming/outgoing velo/angle will be correct
              // bias = pow(bias, 0.7f + pow(averageVeloFactor, 2.0) * 0.2f); // move a bit earlier
              // bias = curve(bias, 1.0f - pow(averageVeloFactor, 2.0) * 0.8f); // concentrate
              // change in the middle of the anim
              float bias = origBias;
              ...
              bias =
                  pow(bias,
                      1.0f * (0.3f + 0.4f * averageVeloFactor +
                              0.3f * NormalizedClamp(movementChangeMPS.GetLength(), 0.0f, 20.0f)));
              ...
              if (n == 1) {  // body
                ...
                angleQuat.SetAngleAxis(angle * pow((bias * 0.7f + origBias * 0.3f), 1.0f),
                                       Vector3(0, 0, 1));
                ...
                movementChangeMPS *=
                    0.5f +
                    0.5f * ((anim1->GetIncomingMovement() * (1.0f - bias) + outgoingMovement * bias)
                                .GetLength() /
                            sprintVelocity);  // high velo = more leaning into the wind and such :p
                Quaternion leanQuat;
                leanQuat.SetAngleAxis(
                    movementChangeMPS.GetLength() * leanAmount * (0.5f + 0.5f * sin(origBias * pi)),
                    Vector3(0, 1, 0).GetRotated2D(movementChangeMPS.GetNormalized(0).GetAngle2D()));
              ...
              gen->SetKeyFrame(
                  nodeAnimsT1.at(n)->nodeName, int(floor(targetFrame)), orientation,
                  cumulativePosition * (1.0f / animSpeedFactor) + Vector3(0, 0, height));
          ...
          // enable this to save all the autogenerated anims to files, so that we can use them as a
          // base for manual anims. don't forget to disable again after usage ;)
          // gen->Save("media/animations/debug_luxury/autogen v" +
          // int_to_str(GetVelocityID(FloatToEnumVelocity(incomingVelocityT1))) + " b" +
          // int_to_str(round(incomingBodyAngleT1 / pi * 180)) + " _to_ v" +
          // int_to_str(GetVelocityID(FloatToEnumVelocity(outgoingVelocityT2))) + " b" +
          // int_to_str(round(outgoingBodyAngleT2 / pi * 180)) + " a" + int_to_str(angle / pi * 180)
          // + ".anim");
  ...
  Log(e_Notice, "AnimCollection", "GenerateAutoAnims",
      int_to_str(autoAnims.size()) + " autogenerated anims! huzzah!");
}

void AnimCollection::Load(std::filesystem::path directory) {
  ...
  */
  ...
  for (unsigned int i = 0; i < autoAnims.size(); i++) {
    ...
    std::shared_ptr<FootballAnimationExtension> extension(
  ...
  for (unsigned int i = 0; i < files.size(); i++) {
    if ((omitLuxuryAnims && files.at(i).find("luxury") != std::string::npos) ||
      ...
      continue;
    wanted.push_back(&files.at(i));
  }

  // Parsing is the whole cost of this loop - 453 MB of text - and every clip
  // parses independently of every other, touching only itself. So the parse
  // runs on every core and only what follows it, which walks the shared player
  // node, stays on this thread. Results are kept in file order, so the
  // collection comes out in the same order it always did.
  std::vector<Animation*> parsed(wanted.size(), nullptr);
  {
    std::atomic<size_t> next{0};
    auto parse = [&]() {
      for (size_t i; (i = next.fetch_add(1)) < wanted.size();) {
        const std::string& file = *wanted[i];
    ...
    const unsigned int threadCount = std::max(
        1u, std::min(std::thread::hardware_concurrency(), (unsigned int)wanted.size()));
    std::vector<std::thread> threads;
    threads.reserve(threadCount);
    for (unsigned int t = 0; t < threadCount; t++) threads.emplace_back(parse);
    for (std::thread& thread : threads) thread.join();
  }

  for (size_t i = 0; i < parsed.size(); i++) {
    Animation* animation = parsed[i];
    if (Verbose())
      printf("%s\n", wanted[i]->c_str());

    // Every clip is held twice, as authored and mirrored. The mirror is the
    // parsed clip reflected, so it is a copy rather than a second parse - the
    // autogenerated clips above already build their variants this way. The copy
    // constructor deliberately does not carry extensions across, so the mirrored
    // clip is given its own. Both are prepared exactly as before, and the copy
    // is taken before _PrepareAnim touches either.
    Animation* mirrored = new Animation(*animation);
    std::shared_ptr<FootballAnimationExtension> mirroredExtension(
        new FootballAnimationExtension(mirrored));
    mirrored->AddExtension("football", mirroredExtension);
    mirrored->Mirror();

    _PrepareAnim(animation, playerNode, bodyParts, nodeMap, false);
    _PrepareAnim(mirrored, playerNode, bodyParts, nodeMap, false);
  }
    ...
    /*
        ...
        if (selectAnim) {
          if (query.byFoot == true) {
            if (animations.at(i)->GetCurrentFoot() != query.foot &&
       FloatToEnumVelocity(animations.at(i)->GetIncomingVelocity()) != e_Velocity_Idle) selectAnim =
       false;
          }
        }
    */
    ...
    if (selectAnim) {
      if (query.byIncomingVelocity == true) {
        e_Velocity animIncomingVelocity =
            FloatToEnumVelocity(animations.at(i)->GetIncomingVelocity());
        ...
        if (query.incomingVelocity_Strict == false) {
          ...
          if (query.incomingVelocity_ForceLinearity) {
            // disallow going from current -> slower/faster -> current; the complete section needs
            // to be linear
            float animIncomingVelocityFloat =
                RangeVelocity(animations.at(i)->GetIncomingVelocity());
            float animOutgoingVelocityFloat =
                RangeVelocity(animations.at(i)->GetOutgoingVelocity());
        ...
        // e_Velocity animOutgoingVelocity =
        // FloatToEnumVelocity(animations.at(i)->GetOutgoingVelocity()); if (animIncomingVelocity ==
        // e_Velocity_Idle && animOutgoingVelocity != e_Velocity_Idle) selectAnim = false; if
        // (animOutgoingVelocity == e_Velocity_Idle && animIncomingVelocity != e_Velocity_Idle)
        // selectAnim = false;
      }
    }
    ...
    // e_Velocity animIncomingVelocity =
    // FloatToEnumVelocity(animations.at(i)->GetIncomingVelocity()); e_Velocity animOutgoingVelocity
    // = FloatToEnumVelocity(animations.at(i)->GetOutgoingVelocity()); if (animIncomingVelocity ==
    // e_Velocity_Sprint || animOutgoingVelocity == e_Velocity_Sprint) selectAnim = false;

    // select by OUTGOING VELOCITY

    if (selectAnim) {
      if (query.byOutgoingVelocity == true) {
        if (FloatToEnumVelocity(animations.at(i)->GetOutgoingVelocity()) != query.outgoingVelocity)
          selectAnim = false;
      }
    }
    ...
    if (selectAnim) {
      if (query.bySide == true) {
        ...
        Vector3 animOutgoingDirection = animations.at(i)->GetOutgoingDirection().GetRotated2D(
            animations.at(i)->GetOutgoingBodyAngle());
        ...
        if (fabs(animTurnAngle) > 0.06f * pi) {  // threshold
          ...
          radian queryIncomingToFenceAngle =
              fencedDirection.GetAngle2D(query.incomingBodyDirection);
          radian fenceToOutgoingAngle = animOutgoingDirection.GetAngle2D(fencedDirection);

          e_Side animIncomingToFenceSide =
              (animIncomingToFenceAngle > 0) ? e_Side_Left : e_Side_Right;
          e_Side queryIncomingToFenceSide =
              (queryIncomingToFenceAngle > 0) ? e_Side_Left : e_Side_Right;
          e_Side fenceToAnimOutgoingSide = (fenceToOutgoingAngle > 0) ? e_Side_Left : e_Side_Right;

          // passes through fence! n000!
          if (animIncomingToFenceSide == animSide && fenceToAnimOutgoingSide == animSide &&
              fabs(animIncomingToFenceAngle + fenceToOutgoingAngle) < pi)
            selectAnim = false;
          if (queryIncomingToFenceSide == animSide && fenceToAnimOutgoingSide == animSide &&
              fabs(queryIncomingToFenceSide + fenceToOutgoingAngle) < pi)
            selectAnim = false;
        }
      }
    }
    ...
    /*
        ...
        if (selectAnim) {
          if (query.byOutgoingAngle == true) {
            if (fabs(animations.at(i)->GetOutgoingAngle() - query.outgoingAngle) > pi * 0.5)
       selectAnim = false;
          }
        }
    */
    ...
    if (selectAnim) {
      if (query.byPickupBall == true) {
        if ((animations.at(i)->GetVariable("outgoing_retain_state") == "" &&
             query.pickupBall == true) ||
            (animations.at(i)->GetVariable("outgoing_retain_state") != "" &&
    ...
    if (selectAnim) {
      if (query.byIncomingBodyDirection == true &&
          !(query.byIncomingVelocity == true && query.incomingVelocity == e_Velocity_Idle)) {
        radian marginRadians =
            0.06f * pi;  // anims can deviate a few degrees from the desired (quantized) directions
        ...
        if (FloatToEnumVelocity(animations.at(i)->GetIncomingVelocity()) != e_Velocity_Idle) {
          ...
          /* this is implicitly happening already because of the section after this one anyway, so
             disable *todo: is it?
                    //if (selectAnim) {
                      if (query.incomingBodyDirection_Strict == true) { // == non-movement, atm
                        // strict
                        //if (fabs(incomingBodyDir.GetAngle2D(Vector3(0, -1, 0))) >
             fabs(query.incomingBodyDirection.GetAngle2D(Vector3(0, -1, 0))) + marginRadians)
             selectAnim = false;
                        // allow 45
                        //if (fabs(incomingBodyDir.GetAngle2D(Vector3(0, -1, 0))) >
             fabs(query.incomingBodyDirection.GetAngle2D(Vector3(0, -1, 0))) + 0.25f * pi +
             marginRadians) selectAnim = false;

                        //} else {
                        // if
             (fabs(query.incomingBodyDirection.GetAngle2D(animations.at(i)->GetIncomingBodyDirection()))
             > marginRadians) selectAnim = false;
                        //}
                      }
                    //}
          */
          if (selectAnim) {
            ...
            if (fabs(FixAngle(animations.at(i)->GetIncomingBodyDirection().GetAngle2D())) >
                fabs(FixAngle(query.incomingBodyDirection.GetAngle2D())) + marginRadians)
              selectAnim = false;
          }

          if (selectAnim) {
            // absolute outgoing body dir is body dir + outgoing dir
            Vector3 outgoingBodyDir = Vector3(0, -1, 0).GetRotated2D(
                animations.at(i)->GetOutgoingBodyAngle() + animations.at(i)->GetOutgoingAngle());

            // disallow > ~135 degrees (not really needed, i guess)
            // if
            // (fabs(animations.at(i)->GetIncomingBodyDirection().GetAngle2D(query.incomingBodyDirection))
            // > 0.75f * pi + marginRadians) selectAnim = false; if
            // (fabs(animations.at(i)->GetIncomingBodyDirection().GetAngle2D(query.incomingBodyDirection))
            // > 0.5f * pi + marginRadians) selectAnim = false;

            // disallow > ~90 degrees larger incoming than current
            // if (fabs(FixAngle(animations.at(i)->GetIncomingBodyDirection().GetAngle2D())) >
            // fabs(FixAngle(query.incomingBodyDirection.GetAngle2D())) + 0.5f * pi + marginRadians)
            // selectAnim = false;
            // disallow > ~90 degrees different incoming than current
            // if (fabs(fabs(FixAngle(animations.at(i)->GetIncomingBodyDirection().GetAngle2D())) -
            // fabs(FixAngle(query.incomingBodyDirection.GetAngle2D()))) > 0.5f * pi +
            // marginRadians) selectAnim = false; if
            // (fabs(animations.at(i)->GetIncomingBodyDirection().GetAngle2D(Vector3(0, -1, 0)) -
            // query.incomingBodyDirection.GetAngle2D(Vector3(0, -1, 0))) > 0.5f * pi +
            // marginRadians) selectAnim = false;
            // this version is not just moar beautiful, but also allows for -135 to 135 deg and vice
            // versa
            if (query.incomingBodyDirection_Strict == true) {
              if (fabs(incomingBodyDir.GetAngle2D(query.incomingBodyDirection)) > marginRadians)
                selectAnim = false;
            } else {
              if (fabs(incomingBodyDir.GetAngle2D(query.incomingBodyDirection)) >
                  0.5f * pi + marginRadians)
                selectAnim = false;
            }

            // disallow > ~135 degrees between query incoming and abs outgoing body dir (kills 180
            // deg anims!)
            // if (fabs(outgoingBodyDir.GetAngle2D(query.incomingBodyDirection)) > 0.75f * pi +
            // marginRadians) selectAnim = false;

            if (query.incomingBodyDirection_ForceLinearity) {
              // if anim incoming body dir == between (including) query incoming and anim outgoing
              // dir, then this anim is legal (or rather: won't look idiotic) how do we check this?
              // 1. if we look at the smallest angles between (anim incoming -> query incoming) and
              // (anim incoming -> anim outgoing), one has to be positive, the other negative.
              // 2. the (absolute) angles added up have to be < pi radians. else, we could be on the
              // 'other side' of the 'virtual half circle' and still have the former condition met
              ...
              if (fabs(shortestAngle1) + fabs(shortestAngle2) > pi + marginRadians)
                selectAnim = false;
            }
          }
        }
          ...
          // allow only same angle (which is moving anims with 0 outgoing body angle. since @ idle,
          // that will become their only angle)
          // if
          // (fabs(animations.at(i)->GetIncomingBodyDirection().GetAngle2D(query.incomingBodyDirection))
          // > marginRadians) selectAnim = false;
          if (query.incomingBodyDirection_Strict == true) {
            if (fabs(Vector3(0, -1, 0).GetAngle2D(query.incomingBodyDirection)) > marginRadians)
              selectAnim = false;
          } else {
            if (fabs(Vector3(0, -1, 0).GetAngle2D(query.incomingBodyDirection)) >
                0.25f * pi + marginRadians)
              selectAnim = false;
          }
        }
      }

      // no backwards body angles (test) if (fabs(animations.at(i)->GetIncomingBodyAngle()) > 0.5 *
      // pi || fabs(animations.at(i)->GetOutgoingBodyAngle()) > 0.5 * pi) selectAnim = false;
    }
    ...
    if (selectAnim) {
      if (query.byIncomingBallDirection == true) {
        Vector3 animBallDirection =
            GetVectorFromString(animations.at(i)->GetVariable("incomingballdirection"));
        if (animBallDirection.GetLength() < 0.1f) {
          Log(e_FatalError, "AnimCollection", "Crudeselection",
              "Anim " + animations.at(i)->GetName() + " missing incoming ball direction");
        }
        if (animBallDirection.GetLength() != 0.0f &&
          ...
          // float ballDirectionSimilarity =
          // adaptedIncomingBallDirection.GetDotProduct(animBallDirection);// * 0.5 + 0.5;
          radian ballDirectionAngle =
              fabs(adaptedIncomingBallDirection.GetAngle2D(animBallDirection));
          // printf("%s\n", animations.at(i)->GetName().c_str());
          // query.incomingBallDirection.Print();
          // animBallDirection.Print();
          // printf("%f\n", ballDirectionDiff);
          radian maxDeviation = fabs(
              atof(animations.at(i)->GetVariable("incomingballdirection_maxdeviation").c_str()) *
              pi);
          if (maxDeviation == 0.0f) {
            maxDeviation = maxIncomingBallDirectionDeviation;  // 0.45f;
            if (animType.compare(defString[e_DefString_Deflect]) == 0)
              maxDeviation = 0.4f * pi;
          }
          if (ballDirectionAngle > maxDeviation)
            selectAnim = false;
        }
      }
    }
    ...
    if (selectAnim) {
      if (query.byOutgoingBallDirection == true) {
        Vector3 animBallDirection =
            GetVectorFromString(animations.at(i)->GetVariable("balldirection"));
        animBallDirection.Normalize(Vector3(0));
        // printf("%s\n", animations.at(i)->GetName().c_str());
        // float ballDirectionSimilarity =
        // query.outgoingBallDirection.Get2D().GetNormalized(animBallDirection).GetDotProduct(animBallDirection);
        radian ballDirectionAngle = fabs(query.outgoingBallDirection.Get2D()
                                             .GetNormalized(animBallDirection)
                                             .GetAngle2D(animBallDirection));
        // query.outgoingBallDirection.Print();
        // animBallDirection.Print();
        // printf("%f\n", ballDirectionDiff);
        radian maxDeviation = fabs(
            atof(animations.at(i)->GetVariable("outgoingballdirection_maxdeviation").c_str()) * pi);
        if (maxDeviation == 0.0) {
    ...
    if (selectAnim) {
      if (query.properties.Get("incoming_special_state")
              .compare(animations.at(i)->GetVariable("incoming_special_state")) != 0)
        selectAnim = false;
      // hax: allow switching of hands (except for deflect anims) (in future, maybe make special
      // case for 'both hands at the same time')
      if ((query.functionType == e_FunctionType_Deflect ||
           ((query.properties.Get("incoming_retain_state").compare("") != 0) !=
            (animations.at(i)->GetVariable("incoming_retain_state").compare("") != 0))) &&
          query.properties.Get("incoming_retain_state")
                  .compare(animations.at(i)->GetVariable("incoming_retain_state")) != 0)
        selectAnim = false;
      if (atof(query.properties.Get("specialvar1").c_str()) !=
          atof(animations.at(i)->GetVariable("specialvar1").c_str()))
        selectAnim = false;
      if (atof(query.properties.Get("specialvar2").c_str()) !=
          atof(animations.at(i)->GetVariable("specialvar2").c_str()))
        selectAnim = false;
    }
    ...
    if (selectAnim) {
      if (query.heedForcedFoot == true) {
        ...
        if (which != 0) {
          ...
          if (which == 1 && query.strongFoot != animFoot)
            selectAnim = false;
          if (which == 2 && query.strongFoot == animFoot)
            selectAnim = false;
...
int AnimCollection::GetQuadrantID(Animation* animation, const Vector3& movement,
                                  radian angle) const {
...
int AddExtraTouches(Animation* animation, boost::intrusive_ptr<Node> playerNode,
  ...
  bool isTouch =
      std::static_pointer_cast<FootballAnimationExtension>(animation->GetExtension("football"))
          ->GetFirstTouch(animBallPos, animTouchFrame);
  // printf("touchframe: %i\n", animTouchFrame);
  if (isTouch) {
    // printf("[touchframe: %i(%i); nodeMap size: %i] ", animTouchFrame, animation->GetFrameCount(),
    // nodeMap.size());
    ...
    */
    ...
    for (int i = animTouchFrame - range_pre; i < animTouchFrame + range_post + 1; i += 1) {
      if (i >= 0 && i < animation->GetFrameCount() - frameOffset - 1) {
        ...
        position.coords[2] = position.coords[2] * 0.4f +
                             animBallPos.coords[2] * 0.6f;  // take anim's height more seriously

        // correction for animation smoothing
        Vector3 origBodyPartPos =
            closestBodyPart->GetDerivedPosition() * bodypartBias +
            nodeMap.find("player")->second->GetDerivedPosition() * (1.0 - bodypartBias);
        ...
        // scale ballposition: this is to correct for the change in animation 'player dud' scale and
        // actual scale (on average - since players may have different heights, of course)
        resultPosition.coords[0] =
            nodeMap.find("player")->second->GetDerivedPosition().coords[0] * 0.12f +
            resultPosition.coords[0] * 0.88f;  // for some reason, anim ball pos is way too far from
                                               // body (todo: investigate)
        resultPosition.coords[1] =
            nodeMap.find("player")->second->GetDerivedPosition().coords[1] * 0.12f +
            resultPosition.coords[1] * 0.88f;

        /*
        // experiment: ball more on the sides (y) to get more incoming anti-outgoingdir-movement
        before touch (aesthetics effect?) Vector3 outVec = Vector3(0, -1,
        0).GetRotated2D(animation->GetOutgoingAngle() + animation->GetOutgoingBodyAngle()); float
        dot = 1.0f - fabs(Vector3(0, -1, 0).GetDotProduct(outVec)); resultPosition += outVec * dot *
        0.30f;
        */

        Quaternion orientation;
        std::static_pointer_cast<FootballAnimationExtension>(animation->GetExtension("football"))
            ->SetKeyFrame(i + frameOffset, orientation, resultPosition, 0);
      }
    }
...
float CalculateAnimDifficulty(Animation* animation, float& absoluteDifficulty) {
  ...
  float bodyDirDifficulty = clamp(
      fabs(animation->GetIncomingBodyDirection().GetAngle2D(animation->GetOutgoingBodyDirection()) /
           pi),
      0.0, 1.0);

  float directionDifficulty =
      clamp(fabs(Vector3(0, -1, 0).GetAngle2D(animation->GetOutgoingDirection()) / pi), 0.0, 1.0);

  float veloChangeDifficulty = clamp(
      fabs(animation->GetIncomingVelocity() - animation->GetOutgoingVelocity()) / sprintVelocity,
      0.0, 1.0);
  float accelDifficulty =
      clamp((animation->GetOutgoingVelocity() - animation->GetIncomingVelocity()) / sprintVelocity,
            0.0, 1.0);
  float veloDifficulty = veloChangeDifficulty * 0.5 +
                         accelDifficulty * 0.5;  // decelerating is easier than accelerating

  float averageVelocity =
      clamp((animation->GetIncomingVelocity() + animation->GetOutgoingVelocity()) /
                (sprintVelocity * 2.0),
            0.0, 1.0);
  float movementDifficulty =
      clamp((animation->GetIncomingMovement() - animation->GetOutgoingMovement()).GetLength() /
                sprintVelocity,
            0.0, 1.0) *
  ...
  result /= bodyDirDifficultyWeight + directionDifficultyWeight + veloDifficultyWeight +
            movementDifficultyWeight;

  float expectedFrameCount = 20 + result * 80;

  // float relativeDifficultyFactor = expectedFrameCount / (float)animation->GetFrameCount(); // < 1
  // == speed up, > 1 == slow down
  float absoluteDifficultyFactor = result;  // towards 0.0 == easy .. towards 1.0 == hard
  // float frameCountBias = 0.9f; // towards 0.0 == absolute difficulty, disregard how many frames
  // the anim takes to do something difficult, towards 1.0 == relative difficulty, if anim takes
  // enough frames to do hard stuff then it's a wrap already float factor = relativeDifficultyFactor
  // * frameCountBias + absoluteDifficultyFactor * (1.0 - frameCountBias);

  // result must be in format 0 == current, 1 == much slower. ignore anims that should be speeded up
  // (it's animators task to keep them at least fast enough == impose slowest possible)
  ...
  //   (int)expectedFrameCount - animation->GetFrameCount(), absoluteDifficulty);

  // printf("%f\n", difficulty);
  // return relativeDifficulty;

  expectedFrameCount =
      clamp(expectedFrameCount * 1.0f, 1, animation->GetEffectiveFrameCount() + 16);
  return expectedFrameCount;
}

void Slowdown(Animation* animation, float veloFactor, float expectedFrameCount,
              bool debug = false) {
  ...
  int targetFrameCount = clamp(round(expectedFrameCount), animation->GetEffectiveFrameCount(),
                               1000000);  // will not come out perfectly correct, because we can
                                          // only add between frames (not before first/after last)
  ...
  if (targetFrameCount > animation->GetFrameCount()) {
    ...
    for (int frame = 1; frame < animation->GetFrameCount(); frame++) {
      ...
      isFrame =
          animation->GetExtension("football")->GetKeyFrame(frame, orientation, position, power);
      if (isFrame) {
        position.coords[0] *= currentShiftFactor;
        position.coords[1] *= currentShiftFactor;
    ...
    if (debug) {
      if (FloatToEnumVelocity(animation->GetIncomingVelocity()) != originalIncomingVelocity)
        printf("incoming: %s: %f to %f\n", animation->GetName().c_str(),
               EnumToFloatVelocity(originalIncomingVelocity),
               RangeVelocity(animation->GetIncomingVelocity()));
      if (FloatToEnumVelocity(animation->GetOutgoingVelocity()) != originalOutgoingVelocity)
        printf("outgoing: %s: %f to %f\n", animation->GetName().c_str(),
               EnumToFloatVelocity(originalOutgoingVelocity),
               RangeVelocity(animation->GetOutgoingVelocity()));
    }
  }
}
...
void SmoothPositions(Animation* animation, bool convertAngledDribbleToWalk) {
  ...
  if (animation->GetAnimType().compare("movement") == 0) {
    bias = 0.5;
    exp = 0.8 +
          pow(clamp(animation->GetOutgoingVelocity(), 0, sprintVelocity) / sprintVelocity, 1.5) *
              0.4 +
          +pow(clamp(animation->GetOutgoingVelocity() - animation->GetIncomingVelocity(), 0,
                     sprintVelocity) /
                   sprintVelocity,
               1.3) *
              0.4;
  ...
  if (convertAngledDribbleToWalk) {
    if (FloatToEnumVelocity(animation->GetIncomingVelocity()) == e_Velocity_Dribble) {
      incoming.Normalize(0);
      incoming *= walkVelocity;
      bias = 1.0;
    }
    if (FloatToEnumVelocity(animation->GetOutgoingVelocity()) == e_Velocity_Dribble &&
        fabs(animation->GetOutgoingAngle()) < 0.5 * pi) {
      outgoing.Normalize(0);
      outgoing *= walkVelocity;
      bias = 1.0;
    }
  }
  ...
  for (int frame = 1; frame < animation->GetFrameCount(); frame++) {
    float frameBias = (float)(frame - 1) /
                      (float)(animation->GetFrameCount() -
                              3);  // first 2 and last 2 frames need to be bias 0 and 1, so our anim
                                   // keeps having the original in- and outgoing movement
    frameBias = pow(clamp(frameBias, 0.0, 1.0), exp);
    // printf("%f, ", bias);
    Vector3 movement = incoming * (1 - frameBias) + outgoing * frameBias;
    // movement.Print();

    bool touchFrame = animation->GetExtension("football")
                          ->GetKeyFrame(frame, touchOrientation, origTouchPosition, power);
    ...
    if (touchFrame)
      animation->GetExtension("football")
          ->SetKeyFrame(frame, touchOrientation, resultingTouchPosition, power);
  ...
  //  printf("incoming: %s: %f to %f\n", animation->GetName().c_str(),
  //  EnumToFloatVelocity(originalIncomingVelocity),
  //  RangeVelocity(animation->GetIncomingVelocity())); if
  //  (FloatToEnumVelocity(animation->GetOutgoingVelocity()) != originalOutgoingVelocity)
  //  printf("outgoing: %s: %f to %f\n", animation->GetName().c_str(),
  //  EnumToFloatVelocity(originalOutgoingVelocity),
  //  RangeVelocity(animation->GetOutgoingVelocity()));
}
...
void AnimCollection::_PrepareAnim(
    ...
    const std::list<boost::intrusive_ptr<Object>>& bodyParts,
    const std::map<const std::string, boost::intrusive_ptr<Node>>& nodeMap,
    bool convertAngledDribbleToWalk) {
  // animation->Hax();
  ...
  bool isTouch =
      std::static_pointer_cast<FootballAnimationExtension>(animation->GetExtension("football"))
          ->GetFirstTouch(animBallPos, animTouchFrame);
  if (isTouch && (animation->GetAnimType() == "movement" || animation->GetAnimType() == "trip" ||
                  animation->GetAnimType() == "special"))
...
inline bool AnimCollection::_CheckFunctionType(const std::string& functionType,
                                               e_FunctionType queryFunctionType) const {
