...
unsigned int CalculateTimeNeededToChangeMovement_ms(const Vector3& currentMovement,
                                                    const Vector3& desiredMovement) {
  // this function is quick 'n dirty, just how i like it
  // just don't expect exact results

  int time_ms = NormalizedClamp((desiredMovement - currentMovement).GetLength(), idleVelocity,
                                sprintVelocity * 2.0f) *
                1000;

  float currentVeloFactor =
      1.0f - NormalizedClamp(currentMovement.GetLength(), idleVelocity, sprintVelocity);
  time_ms *= 1.0f + currentVeloFactor;

  return time_ms;
}

float CalculateBiasForFastCornering(const Vector3& currentMovement, const Vector3& desiredMovement,
  ...
  float currentMovementBias =
      sin(fabs(angle) - 0.5f * pi) * 0.5f +
      0.5f;  // this one is better for anim selection, else 135 anim is never used (since 135 @ idle
             // is worse than 90 @ dribble then - with this one, 90 @ idle will be preferred (which
             // will allow 135 @ idle as well))
  // float currentMovementBias = sin(2.0f * fabs(angle) - 0.5f * pi) * 0.5f + 0.5f; // this one
  // seems more correct puristically though.. hmm. maybe make a 135 anim that ends in dribble? or
  // would that effectively disable the 180 @ idle?

  // effect is less pronounced at low velocities
  float velocityBias =
      pow(clamp(currentMovement.GetLength() / (sprintVelocity - 0.5f), 0.0f, 1.0f), veloPow);
...
Vector3 CalculateMovementAtFrame(const std::vector<Vector3>& positions, unsigned int frameNum,
                                 unsigned int smoothFrames) {
  /*
    // simple version for debugging purposes
    unsigned int adaptedFrameNum = std::min(std::max(frameNum, (unsigned int)1), (unsigned
    int)positions.size() - 1); return (positions.at(adaptedFrameNum) - positions.at(adaptedFrameNum
    - 1)).Get2D() * 100.0f;
  */

  assert(frameNum < positions.size());

  // special case: want the exit movement to be unsmoothed, for we don't want the wrong quantized
  // velocity and such
  if (frameNum == positions.size() - 1) {
    return (positions.at(positions.size() - 1) - positions.at(positions.size() - 2)).Get2D() *
           100.0f;
  }
  // special case: if we want the movement at frame 0, we can't get -1 to 0, we need to get 0 to 1
  // instead
  if (frameNum == 0) {
  ...
  for (int frame = (signed int)(frameNum - smoothFrames);
    ...
    if (frame > 0 &&
        frame < (signed int)positions.size()) {  // was: frame > 1 (i think that was a bug)
...
Vector3 GetFrontOfFootOffsetRel(float velocity, radian bodyAngleRel, float height) {
  ...
  float distance = 0.34f + velocity * defaultTouchOffset_ms * 0.001f *
                               fullDistanceFactor;  // must be > 0 (for upcoming dotproduct)

  Vector3 ffo = Vector3(0, -distance * 0.8f, 0);

  radian bodyAngle = bodyAngleRel;
  if (velocity < idleDribbleSwitch)
    bodyAngle = 0;
...
void GetDifficultyFactors(Match* match, Player* player, const Vector3& positionOffset,
                          float& distanceFactor, float& heightFactor, float& ballMovementFactor) {
  ...
  float positionOffsetPenalty =
      NormalizedClamp(positionOffset.GetLength(), 0.0f, 0.1f) * 2.0f;  // was: 7.0f
  // fast balls are harder
  float ballBodyVeloPenalty =
      std::pow(
          NormalizedClamp((player->GetMovement() - ball->GetMovement()).GetLength(), 10.0f, 50.0f),
          1.5f) *
      5.0f;
  // balls farther away from body are harder
  float fartherAwayPenalty =
      std::pow(NormalizedClamp(((player->GetPosition() + player->GetDirectionVec() * 0.2f) -
  ...
  if (Verbose())
    printf("distanceFactor: positionoffset: %f, ballbodyvelodiff: %f, fartheraway: %f, ",
           positionOffsetPenalty, ballBodyVeloPenalty, fartherAwayPenalty);
  ...
  if (match->GetLastTouchTeamID() != player->GetTeam()->GetID()) {
    Player* lastTouchPlayer =
        match->GetTeam(abs(player->GetTeam()->GetID() - 1))->GetLastTouchPlayer();
    if (lastTouchPlayer) {
      float lastTouchBiasPenalty =
          std::pow(
              lastTouchPlayer->GetLastTouchBias(1000 - player->GetStat("physical_reaction") * 500),
              0.6f) *
          5.0f;
      if (Verbose())
        printf("lastTouchBiasPenalty: %f, ", lastTouchBiasPenalty);
  ...
  if (Verbose())
    printf(
        ...
        "ballmovementfactor: %f\n",
        skillPenaltyMultiplier, distanceFactor, heightFactor, ballMovementFactor);
}

Vector3 GetBallControlVector(Ball* ball, Player* player, const Vector3& nextStartPos,
                             radian nextStartAngle, radian nextBodyAngle,
                             const Vector3& outgoingMovement, const Anim* currentAnim, int frameNum,
                             const SpatialState& spatialState, const Vector3& positionOffset,
                             radian& xRot, radian& yRot, float ffoOffset) {
  // part of the resulting direction is physics, the other part is anim/controller. so
  // originatingBias is only applied on the (1.0 - physicsBias) part of the result
  ...
  float dotFactor1 =
      spatialState.directionVec.GetDotProduct(currentAnim->originatingCommand.desiredDirection) *
          0.5f +
      0.5f;  // inv deviation from the current direction
  float dotFactor2 = Vector3(0, -1, 0)
                             .GetRotated2D(nextStartAngle)
                             .GetDotProduct(currentAnim->originatingCommand.desiredDirection) *
                         0.5f +
                     0.5f;  // inv deviation from the direction we wanted
  ...
  Vector3 desiredMovement =
      ...
      (currentAnim->originatingCommand.desiredVelocityFloat * originatingBias +
       player->GetController()->GetFloatVelocity() * (1.0f - originatingBias));

  // range velocity, pes6 style
  // desiredMovement = desiredMovement.GetNormalized(0) *
  // RangeVelocity(desiredMovement.GetLength());
  ...
  if (desiredMovement.GetLength() > walkSprintSwitch)
    desiredMovement =
        desiredMovement.GetNormalized(0) *
        StretchSprintTo(desiredMovement.GetLength(), sprintVelocity, player->GetMaxVelocity());
  ...
  float velocity = clamp(desiredMovement.GetLength(), outgoingMovement.GetLength(),
                         outgoingMovement.GetLength() + maximumOverdrive_mps);
  desiredMovement =
      desiredMovement.GetNormalized(Vector3(0, -1, 0).GetRotated2D(nextStartAngle)) * velocity;
  ...
  Vector3 FFOsrc = GetFrontOfFootOffsetRel(physicsVelocity, nextBodyAngle - spatialState.angle,
                                           ball->Predict(0).coords[2]);
  float annoyanceVeloFactor =
      curve(NormalizedClamp(currentAnim->anim->GetOutgoingVelocity(), idleVelocity, sprintVelocity),
            0.7f);  // do not apply effect to low velo's; makes it too chaotic
  float opponentAnnoyanceFactor =
      (1.0f - NormalizedClamp(player->GetClosestOpponentDistance(), 0.5f, 1.5f)) *
      (1.0f -
       (player->GetStat("mental_calmness") * 0.5f + player->GetStat("physical_balance") * 0.3f)) *
      annoyanceVeloFactor;
  Vector3 FFO = Vector3(0, -1, 0).GetRotated2D(nextBodyAngle) *
                (FFOsrc.GetLength() + ffoOffset +
                 opponentAnnoyanceFactor *
                     3.0f);  // positionOffset is already in ffoOffset (though only for trap atm)
  float heightFFOOffset = NormalizedClamp(ball->Predict(0).coords[2], 0.5f, 1.0f) *
                          0.5f;  // bounce high balls off body - else they keep colliding inside
                                 // body and stuff like that
  FFO += FFOsrc * heightFFOOffset * 0.5f + player->GetBodyDirectionVec() * heightFFOOffset * 0.5f;

  Vector3 desiredPlannedBallPos = nextStartPos;
  Vector3 physicsPlannedBallPos = nextStartPos;

  // we actually want the ball to be further away than our next position - after all, we want to hit
  // it again a few steps later
  float desiredDelayTime =
      std::pow(NormalizedClamp(desiredVelocity, 0.0f, sprintVelocity), 2.0f) * 0.60f +
  ...
  float physicsDelayTime =
      std::pow(NormalizedClamp(physicsVelocity, 0.0f, sprintVelocity), 2.0f) * 0.60f +
      0.25f;  // how many seconds later do we want to be able to hit it again?
              // (remember: this is minus ffo time and current anim time)
  ...
  Vector3 plannedBallPos =
      physicsPlannedBallPos * physicsBias + desiredPlannedBallPos * (1.0f - physicsBias);
  ...
  timeToGo += defaultTouchOffset_ms *
              0.001f;  // 0.08f; // time into next anim where we want to hit the ball

  float divisor =
      timeToGo * (0.38f + 0.02f * player->GetStat("technical_dribble"));  // higher == closer
  divisor *= 1.1f;

  // to get the ball to the planned position in timeToGo seconds, we need to do some pow() magic,
  // since the ball also slows down faster at higher ball velos
  ...
  float powerMultiplier =
      1.2f - (player->GetStat("technical_ballcontrol") * 0.03f);  // 1.24 .. * 0.1
  float veloBias =
      NormalizedClamp(velocity, walkVelocity,
                      sprintVelocity - 0.8f);  // this multiplier only applies to high velocities
  ...
  */
  ...
  float veloFactor = std::pow(NormalizedClamp(touchVec.GetLength(), 0.0f, 10.0f),
                              0.7f);  // extra around walk velocity
  xRot =
      (FFO.GetNormalized(0) * 0.6f + touchVec.GetNormalized(0) * 0.4f).GetNormalized(0).coords[1] *
      ((2 * pi * velo) - (backspinFactor * veloFactor));
  yRot =
      (FFO.GetNormalized(0) * 0.6f + touchVec.GetNormalized(0) * 0.4f).GetNormalized(0).coords[0] *
      ((2 * pi * velo) - (backspinFactor * veloFactor));
...
Vector3 GetTrapVector(Match* match, Player* player, const Vector3& nextStartPos,
  ...
  GetDifficultyFactors(match, player, positionOffset, distanceFactor, heightFactor,
                       ballMovementFactor);
...
Vector3 GetShotVector(Match* match, Player* player, const Vector3& nextStartPos,
  ...
  Vector3 touchMovement =
      CalculateMovementAtFrame(origPositionCache, currentAnim->frameNum)
          .GetRotated2D(
              spatialState
                  .angle);  // spatialState.movement isn't reliable because of smuggles and such
  ...
  if (FloatToEnumVelocity(touchVelocity) == e_Velocity_Idle)
    touchDirection = spatialState.directionVec;
  // SetBlueDebugPilon(player->GetPosition() + touchDirection * 8.0f);

  float expressionModifierFactor = NormalizedClamp(
      currentAnim->originatingCommand.desiredVelocityFloat, idleVelocity, sprintVelocity);

  // previous: float directionFactor =
  // touchDirection.GetDotProduct(currentAnim->originatingCommand.touchInfo.desiredDirection) * 0.5f
  ...
  // calculate power. take desired power as maximum, and subtract based on player/ball movement and
  // such
  ...
  if (Verbose())
    printf("(power) playerDirDesiredDirPowerFactor: %f\n", playerDirDesiredDirPowerFactor);

  float playerVelocityPowerFactor = std::pow(NormalizedClamp(touchVelocity, 0.0f, sprintVelocity),
                                             0.3f);  // >= walk velo is optimum
  if (Verbose())
    printf("(power) playerVelocityPowerFactor: %f\n", playerVelocityPowerFactor);

  float positionOffsetPowerFactor = 1.0f - NormalizedClamp(positionOffset.GetLength(), 0.0f, 0.1f);
  if (Verbose())
    printf("(power) positionOffsetPowerFactor: %f\n", positionOffsetPowerFactor);

  float powerFactor = 1.0f * (0.7f + playerDirDesiredDirPowerFactor * 0.3f) *
                      (0.7f + playerVelocityPowerFactor * 0.3f) *
                      (0.4f + positionOffsetPowerFactor * 0.6f);
  powerFactor = clamp(powerFactor, 0.0f, 1.0f);
  if (Verbose())
    printf("(power) RESULTING powerFactor: %f\n", powerFactor);

  float adaptedDesiredPower =
      45.0f *
      (0.7f + std::pow(currentAnim->originatingCommand.touchInfo.desiredPower, 0.5f) * 0.3f);
  if (Verbose())
    printf("(power) adaptedDesiredPower: %f\n", adaptedDesiredPower);

  float animMaxPowerFactor = atof(currentAnim->anim->GetVariable("touch_maxpowerfactor").c_str());
  if (animMaxPowerFactor == 0.0f)
    animMaxPowerFactor = 1.0f;
  if (Verbose())
    printf("(power) animMaxPowerFactor: %f\n", animMaxPowerFactor);

  float power = clamp(
      powerFactor * adaptedDesiredPower, 0.0f,
      (32.0f + player->GetStat("physical_shotpower") * 13.0f) * (0.2f + animMaxPowerFactor * 0.8f));

  // add this after previous stat-clamp, because using the current ball movement is like an added
  // (power) bonus that everybody profits from, even sucky players
  ...
  if (Verbose())
    printf("(power) playerMovBallMovPowerFactor: %f\n", playerMovBallMovPowerFactor);
  power *= 1.0f + playerMovBallMovPowerFactor * 0.2f;

  if (Verbose())
    printf("(power) RESULTING power: %f (stat: %f)\n", power,
           player->GetStat("physical_shotpower"));

  // calculate difficulty, based on factors like desired power, player/ball movement, skill,
  // positionoffset etcetera

  float playerDirDesiredDirEasinessFactor =
      std::pow(directionFactor, 0.8f);  // shooting sideways is still pretty easy to aim
  if (Verbose())
    printf("(ease) playerDirDesiredDirEasinessFactor: %f\n", playerDirDesiredDirEasinessFactor);

  float playerVelocityEasinessFactor =
      1.0f - NormalizedClamp(fabs(touchVelocity - dribbleVelocity), 0.0f,
                             4.0f);  // dribble velo is optimum
  if (Verbose())
    printf("(ease) playerVelocityEasinessFactor: %f\n", playerVelocityEasinessFactor);

  float positionOffsetEasinessFactor =
      1.0f - NormalizedClamp(positionOffset.GetLength(), 0.0f, 0.1f);
  if (Verbose())
    printf("(ease) positionOffsetEasinessFactor: %f\n", positionOffsetEasinessFactor);
  ...
  if (Verbose())
    printf("(ease) powerEasinessFactor: %f\n", powerEasinessFactor);

  float easinessFactor =
      1.0f * (0.4f + playerDirDesiredDirEasinessFactor * 0.6f) *
      (0.7f + playerVelocityEasinessFactor * 0.3f) * (0.3f + positionOffsetEasinessFactor * 0.7f) *
      (0.6f + playerMovBallMovEasinessFactor * 0.4f) * (0.6f + powerEasinessFactor * 0.4f);
  float difficultyFactor = clamp(1.0f - easinessFactor, 0.0f, 1.0f);

  if (Verbose())
    printf("(ease) RESULTING difficultyFactor: %f\n", difficultyFactor);
  ...
  // random dir
  worstCaseDirection = worstCaseDirection + (Vector3(random(-1, 1), random(-1, 1), random(-1, 1)) *
  ...
  Vector3 worstCaseShot =
      (worstCaseDirection + Vector3(0, 0, worstCaseHeight)).GetNormalized() * worstCasePower;
  if (Verbose()) {
    worstCaseShot.Print();
    printf("^ shot: worstcase\n");
  }
