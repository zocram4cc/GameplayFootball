...
// todo: do we need "match" stuff into MentalImage? especially GetPlayer, at the moment of writing
// that's the only method used here, so we could get rid of the match pointers.
...
Vector3 AI_GetAdaptedInitialPos(Match* match, const Vector3& initialPosition, Vector3 focusPoint,
  ...
  if (scaledFocusPoint.coords[0] > pitchHalfW)
    scaledFocusPoint.coords[0] = pitchHalfW;
  if (scaledFocusPoint.coords[0] < -pitchHalfW)
    scaledFocusPoint.coords[0] = -pitchHalfW;
  if (scaledFocusPoint.coords[1] > pitchHalfH)
    scaledFocusPoint.coords[1] = pitchHalfH;
  if (scaledFocusPoint.coords[1] < -pitchHalfH)
    scaledFocusPoint.coords[1] = -pitchHalfH;
  ...
  if (ballDistance < ballMagnetDistance)
    ballMagnetOffset =
        (ballMagnetPos - targetPos) *
        pow(((ballMagnetDistance - ballDistance) / ballMagnetDistance), ballMagnetDistancePow);
...
Vector3 AI_GetAdaptedFormationPosition(Match* match, Player* player, float backXBound,
                                       ...
                                       float xFocus, float xFocusStrength, float yFocus,
                                       float yFocusStrength, const Vector3& microFocus,
                                       float microFocusStrength, float midfieldFocus,
                                       float midfieldFocusStrength,
  ...
  if (midfieldFocusStrength > 0.0f) {
    ...
    float stretchBias = clamp(1.0f - fabs(position.coords[0] * 1.2f), 0.0f,
                              1.0f);  // overstretch a bit so defenders/attackers are left alone
                                      // (they usually aren't fully x = 0 or 1)
    stretchBias = curve(stretchBias, 1.0f);

    stretchBias *= midfieldFocusStrength;
    // if (player->GetTeam()->GetID() == 0) printf("==> %f\n", position.coords[0]);
    position.coords[0] =
        position.coords[0] * (1.0f - stretchBias) + midfieldPositionFactor * stretchBias;
    // if (player->GetTeam()->GetID() == 0) printf("--> %f %f %f\n", position.coords[0],
    // midfieldPositionFactor, stretchBias);
  }
  ...
  if (xFocusStrength > 0.0f) {
    float bias = 1.0f - clamp(fabs(xFocus - position.coords[0]) / fabs(backXBound - frontXBound),
                              0.0f, 1.0f);
  ...
  if (yFocusStrength > 0.0f) {
    float distance =
        clamp(fabs(yFocus - position.coords[1]) / fabs(highYBound - lowYBound), 0.0f, 1.0f);
  ...
  if (microFocusStrength > 0.0f) {
    float homogeneousYInfluenceBias = 0.2f;  // 1.0f == act as if everybody is on the same Y
                                             // position (so, as if we're in 1D, with only X)
    float homogeneousYPositionBias = 0.4f;  // 1.0f == don't influence player's resulting Y position
                                            // (so, as if we're in 1D, with only X)

    // this way, players are more strictly keeping to their positions
    float distStatic =
        ((microFocus - purePosition) * Vector3(1, 1.0f - homogeneousYInfluenceBias, 0))
            .GetLength() /
        50.0f;  // 26.0f;//32.0f;
    // this way, microfocus gets more direct attention from players around the action - they kinda
    // 'forget' their base positions a bit though
    ...
    if (dist < 1.0f) {
      /*
      // wolfram alpha: (sin((x + 0.5) * pi) * 0.5 + 0.5) * 0.7 + (sin((x - 0.25) * 2.0 * pi) * 0.5
      + 0.5) * 0.3 | from x = 0 to 1 float microFocusBias1 = sin((dist + 0.5f) * pi) * 0.5f + 0.5f;
      // -\_ float microFocusBias2 = sin((dist - 0.25f) * 2.0f * pi) * 0.5f + 0.5f; // _/-\_
      // mates in team in possession want to keep some distance for easier pass-to-ability, while
      defenders just want to jump into the action more
      // todo ^ this is bullshit, right? as in, it's not about the target position, but about the
      target players to reposition (towards the focus).. float sineBias = 0.15f;// +
      clamp(player->GetTeam()->GetFadingTeamPossessionAmount() - 0.5f, 0.0f, 1.0f) * 0.2f; float
      microFocusBias = microFocusBias1 * (1.0f - sineBias) + microFocusBias2 * sineBias;
      */

      float microFocusBias = 1.0f;

      // less serious when far away
      // we need this, because we use bias to vary between pos and microfocuspos. so for far away
      // players, microfocuspos has way more influence since it's farther away.
      ...
      microFocusBias +=
          (1.0f - NormalizedClamp(fabs(dist - peakLocation), 0.0f, peakWidth)) * peakHeight;
      ...
      Vector3 microFocusPosition = microFocus * Vector3(1, 1.0f - homogeneousYPositionBias, 1) +
                                   position * Vector3(0, homogeneousYPositionBias, 0);
...
float AI_CalculatePassingOdds(Match* match, const Vector3& origin, const Vector3& target,
  ...
  Vector3 step = (target - origin) * (1.0 / (float)checkCount) *
                 0.96;  // * 0.96: we don't mind players standing behind target that much anyway
...
float AI_CalculatePassingOdds(Match* match, PlayerImage thisPlayerImage,
                              PlayerImage targetPlayerImage,
                              const std::vector<PlayerImage>& opponentPlayerImages) {
  // player position predictions
  thisPlayerImage.position =
      thisPlayerImage.position + thisPlayerImage.directionVec * thisPlayerImage.velocity * 0.1;
  targetPlayerImage.position = targetPlayerImage.position + targetPlayerImage.directionVec *
                                                                targetPlayerImage.velocity *
                                                                0.5;  // situation in half a second

  float currentOdds = AI_CalculatePassingOdds(match, thisPlayerImage.position,
                                              targetPlayerImage.position, opponentPlayerImages);

  return currentOdds;
}

void AI_GetPassRatings(Match* match, int thisPlayerID, const MentalImage* mentalImage,
                       float opportunism, PassRatings& passRatings) {
  ...
  mentalImage->GetTeamPlayerImages(match->GetPlayer(thisPlayerID)->GetTeamID(), thisPlayerID,
                                   playerImages);
  ...
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    opponentPlayerImages.at(i).position =
        opponentPlayerImages.at(i).position + opponentPlayerImages.at(i).directionVec *
                                                  opponentPlayerImages.at(i).velocity *
                                                  0.3;  // situation in half a second
  }
  ...
  for (int i = 0; i < (signed int)playerImages.size(); i++) {
    ...
    float bodyDirPenalty = thisPlayerImage.directionVec.GetDotProduct(
        (targetPlayerImage.position - thisPlayerImage.position).GetNormalized(Vector3(side, 0, 0)));
    ...
    float goalDistance =
        clamp((Vector3(pitchHalfW * -side, 0, 0) - thisPlayerImage.position).GetLength(), 8.0,
              100.0) *
        0.01;  // ~= 0.18 .. 1.0
    float mateGoalDistance =
        clamp((Vector3(pitchHalfW * -side, 0, 0) - playerImages.at(i).position).GetLength(), 8.0,
              100.0) *
        0.01;  // ~= 0.18 .. 1.0
...
float AI_GetSituationRating(Match* match, int thisPlayerID, const MentalImage* mentalImage) {
  ...
  std::vector<PlayerImage> opponentPlayerImages;
  mentalImage->GetTeamPlayerImages(abs(match->GetPlayer(thisPlayerID)->GetTeamID() - 1), -1,
                                   opponentPlayerImages);

  // player position predictions
  thisPlayerImage.position = thisPlayerImage.position + thisPlayerImage.directionVec *
                                                            thisPlayerImage.velocity *
                                                            0.5;  // situation in half a second
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    opponentPlayerImages.at(i).position =
        opponentPlayerImages.at(i).position + opponentPlayerImages.at(i).directionVec *
                                                  ...
                                                  0.5;  // situation in half a second
    float situation =
        clamp((opponentPlayerImages.at(i).position - thisPlayerImage.position).GetLength(), 0,
              safeDistance) /
        safeDistance;
  ...
  if (fabs(position.coords[0] > 50))
    penalty += (fabs(position.coords[0]) - 50);
  if (fabs(position.coords[1] > 32))
    penalty += (fabs(position.coords[1]) - 32);
...
float AI_CalculateFreeSpace(Match* match, const MentalImage* mentalImage, int teamID,
                            const Vector3& focusPos, float safeDistance, float futureTime_sec,
                            bool ignoreKeeper) {
  ...
  // player position predictions
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    if (ignoreKeeper == false ||
        opponentPlayerImages.at(i).dynamicFormationEntry.role != e_PlayerRole_GK) {
      // resulting opp position
      opponentPlayerImages.at(i).position = opponentPlayerImages.at(i).position +
                                            opponentPlayerImages.at(i).movement * 0.2f;  // slowness
      Vector3 toFocusMovement = (focusPos - opponentPlayerImages.at(i).position).GetNormalized(0) *
                                sprintVelocity * clamp(futureTime_sec - 0.2f, 0.0f, 1000.0f);
      if (toFocusMovement.GetLength() >
          (focusPos - opponentPlayerImages.at(i).position).GetLength())
        toFocusMovement = focusPos - opponentPlayerImages.at(i).position;
      opponentPlayerImages.at(i).position += toFocusMovement;

      float situation = 1.0f - clamp((opponentPlayerImages.at(i).position - focusPos).GetLength(),
                                     0, safeDistance) /
                                   safeDistance;
...
float AI_GetOffsideLine(Match* match, const MentalImage* mentalImage, int teamID,
  ...
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    opponentPlayerImages.at(i).position.coords[0] +=
        opponentPlayerImages.at(i).movement.coords[0] * futureSim_ms * 0.001f;

    // for offside
    if (opponentPlayerImages.at(i).position.coords[0] * side >
        opponentPlayerImages.at(dudDeepestOpponent).position.coords[0] * side) {
      dudDeepestOpponent = i;
    }
  }

  // offside: we are actually looking for the one-but-deepest opponent (association football rule!
  // EAT THAT, PES6!! :P)
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    if (opponentPlayerImages.at(i).position.coords[0] * side >
  ...
  if (offsideLine * side < 0)
    offsideLine = 0.01 * -side;
...
void AI_GetBestDribbleMovement(Match* match, int thisPlayerID, const MentalImage* mentalImage,
                               Vector3& desiredDirection, float& desiredVelocity,
  ...
  float nearBackline =
      NormalizedClamp(fabs(player->GetPosition().coords[0]) / pitchHalfW, 0.0f, 1.0f);
  float centerModifierInv =
      1.0f - std::pow(nearBackline,
  ...
  Vector3 forceFieldMovement =
      AI_GetForceFieldMovement(forceField, myPos + myMov * future_sec, 1.0f);
...
Vector3 AI_GetForceFieldMovement(const std::vector<ForceSpot>& forceField,
                                 const Vector3& currentPos, float attractorDampingDistance) {
  // attractorDampingDistance: from this distance to attractor, dampen influence so we won't
  // overshoot target
  ...
  for (unsigned int i = 0; i < forceField.size(); i++) {
    ...
    if (forceSpot.decayType == e_DecayType_Constant) {
      ...
      if (forceSpot.exp != 1.0f)
        intensity = std::pow(intensity, forceSpot.exp);
    }
    if (intensity > 0.0f) {
      ...
      if (forceSpot.magnetType == e_MagnetType_Repel) {
        ...
        if (distance < attractorDampingDistance)
          relativeOrigin *= distance / attractorDampingDistance;
      }
...
TimeNeeded AI_GetTimeNeededForDistance_ms(const Vector3& playerPos, const Vector3& playerMovement,
                                          const Vector3& targetPos, float maxVelocity, bool precise,
                                          int maxTime_ms, bool debug) {
  TimeNeeded result;
  ...
  unsigned int defaultOptimizedTime_ms =
      int(std::round((targetPos - (playerPos + playerMovement * 0.2f)).GetLength() /
  ...
  float radius_usual = 0.28f;  // starting distance from our base position where we can reach balls
                               // (effectively: leg extension length)
  float radius_optimistic = 0.9f;
  float resultingRadius_usual =
      radius_usual;  // initial value > 0 because we don't want division by zero
  bool foundOptimisticTime = false;

  float adaptedMaxVelocity =
      maxVelocity * 0.94f;  // don't use full maxvelocity, since the last part of that velo is very
                            // hard to attain (due to exponential air resistance)

  while (true) {  // =]

    // too unstable! timeStep_ms = clamp(int(round(previousDistance * 30)) - 20, 10, 40); //
    // variable timestep may not be 100% correct, so don't overdo it round to 10s
    ...
    if (bias >= 1.0f) {
      ...
      float remainingDistance_usual =
          clamp((targetPos - currentPos).GetLength() - radius_usual, 0.0f, 100000.0f);
      ...
      if (!foundOptimisticTime) {
        float remainingDistance_optimistic =
            clamp((targetPos - currentPos).GetLength() - radius_optimistic, 0.0f, 100000.0f);
        result.optimistic_ms =
            currentTime_ms + (remainingDistance_optimistic / adaptedMaxVelocity) * 1000;
      }

      break;

    } else {
      // manual copy: this function is called a lot, so if we use the Vector3::operator*, it's
      // creating a lot of temp vars. currentMovement = playerMovement * (1.0f - bias);
      ...
      // if (currentTime_ms > 1000 && currentTime_ms % 100 == 0) printf("currentTime_ms: %i,
      // targetDistance: %f, currentMovementLength: %f, bias: %f, radius: %f, changeTime_ms: %i\n",
      // currentTime_ms, targetDistance, currentMovement.GetLength(), bias, radius, changeTime_ms);
      // if (debug) if (maxTime_ms != -1 && currentTime_ms == (unsigned int)maxTime_ms)
      // printf("CANNOT GO FURTHER: %i\n", currentTime_ms);
      if ((targetDistance < radius_optimistic ||
           (maxTime_ms != -1 && currentTime_ms > (unsigned int)maxTime_ms)) &&
          !foundOptimisticTime) {
        result.optimistic_ms = currentTime_ms;
        foundOptimisticTime = true;
      }
      if (targetDistance < radius_usual ||
          (maxTime_ms != -1 && currentTime_ms > (unsigned int)maxTime_ms)) {
  ...
  if (result.usual_ms == 0) {
    result.usual_ms = int(std::round(
        clamp((targetPos - playerPos).GetLength() / resultingRadius_usual, 0.0f, 1.0f) * 10));
    result.optimistic_ms = result.usual_ms;
  }
  ...
  /* too simple version
  return int(round((targetPos - (playerPos + playerMovement * 0.02)).GetLength() / (sprintVelocity *
  0.9) * 1000));
  */
}

unsigned int AI_GetToBallMovement(Match* match, const MentalImage* mentalImage, Player* player,
                                  const Vector3& desiredDirection, float desiredVelocityFloat,
                                  Vector3& bestDirection, float& bestVelocityFloat,
                                  Vector3& bestLookAt, float haste) {
  ...
  // precalc shortest distance to ball 'line' for perpendicularity rating (so we have a minimum
  // value there)

  Line ballLine(mentalImage->GetBallPrediction(0).Get2D(),
                mentalImage->GetBallPrediction(1000).Get2D());
  float u = 0;
  float playerBallLineShortestDistance = ballLine.GetDistanceToPoint(playerPos, u);
  Vector3 playerBallShortestTargetPos =
      ballLine.GetVertex(0) + (ballLine.GetVertex(1) - ballLine.GetVertex(0)) * u;

  // quantize input 8- or 16-way in relation to 'ball direction space'

  Vector3 ballSpaceDirection =
      (mentalImage->GetBallPrediction(0).Get2D() - mentalImage->GetBallPrediction(1000).Get2D())
          .GetNormalized(adaptedDesiredDirection);
  radian toBallSpaceAngle = ballSpaceDirection.GetAngle2D();
  Vector3 adaptedDesiredDirectionBallSpace =
      adaptedDesiredDirection.GetRotated2D(-toBallSpaceAngle);
  ...
  float bias = 1.0f;
  adaptedDesiredDirectionBallSpace = (adaptedDesiredDirectionBallSpace * (1.0 - bias) +
                                      (Vector3(1, 0, 0).GetRotated2D(angle) * bias))
  ...
  // printf("ball angle: %f, result angle: %f\n", ballSpaceDirection.GetAngle2D(),
  // adaptedDesiredDirection.GetAngle2D());
  ...
  /*
    ...
    float desiredVsPerpendicularDot =
    adaptedDesiredDirection.GetDotProduct((playerBallShortestTargetPos -
    playerPos).GetNormalized(0));
    //if (desiredVsPerpendicularDot < 0.3f) movementWeight = 1.0f; else perpendicularWeight = 1.0f;
    float desiredVsPerpendicularBias = pow(NormalizedClamp(desiredVsPerpendicularDot, 0.0f, 1.0f),
    0.5f);
    // if (player->GetDebug()) {
    //   SetGreenDebugPilon(playerPos + (playerBallShortestTargetPos - playerPos).GetNormalized(0)
    * 5.0f);
  ...
  */
  ...
  unsigned int startTime_ms =
      clamp(player->GetTimeNeededToGetToBall_ms(), 40, ballPredictionSize_ms - 10);
  // we can't start optimized at a later moment, because that would mean we'd aim outside the pitch
  // to start with
  ...
  float ffoLength =
      GetFrontOfFootOffsetRel(desiredVelocityFloat * 0.2f + player->GetFloatVelocity() * 0.8f, 0, 0)
          .GetLength();

  Vector3 ballMovementRough =
      (mentalImage->GetBallPrediction(player->GetTimeNeededToGetToBall_ms() + 10) -
       mentalImage->GetBallPrediction(player->GetTimeNeededToGetToBall_ms()))
          .Get2D() *
      100.0f;
  ...
  for (unsigned int time_ms = startTime_ms; time_ms < ballPredictionSize_ms;
    ...
    if (!forced)
      if (targetPos.coords[2] >= 1.0f)
        continue;  // unattainable
    targetPos = targetPos.Get2D();

    TimeNeeded timeNeeded = AI_GetTimeNeededForDistance_ms(
        playerPos, player->GetMovement(), targetPos, player->GetMaxVelocity(), true, time_ms);
    unsigned int timeNeeded_ms = timeNeeded.usual_ms;

    timeStep =
        std::min(std::max(int(std::round((signed int)(timeNeeded_ms - time_ms) * 0.05f)) - 5, 1),
    ...
    if (timeNeeded_ms <= time_ms) {
      ...
      /* to do this, we need to change the look at stuff below too, else we will walk backwards with
      a strange body dir sometimes (i think) if (dot >= 0.6f) forced = true;
      //Vector3 ballMovement = mentalImage->GetBallPrediction(time_ms + 10) -
      mentalImage->GetBallPrediction(time_ms); if (ballMovementRough.GetLength() < sprintVelocity
      * 2.0f) { if (dot >= 0.4f) forced = true; // when ball goes slower, allow less walking in ball
      movement direction
      }
      */
      // if (dot >= 0.0f) forced = true; // force <= perpendicular
      float justInTimeFactor = clamp(player->GetTimeNeededToGetToBall_ms() / (float)time_ms, 0.0f,
                                     1.0f);  // lower == more time to spare
      ...
      float targetVelocity =
          clamp(targetDistance * distanceToVelocityMultiplier, idleVelocity, sprintVelocity);
      Vector3 targetMovement = (targetPos - playerPos).GetNormalized(0) * targetVelocity;
      float movementRating =
          1.0f - NormalizedClamp((desiredMovement - targetMovement).GetLength(), 0.0f,
                                 sprintVelocity);  // > sprintvelocity will often get us farther
                                                   // from where we want to go to
      float directionRating =
          1.0f -
          NormalizedClamp(
              fabs(adaptedDesiredDirection.GetAngle2D(
                  targetMovement.GetNormalized(adaptedDesiredDirection))),
              0.0f,
              0.5f * pi);  // > 0.5f * pi will often only get us farther from where we want to go to
      movementRating =
          movementRating * 0.4f +
          directionRating *
              0.6f;  // directionrating omits velocity and therefore has another quality
      ...
      float perpendicularRating =
          1.0f - NormalizedClamp((playerBallShortestTargetPos - targetPos).GetLength(), 0,
                                 2.0f * sprintVelocity);
      ...
      if (previousDesiredTargetTime_ms >= timeNeededToGetToBall_ms) {
        previousTargetRating =
            1.0f - abs(previousDesiredTargetTime_ms - timeNeededToGetToBall_ms) / 500.0f;
      }

      float rating = movementRating * movementWeight + timeRating * timeWeight +
  ...
  float bestVelocityRelaxed = clamp(
      distance * ((bestChoice.timeNeeded_ms + defaultTouchOffset_ms) / (float)bestChoice.time_ms) *
          distanceToVelocityMultiplier,
      idleVelocity, sprintVelocity);
  float bestVelocityTimeBased =
      clamp((bestChoice.timeNeeded_ms / (float)bestChoice.time_ms) * player->GetMaxVelocity(),
            idleVelocity, sprintVelocity);
  float bestVelocityASAP =
      clamp(distance * distanceToVelocityMultiplier, idleVelocity, sprintVelocity);

  // simple version
  bestVelocityFloat = bestVelocityTimeBased;  // bestVelocityRelaxed;
  // maybe just for sprinting? if (bestChoice.timeNeeded_ms > 500 && desiredVelocityFloat >
  // bestVelocityFloat) bestVelocityFloat = bestVelocityFloat * 0.5f + desiredVelocityFloat *
  // 0.5f;//sprintVelocity;
  // if (distance > 0.25f * sprintVelocity) bestVelocityFloat = clamp(desiredVelocityFloat,
  // bestVelocityFloat, sprintVelocity); // allow premature arrival at target if (bestVelocityFloat
  // < desiredVelocityFloat && bestVelocityASAP <= desiredVelocityFloat) bestVelocityFloat =
  // bestVelocityASAP;
  // stick to current velo, to prevent switching back and forth
  if (bestChoice.time_ms > 250)
    bestVelocityFloat = bestVelocityFloat * 0.97f + player->GetFloatVelocity() * 0.03f;

  // lookat dir

  Vector3 lookAtBall = (mentalImage->GetBallPrediction(20).Get2D() - playerPos)
                           .GetNormalized(adaptedDesiredDirection);
  Vector3 lookAtDirection = bestDirection;
  Vector3 lookAtDesiredDir;
  // if we're close to target, allow looking in desired direction more. if we are farther away, we
  // don't want to walk backwards yet, it makes no sense.
  ...
  // combine!
  bestLookAt =
      playerPos + lookAtBall.GetRotated2D(clamp(lookAtDesiredDir.GetAngle2D(lookAtBall) * 1.0f,
                                                -0.25f * pi, 0.25f * pi)) *
                      10.0f;
...
unsigned int AI_GetBallControlMovement(const MentalImage* mentalImage, Player* player,
                                       const Vector3& desiredDirection, float desiredVelocityFloat,
                                       Vector3& bestDirection, float& bestVelocityFloat,
                                       Vector3& bestLookAt) {
  ...
  Vector3 toBallMovement =
      mentalImage->GetBallPrediction(desiredTimeToBall_ms).Get2D() - player->GetPosition();
  ...
  if (toBallDistance < manualDirectionEndDistanceThreshold) {
    autoDirectionBias =
        std::pow(NormalizedClamp(toBallDistance, manualDirectionStartDistanceThreshold,
  ...
  // test this: if (player->GetDirectionVec().GetDotProduct(desiredDirection) < 0) manualDirection =
  // desiredDirection;
  ...
  // bestVelocityFloat = bestVelocityFloat * 0.5f + RangeVelocity(bestVelocityFloat) * 0.5f; //
  // quantization is the root of all happiness
  if (bestVelocityFloat < dribbleVelocity) {  // don't quantize low velos
    // bestVelocityFloat = idleVelocity;
  } else {
    float clampedDesiredVelocityFloat =
        clamp(desiredVelocityFloat, bestVelocityFloat, bestVelocityFloat + 8.0f);
    bestVelocityFloat = clampedDesiredVelocityFloat;
    if (RangeVelocity(bestVelocityFloat) < bestVelocityFloat)
      bestVelocityFloat = bestVelocityFloat * 0.9f + RangeVelocity(bestVelocityFloat) * 0.1f;
    if (RangeVelocity(bestVelocityFloat) > bestVelocityFloat)
      bestVelocityFloat = bestVelocityFloat * 0.1f + RangeVelocity(bestVelocityFloat) * 0.9f;
  }
...
bool AI_HasPossession(Ball* ball, Player* player) {
  ...
  if ((player->GetPosition() - ball->Predict(0)).GetLength() > 5.0)
    return false;

  Vector3 ballMovement = ball->GetMovement();
  if (fabs(ball->Predict(0).coords[2]) > 0.5)
    return false;

  bool distanceOK = true;
  float radius = 1.0f;
  Vector3 center = player->GetPosition() + player->GetMovement() * 0.05f +
                   player->GetDirectionVec() * 0.1f;  // was: 0.05f
  if ((ball->Predict(0).Get2D() - center).GetLength() > radius)
    distanceOK = false;

  bool movementOK = true;
  Vector3 ballMovement3D = (ball->Predict(10) - ball->Predict(0)) * 100.0f;
  if ((ballMovement3D - playerMovement).GetLength() > 6.0f)
    movementOK = false;

  if (distanceOK && movementOK)
    return true;
  else
    return false;
}
...
Player* AI_GetBestSwitchTargetPlayer(Match* match, Team* team, const Vector3& desiredMovement) {
  ...
  Vector3 defensePosition =
      (actionPosition * Vector3(1.0f, 0.8f, 0.0f)) + Vector3(team->GetSide() * 4.0f, 0.0f, 0.0f);
  Vector3 offensePosition =
  ...
  offenseBias = offenseBias * 0.2f + clamp(team->GetTeamPossessionAmount() - 0.5f, 0.0f, 1.0f) *
                                         0.8f;                         // more direct, more urgent
  offenseBias = clamp((offenseBias - 0.5f) * 2.0 + 0.5f, 0.0f, 1.0f);  // make more binary
  offenseBias = clamp(std::pow(offenseBias, 1.5f), 0.0f, 1.0f);        // tend towards defensive

  Vector3 resultingPosition =
      defensePosition * (1.0f - offenseBias) + offensePosition * offenseBias;

  if (team->GetID() == 0) {
    if (Verbose())
      printf("SWITCH: offenseBias: %f\n", offenseBias);
  ...
  if (designated->GetTeamID() != team->GetID()) {
    ...
    for (unsigned int i = 0; i < teamPlayers.size(); i++) {
      float mateGoalDist =
          (goalPos - (teamPlayers.at(i)->GetPosition() + teamPlayers.at(i)->GetMovement() * 0.5f))
              .GetLength();
      if (mateGoalDist <
          oppGoalDist + tooLateDistance +
              clamp(oppGoalDist * 0.1f, 0.0f,
                    3.0f)) {  // doesn't matter much when opp is still far away from goal
        bestPlayerIndex = i;
        break;
      }
    }
...
void AI_GetAutoPass(e_FunctionType passType, const Vector3& vector, Vector3& resultingDirection,
  ...
  resultingPower =
      std::pow(NormalizedClamp(vector.GetLength(), 0.0f, 60.0f), distanceExp) * powerFactor;
}

void AI_GetPass(Player* player, e_FunctionType passType, const Vector3& inputDirection,
                float inputPower, float autoDirectionBias, float autoPowerBias,
                Vector3& resultingDirection, float& resultingPower, Player*& targetPlayer,
                Player* forcedTargetPlayer) {
  // cheat for digital input

  bool fullAutoDirection = false;
  bool fullAutoPower = false;
  if (player->GetExternalController()) {
    if (static_cast<HumanController*>(player->GetExternalController())
            ->GetHIDevice()
            ->GetDeviceType() == e_HIDeviceType_Keyboard) {
  ...
  Vector playerPos =
      player->GetPosition() + player->GetMovement().GetNormalized(0) *
                                  0.2f;  // + ffo. don't need movement for future stuff, since
                                         // getpass is recalled at moment of passing, for refinement
  if (player->TouchAnim()) {
    ...
    for (int i = 0; i < (signed int)players.size(); i++) {
      if (players.at(i) != player /* && players.at(i)->IsActive()*/) {
        ...
        float distanceRating =
            std::pow(NormalizedClamp((targetPos - manualTarget).GetLength(), 0.0f, 70.0f),
                     0.8f) *
            0.8f;  // pow() this so small differences matter more - from some
                   // point on, it just doesn't really matter that much anymore
        float angleRating =
            fabs((targetPos - playerPos).GetNormalized(0).GetAngle2D(inputDirection) /
                 (1.0f * pi)) *
            1.0f;
  ...
  if (forcedTargetPlayer || (fullAutoDirection && fullAutoPower)) {
    ...
    float distanceFactor =
        1.0 - std::pow(clamp((autoTargetRel - manualTargetRel).GetLength() / maxAllowedDistance,
                             0.0f, 1.0f),
                       1.5f);
    // extra help with power on close targets (because people can only physicaly press the pass
    // button so short)
    float proximityBonus =
        std::pow(1.0f - NormalizedClamp((playerPos - autoTarget).GetLength(), 0.0f, 12.0f), 0.5f);

    if (fullAutoDirection) {
      adaptedAutoDirectionBias = 1.0f;
    } else {
      adaptedAutoDirectionBias *= distanceFactor;
      adaptedAutoDirectionBias = std::pow(adaptedAutoDirectionBias, 1.0f - proximityBonus * 0.9f);
    }
    if (fullAutoPower) {
  ...
  Vector3 resultingTargetRel =
      (autoTargetRel.GetNormalized(0) * adaptedAutoDirectionBias +
       manualTargetRel.GetNormalized(0) * (1.0f - adaptedAutoDirectionBias))
          .GetNormalized(manualTarget);
  resultingTargetRel *= float(autoTargetRel.GetLength() * adaptedAutoPowerBias +
                              manualTargetRel.GetLength() * (1.0f - adaptedAutoPowerBias));
...
Vector3 AI_GetShotDirection(Player* player, const Vector3& inputDirection,
  ...
  return (manualDirection * (1.0f - autoDirectionBias) + autoDirection * autoDirectionBias)
...
float AI_GetMindSet(e_PlayerRole role) {
  ...
  if (role == e_PlayerRole_DM)
    mindSet = 0.25;

  if (role == e_PlayerRole_LM || role == e_PlayerRole_CM || role == e_PlayerRole_RM)
    mindSet = 0.5;
