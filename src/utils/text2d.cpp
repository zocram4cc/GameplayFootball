...
Text2D::Text2D(std::shared_ptr<Scene2D> scene2D, const std::string& fontFile, int pts)
    : count(0), scene2D(scene2D) {
  assert(1 == 2);  // todo: error, this file is deprecated, can't do ttf_init for one, already done
                   // that. check if this file/this whole old 2d/gui system is used anyway, and if
                   // not, remove from project
  TTF_Init();
...
boost::intrusive_ptr<Image2D> Text2D::Create(int width) {
  ...
  SDL_Surface* sdlTextPow2 = SDL_CreateRGBSurface(alpha, pot(sdlText->w), pot(sdlText->h), 32,
  ...
  boost::intrusive_ptr<Resource<Surface>> surface =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch("text2D #" + int_to_str(count), false, false);
  surface->GetResource()->SetData(sdlTextPow2);

  // todo: unique name?
  boost::intrusive_ptr<Image2D> image =
      boost::static_pointer_cast<Image2D>(ObjectFactory::GetInstance().CreateObject(
          "text2D #" + int_to_str(count), e_ObjectType_Image2D));
...
void Text2D::SetText(boost::intrusive_ptr<Image2D> image, const std::string& text,
  ...
  assert(sdlTextPow2);
  // no clipping
  if (rect.w > sdlTextPow2->w)
    rect.w = sdlTextPow2->w;
...
SDL_Surface* Text2D::RenderTextSurface(const std::string& text, const Vector3& color) const {
  SDL_Color sdlColor = {(unsigned char)(floor(color.coords[0] * 255.0)),
                        (unsigned char)(floor(color.coords[1] * 255.0)),
                        (unsigned char)(floor(color.coords[2] * 255.0)), 0};
