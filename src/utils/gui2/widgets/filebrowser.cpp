...
void Gui2FileBrowser::DisplayDirectory() {
  ...
  for (unsigned int i = 0; i < dirContents.size(); i++) {
    Vector3 color;
    if (dirContents.at(i).type == e_DirEntryType_File)
...
void Gui2FileBrowser::GetDirectoryContents() {
  ...
  if (!fs::exists(path) || !fs::is_directory(path))
    Log(e_Error, "GuiFileBrowser", "GetDirectoryContents",
        "Could not open directory " + directory + " for reading");
  ...
  while (dirIter != endIter) {
    DirEntry entry;
    entry.name =
        dirIter->path().filename().string();  // used to be .native() .. todo, check out why
