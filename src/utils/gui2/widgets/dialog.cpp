...
Gui2Button *Gui2Dialog::AddPosNegButtons(const std::string &posName, const std::string &negName) {
  ...
  Gui2Button *posButton = new Gui2Button(windowManager, name + "_" + posName + "_button",
                                         width_percent / 2.0 - buttonWidth - 0.5,
                                         height_percent - 4, buttonWidth, 3, posName);
  Gui2Button *negButton =
      new Gui2Button(windowManager, name + "_" + negName + "_button", width_percent / 2.0 + 0.5,
                     height_percent - 4, buttonWidth, 3, negName);
...
void Gui2Dialog::ProcessWindowingEvent(WindowingEvent *event) {
  if (event->IsEscape()) {
    // not used? (and sig_OnClose is now in View, so at least should be renamed to OnCloseDialog or
    // something): sig_OnClose(this);
  } else {
    event->Ignore();
  }
}
