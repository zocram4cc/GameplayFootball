...
Gui2IconSelector::Gui2IconSelector(Gui2WindowManager* windowManager, const std::string& name,
  ...
  Gui2Caption* captionView =
      new Gui2Caption(windowManager, name + "caption", 1, 0.3, width_percent, 2.5, caption);
  this->AddView(captionView);
  captionView->Show();
  selectedCaption = new Gui2Caption(windowManager, name + "selectedcaption", 1,
                                    height_percent * 0.83, width_percent, 2.5, "empty");
...
void Gui2IconSelector::Process() {
  ...
  if (fabs(visibleSelectedEntry - (float)selectedEntry) <= 0.08f * scrollSpeedFactor) {
    ...
    if (visibleSelectedEntry > (float)selectedEntry)
      visibleSelectedEntry -= 0.08f * scrollSpeedFactor;
    else if (visibleSelectedEntry < (float)selectedEntry)
      visibleSelectedEntry += 0.08f * scrollSpeedFactor;
  }
...
void Gui2IconSelector::Redraw() {
  ...
  int x, y, w, h;
  windowManager->GetCoordinates(x_percent, y_percent, width_percent, height_percent, x, y, w, h);
  image->DrawRectangle(0, 0, w, h, windowManager->GetStyle()->GetColor(e_DecorationType_Bright2),
                       alpha);
  image->OnChange();
  ...
  if (entries.size() != 0) {
    ...
    selectedCaption->SetCaption("None selected");
}
...
void Gui2IconSelector::AddEntry(const std::string& id, const std::string& caption,
...
void Gui2IconSelector::ProcessWindowingEvent(WindowingEvent* event) {
  ...
  if (visibleSelectedEntry == (float)selectedEntry) {
    ...
    if (selectedEntry > (signed int)entries.size() - 1)
      selectedEntry = (signed int)entries.size() - 1;
    if (selectedEntry < 0)
      selectedEntry = 0;
