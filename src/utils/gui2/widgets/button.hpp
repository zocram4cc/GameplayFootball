...
public:
  Gui2Button(Gui2WindowManager* windowManager, const std::string& name, float x_percent,
  ...
  virtual void SetActive(bool onOff) {
    this->active = onOff;
    if (onOff)
      SetColor(windowManager->GetStyle()->GetColor(e_DecorationType_Bright1));
    else
      SetColor(windowManager->GetStyle()->GetColor(e_DecorationType_Dark2));
    Redraw();
  }
