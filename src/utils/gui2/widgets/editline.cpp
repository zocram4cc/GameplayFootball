...
Gui2EditLine::Gui2EditLine(Gui2WindowManager* windowManager, const std::string& name,
                           float x_percent, float y_percent, float width_percent,
                           float height_percent, const std::string& defaultText)
  ...
  caption = new Gui2Caption(windowManager, GetName() + "_caption", 0, 0, width_percent,
                            height_percent, defaultText);
      ...
      switch (key) {
        ...
        case SDLK_RIGHT: {
          ...
          if (cursorPos > (signed int)currentText.length())
            cursorPos = (signed int)currentText.length();
          break;
        }
        case SDLK_BACKSPACE: {
          if (cursorPos > 0) {
