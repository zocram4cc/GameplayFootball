...
Gui2WindowManager::Gui2WindowManager(std::shared_ptr<Scene2D> scene2D, float aspectRatio,
                                     float margin)
    : scene2D(scene2D), aspectRatio(aspectRatio), margin(margin), pageFactory(0) {
  ...
  if (contextAspectRatio >
      aspectRatio) {  // context width is larger than "virtual context's" width, so cap by height
  ...
  boost::intrusive_ptr<Resource<Surface>> resource =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch("gui2_blackoutbackground", false, true);
...
void Gui2WindowManager::GetCoordinates(float x_percent, float y_percent, float width_percent,
...
boost::intrusive_ptr<Image2D> Gui2WindowManager::CreateImage2D(const std::string& name, int width,
                                                               int height, bool sceneRegister) {
  SDL_Surface* sdlSurface = CreateSDLSurface(width, height);

  boost::intrusive_ptr<Resource<Surface>> resource =
      ResourceManagerPool::GetInstance()
          .GetManager<Surface>(e_ResourceType_Surface)
          ->Fetch(name, false, true);
  Surface* surface = resource->GetResource();

  surface->SetData(sdlSurface);

  boost::intrusive_ptr<Image2D> image = boost::static_pointer_cast<Image2D>(
      ObjectFactory::GetInstance().CreateObject(name, e_ObjectType_Image2D));
  if (sceneRegister)
