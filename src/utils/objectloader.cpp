...
boost::intrusive_ptr<Node> ObjectLoader::LoadLevel(std::shared_ptr<Scene3D> scene3D,
...
boost::intrusive_ptr<Node> ObjectLoader::LoadObject(std::shared_ptr<Scene3D> scene3D,
  ...
  boost::intrusive_ptr<Node> result =
      LoadObjectImpl(scene3D, filename, objectTree.children.begin()->second, offset);
  // printf("\n\n");
  // result->PrintTree();
  // printf("\n\n");
  return result;
...
boost::intrusive_ptr<Node> ObjectLoader::LoadObjectImpl(std::shared_ptr<Scene3D> scene3D,
  ...
  while (objectIter != objectTree.children.end()) {
    ...
    // GEOMETRY
      ...
      boost::intrusive_ptr<Resource<GeometryData>> geometry =
          ResourceManagerPool::GetInstance()
              ...
              ->Fetch(dirpart + aseFilename, true);
      boost::intrusive_ptr<Geometry> object = boost::static_pointer_cast<Geometry>(
          ObjectFactory::GetInstance().CreateObject(objectName, objectType));
      if (properties.GetBool("dynamic"))
        geometry->GetResource()->SetDynamic(true);
    ...
    // LIGHT
      ...
      boost::intrusive_ptr<Light> object = boost::static_pointer_cast<Light>(
          ObjectFactory::GetInstance().CreateObject(objectName, objectType));
    ...
    // JOINT waaaah smoke em
      ...
      boost::intrusive_ptr<Joint> object = boost::static_pointer_cast<Joint>(
          ObjectFactory::GetInstance().CreateObject(objectName, objectType));
      object->SetProperties(properties);
      scene3D->CreateSystemObjects(object);
      objNode->AddObject(object);
      ...
      if (target_1 != "")
        target_1_object = boost::static_pointer_cast<Geometry>(objNode->GetObject(target_1));
      if (target_2 != "")
        target_2_object = boost::static_pointer_cast<Geometry>(objNode->GetObject(target_2));
