...
boost::intrusive_ptr<Resource<GeometryData>> GetGridGeom(const std::string& name,
                                                         std::shared_ptr<Scene3D> scene3D,
                                                         std::vector<GeomIndex>& geomVec,
                                                         const AABB& aabb, float gridSize) {
  ...
  newIndex.geomData =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch(name + " gridGeomData @ " + int_to_str(x) + ", " + int_to_str(y), false, false);
...
boost::intrusive_ptr<Node> SplitGeometry(std::shared_ptr<Scene3D> scene3D,
                                         boost::intrusive_ptr<Geometry> source, float gridSize) {
  ...
  for (int i = 0; i < (signed int)tmeshes.size(); i++) {
    ...
    AABB aabb =
        GetTriangleMeshAABB(tmeshes.at(i).vertices, tmeshes.at(i).verticesDataSize, indices);
    boost::intrusive_ptr<Resource<GeometryData>> subGeomData =
        GetGridGeom(source->GetName(), scene3D, geomVec, aabb, gridSize);
    ...
    subGeomData->GetResource()->AddTriangleMesh(tmeshes.at(i).material, newTMesh,
                                                tmeshes.at(i).verticesDataSize, indices);
  ...
  for (int i = 0; i < (signed int)geomVec.size(); i++) {
    ...
    boost::intrusive_ptr<Geometry> geom =
        boost::static_pointer_cast<Geometry>(ObjectFactory::GetInstance().CreateObject(
            source->GetName() + " gridGeom @ " + int_to_str(x) + ", " + int_to_str(y),
