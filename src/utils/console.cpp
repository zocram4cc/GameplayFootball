...
Console::Console(std::shared_ptr<Scene2D> scene2D, int numlines)
    : scene2D(scene2D), numlines(numlines) {
  ...
  logConn = BindLog(
      [this](e_LogType a, std::string b, std::string c, std::string d) { PrintLog(a, b, c, d); });
}
