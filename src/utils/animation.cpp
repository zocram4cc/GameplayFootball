...
radian FixAngle(radian angle, bool modulateIntoRange = true) {
  ...
  newAngle += 0.5 * pi;
  if (modulateIntoRange)
    newAngle = ModulateIntoRange(-pi, pi, newAngle);
  return newAngle;
}
...
bool Animation::GetKeyFrame(std::string nodeName, int frame, Quaternion& orientation,
  ...
  for (int i = 0; i < animSize; i++) {
    if (nodeAnimations.at(i)->nodeName == nodeName) {
      GetInterpolatedValues(nodeAnimations.at(i)->animation, frame, orientation, position,
...
void Animation::DeleteKeyFrame(std::string nodeName, int frame) {
  ...
  for (int i = 0; i < animSize; i++) {
    ...
    if (nodeAnimation->nodeName == nodeName) {
      ...
      if (animIter == nodeAnimation->animation.end())
        printf("ERROR! keyframe does not exist\n");
    }
  }
...
void Animation::GetInterpolatedValues(const std::map<int, KeyFrame>& animation, int frame,
                                      Quaternion& orientation, Vector3& position,
                                      bool getOrientation, bool getPosition) const {
  ...
  if (frame > 0 && frame < GetFrameCount()) {
    ...
    if (getOrientation) {
      if (weighedKeys.size() > 1) {
        orientation = weighedKeys.at(0).keyFrame.orientation.GetSlerped(
            bias, weighedKeys.at(1).keyFrame.orientation);
      } else {
    ...
    if (weighedKeys.size() == 0)
      return;  // should not happen
    if (weighedKeys.size() == 1) {
      if (getOrientation)
        orientation = weighedKeys.at(0).keyFrame.orientation;
      if (getPosition)
        position = weighedKeys.at(0).keyFrame.position;
      return;
    }
    // extrapolate
    ...
    // extrapolation through hax: bias > 1 doesn't really give proper results. however, it works for
    // small angles so FUFUUFUUUU XD (use slerp instead of lerp - lerp can't extrapolate at all)
    // bias = clamp(bias, 0.0f, 1.0f);
    if (getOrientation)
      orientation = weighedKeys.at(1).keyFrame.orientation.GetSlerped(
          bias, weighedKeys.at(0).keyFrame.orientation);
    if (getPosition)
      position = weighedKeys.at(1).keyFrame.position * (1 - bias) +
                 weighedKeys.at(0).keyFrame.position * bias;
...
void Animation::ConvertToStartFacingForwardIfIdle() {
  ...
  // variables
  Vector3 newBallDirVec =
      GetVariable("balldirection") != ""
          ? GetVectorFromString(GetVariable("balldirection")).GetRotated2D(-incomingBodyAngle)
          : 0;
  Vector3 newIncomingBallDirVec = GetVariable("incomingballdirection") != ""
                                      ? GetVectorFromString(GetVariable("incomingballdirection"))
                                            .GetRotated2D(-incomingBodyAngle)
                                      : 0;
  Vector3 newBumpDirVec =
      GetVariable("bumpdirection") != ""
          ? GetVectorFromString(GetVariable("bumpdirection")).GetRotated2D(-incomingBodyAngle)
          : 0;
...
void Animation::Apply(const std::map<const std::string, boost::intrusive_ptr<Node>> nodeMap,
                      int frame, int timeOffset_ms, bool smooth, float smoothFactor,
                      /*const std::shared_ptr<Animation> previousAnimation, int smoothFrames, */
                      const Vector3& basePos, radian baseRot,
                      std::map<std::string, BiasedOffset>& offsets,
                      MovementHistory* movementHistory, int timeDiff_ms, bool noPos,
                      bool updateSpatial) {
  // simple keyframe-to-keyframe version
  ...
  for (unsigned int i = 0; i < animSize; i++) {
    ...
    float bias = 0.5f;
    if (timeOffset_ms != -1)
      bias = clamp(timeOffset_ms / 10.0f, 0.0f, 1.0f);
    int smoothFrames = 0;
    if (smooth && 1 == 2) {
      smoothFrames = 0;
      if (GetAnimType() == "movement")
        smoothFrames = 1;
      float factor = (smoothFrames * 2) + 1;
      bias /= factor;
      bias += 0.5f - ((1.0f / factor) * 0.5f);  // center, then subtract half of new size
    }
    GetInterpolatedValues(nodeAnimation->animation, frame - smoothFrames, orientation_pre,
                          position_pre);
    GetInterpolatedValues(nodeAnimation->animation, frame + 1 + smoothFrames, orientation_post,
                          position_post);
    ...
    if (iter != offsets.end()) {
      ...
      if (iter->second.isRelative) {
        orientation =
            orientation.GetLerped(iter->second.bias, iter->second.orientation * orientation)
                .GetNormalized();
      } else {
        orientation =
            orientation.GetLerped(iter->second.bias, iter->second.orientation).GetNormalized();
      }
    }
    ...
    if (smooth) {
      // needed for smoothing - keep track of old limb positions/movements so we can extrapolate
      ...
      if (nodeAnimation->nodeName.compare("player") != 0) {
        ...
        if (timeDiff_ms > 0) {
          ...
          if (simpleMethod == false) {
            ...
            // (previousOrientation.GetRotationTo(currentOrientation)).GetNormalized() * (1.0f /
            // (movementHistoryEntry->timeDiff_ms * 0.001f));
            Quaternion currentRotation_per_ms =
                previousOrientation.GetRotationTo(currentOrientation).GetNormalized();
            currentRotation_per_ms = currentRotation_per_ms.GetRotationMultipliedBy(
                1.0f / (float)movementHistoryEntry->timeDiff_ms);  // already normalized in function
            ...
            orientation = orientation
                              .GetSlerped(pow(1.0f - clamp(timeDiff_ms / 30.0f, 0.0f, 1.0f), 0.5f) *
                                              (0.5f + smoothFactor * 0.5f * beginBias),
                                          currentOrientation)
                              .GetNormalized();
            // orientation = orientation.GetSlerped(pow(1.0f - clamp(timeDiff_ms / 30.0f,
            // 0.0f, 1.0f), 0.5f), currentOrientation).GetNormalized(); orientation =
            // orientation.GetSlerped(0.9f + 0.1f * beginBias, currentOrientation).GetNormalized();
            ...
            if (angleDiff_per_ms > maxDiff_per_ms) {
              ...
              // if (desiredRotation.elements[3] < 0) desiredRotation.elements[3] =
              // -desiredRotation.elements[3];
              orientation =
                  desiredRotation.GetRotationMultipliedBy(allowFraction) * currentOrientation;
              orientation.Normalize();
            }
            ...
            /*
               ...
               currentOrientation.GetRotationTo(orientation).GetNormalized(); desiredRotation_per_ms
               = desiredRotation_per_ms.GetRotationMultipliedBy(1.0f / (float)timeDiff_ms);
                    //assert(currentRotation_per_ms.GetDotProduct(desiredRotation_per_ms) >= 0.0f);
                    //float dot =
               currentRotation_per_ms.MakeSameNeighborhood(desiredRotation_per_ms); Quaternion
               dampingRotation_per_ms =
               currentRotation_per_ms.GetRotationTo(desiredRotation_per_ms).GetNormalized(); dot =
               dampingRotation_per_ms.GetDotProduct(currentRotation_per_ms); radian angle_sec = 2.0f
               * acos(clamp(dot, -1.0f, 1.0f)) * 1000.0f; // basically: angle between desired and
               current rotation (per second) if (angle_sec > 0.0f) { radian dampAngle_sec = 0.15f *
               pi; float dampFactor = NormalizedClamp(dampAngle_sec, 0.0f, angle_sec);

                      if (currentRotation_per_ms.GetDotProduct(identity) <
               desiredRotation_per_ms.GetDotProduct(identity)) dampFactor = 1.0f; // slowing down
               joints is easier than accelerating

                      // now change (hax!) currentRotation so that it's closer to the
               desiredRotation
                      //XXcurrentRotation_per_ms =
               dampingRotation_per_ms.GetRotationMultipliedBy(dampFactor) * currentRotation_per_ms;
                    }
            */
            /*
                    Quaternion extrapolatedOrientation =
               currentRotation_per_ms.GetRotationMultipliedBy(timeDiff_ms) * currentOrientation;
                    //Quaternion extrapolatedOrientation =
               desiredRotation_per_ms.GetRotationMultipliedBy(timeDiff_ms) * currentOrientation;
                    extrapolatedOrientation.Normalize();

                    // now we want to go from current to new orientation, but we can only differ so
            ...
            */
            ...
            // now get the resulting rotation, which is the current rotation, converged as much
            // towards the desired rotation as allowed, based upon maximum rotational angle change
            // per time unit
            Quaternion resultingRotation_per_ms = desiredRotation_per_ms;
            float currentToDesiredRotationDot =
                desiredRotation_per_ms.MakeSameNeighborhood(currentRotation_per_ms);
            radian angleDiff_per_ms_per_s = 2.0f *
                                            acos(clamp(currentToDesiredRotationDot, -1.0f, 1.0f)) /
            ...
            if (angleDiff_per_ms_per_s > maxDiff_per_ms_per_s) {
              ...
              Quaternion desiredRotationRotation =
                  currentRotation_per_ms.GetRotationTo(desiredRotation_per_ms).GetNormalized();
              // if (desiredRotation.elements[3] < 0) desiredRotation.elements[3] =
              // -desiredRotation.elements[3];
              resultingRotation_per_ms =
                  desiredRotationRotation.GetRotationMultipliedBy(allowFraction) *
                  currentRotation_per_ms;
              resultingRotation_per_ms.Normalize();
            }

            // Quaternion resultingRotation_per_ms =
            // desiredRotation_per_ms.GetLerped(movementInfluence, currentRotation_per_ms);
            Quaternion resultingOrientation =
                resultingRotation_per_ms.GetRotationMultipliedBy(timeDiff_ms) * currentOrientation;
            ...
            // add some extra overall smoothness
            // orientation = orientation.GetSlerped(pow(1.0f - clamp(timeDiff_ms / 30.0f,
            // 0.0f, 1.0f), 0.5f) * (0.6f + 0.2f * beginBias), currentOrientation).GetNormalized();
            // orientation =
            // (identity.GetRotationTo(orientation).GetNormalized().GetRotationMultipliedBy(0.1f) *
            // orientation).GetNormalized(); orientation = orientation.GetSlerped(pow(1.0f -
            // clamp(timeDiff_ms / 30.0f, 0.0f, 1.0f), 0.5f) * (0.2f + 0.1f * beginBias),
            // currentOrientation).GetNormalized(); orientation = orientation.GetSlerped(0.3f +
            // beginBias * 0.2f, currentOrientation).GetNormalized(); // incorrect! incorporate
            // timeDiff_ms into this somehow, whilst still allowing for less influence than the
            // above version
        ...
        */
        ...
        if (timeDiff_ms > 0 && beginBias > 0.01f) {
          Vector3 currentMovement =
              (currentPosition - previousPosition) / (movementHistoryEntry->timeDiff_ms * 0.001f);
          /*
                    // damping (maybe useful: http://www.freebasic.net/forum/viewtopic.php?t=9769 )
                    Vector3 desiredMovement = ((basePos + position) - currentPosition) /
             (timeDiff_ms * 0.001f); Vector3 dampingMovement = (desiredMovement - currentMovement);
                    currentMovement += dampingMovement.GetNormalizedMax(timeDiff_ms * 0.02f);

                    Vector3 extrapolatedPosition = currentPosition + currentMovement * (timeDiff_ms
             * 0.001f);

                    // now we want to go from current to new position, but we can only differ so
             much from the movement from previous to current position float movementInfluence = 1.0f
             - clamp(timeDiff_ms / 50.0f, 0.0f, 1.0f); movementInfluence = pow(movementInfluence,
             0.5f); // influence of old movement wears off in exponential fashion (source: laws of
             nature) float beginBias = pow(1.0f - NormalizedClamp(frame, 0, 15), 0.5f);
                    movementInfluence *= 0.4f + beginBias * 0.6f;

                    //if (desiredMovement.GetLength() < currentMovement.GetLength())
             movementInfluence = 0.0f;
                    //if (currentMovement.GetLength() > desiredMovement.GetLength()) currentMovement
             = currentMovement.GetNormalized() * desiredMovement.GetLength();
                    //if (currentMovement.GetLength() > desiredMovement.GetLength()) currentMovement
             = currentMovement.GetNormalized() * (currentMovement.GetLength() * movementInfluence +
             desiredMovement.GetLength() * (1.0f - movementInfluence));

                    Vector3 newPosition = (extrapolatedPosition) * movementInfluence + //
             current-movement-implied position (basePos + position) * (1.0f - movementInfluence); //
             desired position
          */

          // smooth
          position.coords[2] =
              position.coords[2] * (1.0f - currentBias) + currentPosition.coords[2] * currentBias;

          // old version, use for now, until new version above is finished

          float maxMetersPerSec = 2.8f;  // 1.8f
          if (GetVariable("outgoing_special_state").compare("") != 0)
            maxMetersPerSec = 6.0f;
          float allowedDistance = maxMetersPerSec * ((float)timeDiff_ms * 0.001f);

          // float oldPosition =
          // nodeMap.find(nodeAnimation->nodeName)->second->GetPosition().coords[2];
          ...
          newZ = (newZ * bias + currentPosition.coords[2] * (1.0f - bias)) * beginBias +
                 newZ * (1.0f - beginBias);  // dual biasses ^_^ works like this: currentPosition
                                             // influence only effective when beginbias > 0
          ...
          // nodeMap.find(nodeAnimation->nodeName)->second->SetPosition((position + basePos).Get2D()
          // + Vector3(0, 0, newZ), false);
        }
  ...
  if (updateSpatial)
    (*nodeMap.find(nodeAnimations.at(0)->nodeName))
        .second->RecursiveUpdateSpatialData(e_SpatialDataType_Both);
}
    ...
    if (nodeAnimations.at(0)->animation.size() > 1) {
      cache_incomingMovement = ((++nodeAnimations.at(0)->animation.begin())->second.position -
                               ...
                               ((++nodeAnimations.at(0)->animation.begin())->first -
                                nodeAnimations.at(0)->animation.begin()->first * 1.0) *
                               100;
    ...
    if (nodeAnimations.at(0)->animation.size() > 1) {
      cache_outgoingMovement = ((--nodeAnimations.at(0)->animation.end())->second.position -
                               ...
                               ((--nodeAnimations.at(0)->animation.end())->first -
                                (--(--nodeAnimations.at(0)->animation.end()))->first * 1.0) *
                               100;
...
Vector3 Animation::GetRangedOutgoingMovement() const {
  if (cache_rangedOutgoingMovement_dirty || cache_outgoingVelocity_dirty || cache_angle_dirty) {
    if (nodeAnimations.at(0)->animation.size() > 1) {
      cache_rangedOutgoingMovement =
          Vector3(0, -GetOutgoingVelocity(), 0).GetRotated2D(GetOutgoingAngle());
    } else {
...
float Animation::GetOutgoingVelocity() const {
  if (cache_outgoingVelocity_dirty) {
    if (nodeAnimations.at(0)->animation.size() > 1) {
      ...
      if (cache_outgoingVelocity < 1.8)
        cache_outgoingVelocity = 0.0;
      else if (cache_outgoingVelocity >= 1.8 && cache_outgoingVelocity < 4.2)
        cache_outgoingVelocity = 3.5;
      else if (cache_outgoingVelocity >= 4.2 && cache_outgoingVelocity < 6.0)
        cache_outgoingVelocity = 5.0;
      else if (cache_outgoingVelocity >= 6.0)
        cache_outgoingVelocity = 7.0;
    } else {
...
radian Animation::GetOutgoingAngle() const {
  if (cache_angle_dirty || cache_outgoingVelocity_dirty) {
    if (GetOutgoingVelocity() >= 1.8) {
      ...
      // if angle is close to 180 degrees, we can't be sure if we want 180 or -180 deg. use body
      // angle as hint.
      if (cache_angle < -0.95f * pi || cache_angle > 0.95f * pi) {
        ...
        if (signSide(cache_angle) != signSide(z)) {
          ...
          cache_angle = clamp(
              cache_angle, -0.99f * pi,
              0.99f * pi);  // also do this if the side is already correct: we want to be a little
                            // away from pi, else vectors based on this don't have a clear sidedness
        }
      }
...
void Animation::Load(const std::string& filename) {
  ...
  for (unsigned int i = 0; i < tokenizedLines.size(); i++) {
    ...
    if (extensionIter != extensions.end())
      (*extensionIter).second->Load(tokenizedLines.at(i));
  }
  ...
  if (iter != customData->children.end()) {
    ...
    if (bumpDirection.GetLength() > 0)
      bumpDirection.Normalize();
    iter->second.value = real_to_str(bumpDirection.coords[0]) + "," +
                         real_to_str(bumpDirection.coords[1]) + "," +
                         real_to_str(bumpDirection.coords[2]);
  }
  ...
  if (iter != customData->children.end()) {
    ...
    if (ballDirection.GetLength() > 0)
      ballDirection.Normalize();
    iter->second.value = real_to_str(ballDirection.coords[0]) + "," +
                         real_to_str(ballDirection.coords[1]) + "," +
                         real_to_str(ballDirection.coords[2]);
  }
  ...
  if (iter != customData->children.end()) {
    ...
    if (incomingBallDirection.GetLength() > 0)
      incomingBallDirection.Normalize();
    iter->second.value = real_to_str(incomingBallDirection.coords[0]) + "," +
                         real_to_str(incomingBallDirection.coords[1]) + "," +
                         real_to_str(incomingBallDirection.coords[2]);
  }
...
void Animation::Mirror() {
  ...
  Vector3 newBallDirVec =
      GetVariable("balldirection") != ""
          ? GetVectorFromString(GetVariable("balldirection")) * Vector3(-1, 1, 1)
          : 0;
  Vector3 newIncomingBallDirVec =
      GetVariable("incomingballdirection") != ""
          ? GetVectorFromString(GetVariable("incomingballdirection")) * Vector3(-1, 1, 1)
          : 0;
  Vector3 newBumpDirVec =
      GetVariable("bumpdirection") != ""
          ? GetVectorFromString(GetVariable("bumpdirection")) * Vector3(-1, 1, 1)
          : 0;
  SetVariable("balldirection", GetStringFromVector(newBallDirVec));
  SetVariable("incomingballdirection", GetStringFromVector(newIncomingBallDirVec));
  SetVariable("bumpdirection", GetStringFromVector(newBumpDirVec));
...
void Animation::AddExtension(const std::string& name,
...
void Animation::Hax() {
  /*
      ...
      int frame;
      bool touch =
     std::static_pointer_cast<FootballAnimationExtension>(GetExtension("football"))->GetFirstTouch(position,
     frame); if (touch) { Vector3 angles, position; float power;
