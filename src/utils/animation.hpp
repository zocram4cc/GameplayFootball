...
enum e_Foot { e_Foot_Left, e_Foot_Right };
...
//
// rules:
//   - first node inserted (by using SetKeyFrame) should be root node of the animated object (for
...
public:
  ...
  void Invert();
  void Apply(const std::map<const std::string, boost::intrusive_ptr<Node>> nodeMap, int frame,
             int timeOffset_ms = 0, bool smooth = true, float smoothFactor = 1.0f,
             ...
             MovementHistory* movementHistory = 0, int timeDiff_ms = 10, bool noPos = false,
             bool updateSpatial = true);
