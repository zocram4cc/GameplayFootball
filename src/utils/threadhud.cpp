...
ThreadHud::ThreadHud(std::shared_ptr<Scene2D> scene2D) : scene2D(scene2D) {
  ...
  graph = boost::static_pointer_cast<Image2D>(
      ObjectFactory::GetInstance().CreateObject("threadHUD graphic", e_ObjectType_Image2D));
  scene2D->CreateSystemObjects(graph);

  boost::intrusive_ptr<Resource<Surface>> surface = ResourceManagerPool::GetInstance()
                                                        .GetManager<Surface>(e_ResourceType_Surface)
                                                        ->Fetch("threadHUD graphic", false, false);
...
void ThreadHud::Execute() {
  ...
  TaskManager* taskManager = TaskManager::GetInstancePtr();
  unsigned int workerThreadNum = taskManager->GetWorkerThreadCount();
  assert(workerThreadNum ==
         workerThreadHistory.size());  // this class needs work if we ever decide to make the number
                                       // of worker threads dynamic
  ...
  if (eventManager->GetKeyboardState(SDLK_F4)) {
    gui_autoPlay ? gui_autoPlay = false : gui_autoPlay = true;
    gui_focusTime_ms = time_ms;
...
void ThreadHud::Redraw(bool forceRedraw) {
  if (skipRedrawCounter == 0 || forceRedraw) {
    ...
    graph->DrawSimpleText("time: " + int_to_str(gui_beginTime_ms) + " ms", 5, 5, font,
                          Vector3(200, 200, 200), 255);
    graph->DrawSimpleText("range: " + int_to_str(gui_endTime_ms - gui_beginTime_ms) + " ms",
                          _width * 0.5 - 50, 5, font, Vector3(200, 200, 200), 255);
    graph->DrawSimpleText("time: " + int_to_str(gui_endTime_ms) + " ms", _width - 100, 5, font,
                          Vector3(200, 200, 200), 255);
    ...
    for (unsigned int thread = 0; thread < workerThreadNum; thread++) {
      for (unsigned int entry = 0; entry < workerThreadHistory.at(thread).size(); entry++) {
        ...
        if (workerThreadHistory.at(thread).at(entry).state != e_ThreadState_Idle) {
          if (workerThreadHistory.at(thread).at(entry).beginTime_ms <= gui_endTime_ms &&
              workerThreadHistory.at(thread).at(entry).endTime_ms >= gui_beginTime_ms) {
            // calculate min/max visible time of this item
            int beginDisplayTime_ms =
                std::max(workerThreadHistory.at(thread).at(entry).beginTime_ms, gui_beginTime_ms);
            int endDisplayTime_ms =
                std::min(workerThreadHistory.at(thread).at(entry).endTime_ms, gui_endTime_ms);
            ...
            unsigned char r = GetHashFromCharString(
                (workerThreadHistory.at(thread).at(entry).commandName + "blabliebloe").c_str());
            unsigned char g = GetHashFromCharString(
                (workerThreadHistory.at(thread).at(entry).commandName + "pirpriii").c_str());
            unsigned char b = GetHashFromCharString(
                (workerThreadHistory.at(thread).at(entry).commandName + "poing").c_str());
            // printf("%i %i %i\n", r, g, b);
            Vector3 color(r, g, b);
            ...
            if (x2percentage - x1percentage > 0.05) {
              graph->DrawSimpleText(workerThreadHistory.at(thread).at(entry).commandName, x1, y1,
                                    font, textcolor, 255);
            }
          }
        }
