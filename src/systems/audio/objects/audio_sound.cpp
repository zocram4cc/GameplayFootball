...
boost::intrusive_ptr<Interpreter> AudioSound::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_Sound) {
    boost::intrusive_ptr<AudioSound_SoundInterpreter> soundInterpreter(
        new AudioSound_SoundInterpreter(this));
    return soundInterpreter;
  }
  Log(e_FatalError, "AudioSound", "GetInterpreter",
...
void AudioSound_SoundInterpreter::OnLoad(boost::intrusive_ptr<Resource<SoundBuffer>> resource) {
  ...
  caller->audioSoundBuffer = ResourceManagerPool::GetInstance()
                                 .GetManager<AudioSoundBuffer>(e_ResourceType_AudioSoundBuffer)
...
void AudioSound_SoundInterpreter::OnPoke() {
  ...
  boost::intrusive_ptr<AudioRendererMessage_PlayAudioSoundBuffer> playMsg(
      new AudioRendererMessage_PlayAudioSoundBuffer(
...
void AudioSound_SoundInterpreter::UpdateParams() {
  AudioRenderer* renderer = caller->GetAudioScene()->GetAudioSystem()->GetAudioRenderer();
  caller->audioSoundBuffer->resourceMutex.lock();
  boost::intrusive_ptr<AudioRendererMessage_ConfigAudioSoundBuffer> updateMsg(
      new AudioRendererMessage_ConfigAudioSoundBuffer(
          caller->audioSoundBuffer->GetResource()->GetID(), caller->GetGain(), caller->GetPitch(),
          caller->GetLoop()));
  caller->audioSoundBuffer->resourceMutex.unlock();
