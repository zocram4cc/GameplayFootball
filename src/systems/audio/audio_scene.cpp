...
boost::intrusive_ptr<ISceneInterpreter> AudioScene::GetInterpreter(e_SceneType sceneType) {
  if (sceneType == e_SceneType_Scene2D) {
    boost::intrusive_ptr<AudioScene_Scene2DInterpreter> scene2DInterpreter(
        new AudioScene_Scene2DInterpreter(this));
    return scene2DInterpreter;
  }
  if (sceneType == e_SceneType_Scene3D) {
    boost::intrusive_ptr<AudioScene_Scene3DInterpreter> scene3DInterpreter(
        new AudioScene_Scene3DInterpreter(this));
    return scene3DInterpreter;
  }
  Log(e_FatalError, "AudioScene", "GetInterpreter",
...
ISystemObject* AudioScene_Scene3DInterpreter::CreateSystemObject(
...
ISystemObject* AudioScene_Scene2DInterpreter::CreateSystemObject(
    boost::intrusive_ptr<Object> object) {
  return caller->CreateSystemObject(object);
}
