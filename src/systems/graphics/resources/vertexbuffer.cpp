...
VertexBuffer::VertexBuffer()
    : vertices(0), verticesDataSize(0), vertexCount(0), dynamicBuffer(false), sizeChanged(false) {
...
VertexBuffer::~VertexBuffer() {
  ...
  if (vertexBufferID.bufferID != -1) {
    boost::intrusive_ptr<Renderer3DMessage_DeleteVertexBuffer> deleteVertexBuffer(
        new Renderer3DMessage_DeleteVertexBuffer(vertexBufferID));
...
VertexBufferID VertexBuffer::CreateOrUpdateVertexBuffer(Renderer3D* renderer3D,
  ...
  if (vertexBufferID.bufferID == -1) {
    ...
    if (dynamicBuffer)
      usage = e_VertexBufferUsage_DynamicDraw;

    assert(vertices != 0);
    boost::intrusive_ptr<Renderer3DMessage_CreateVertexBuffer> createVertexBuffer(
        new Renderer3DMessage_CreateVertexBuffer(vertices, verticesDataSize, indices, usage));
