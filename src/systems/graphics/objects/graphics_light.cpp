...
GraphicsLight_LightInterpreter::GraphicsLight_LightInterpreter(GraphicsLight* caller)
    : caller(caller) {}
...
void GraphicsLight_LightInterpreter::OnUnload() {
  ...
  // maybe a timer? if not used for x seconds, unload? would take a away the need to let camera keep
  // record of lights
  ...
  while (iter != caller->shadowMaps.end()) {
    ...
    // delete framebuffer
    boost::intrusive_ptr<Renderer3DMessage_DeleteFrameBuffer> deleteFrameBuffer(
        new Renderer3DMessage_DeleteFrameBuffer((*iter).frameBufferID, e_TargetAttachment_Depth));
    renderer3D->messageQueue.PushMessage(deleteFrameBuffer);
    deleteFrameBuffer->Wait();

    // happens automagically when noone links to texture anymore.. disabled here so we don't delete
    // the texture twice
    /*
    (*iter).texture->resourceMutex.lock();
    boost::intrusive_ptr<Renderer3DMessage_DeleteTexture> deleteTexture(new
    Renderer3DMessage_DeleteTexture((*iter).texture->GetResource()->GetID()));
...
void GraphicsLight_LightInterpreter::OnSpatialChange(const Vector3& position,
                                                     const Quaternion& rotation) {
  caller->SetPosition(position);
  // caller->SetRotation(rotation);
}

bool GLLI_SortVertexBufferQueueEntries(const VertexBufferQueueEntry& vb1,
...
void GraphicsLight_LightInterpreter::EnqueueShadowMap(
  ...
  for (int i = 0; i < (signed int)caller->shadowMaps.size(); i++) {
    if (caller->shadowMaps.at(i).cameraName ==
        camera->GetName()) {  // todo: what if cam name changes? this is hardly ideal
      index = i;
    }
  }
  ...
  if (index == -1) {
    ...
    ShadowMap map;
    map.texture = ResourceManagerPool::GetInstance()
                      .GetManager<Texture>(e_ResourceType_Texture)
                      ->Fetch(std::string(camera->GetName() + int_to_str(intptr_t(this))), false,
                              false);  // false == don't try to use loader
    if (map.texture->GetResource()->GetID() == -1) {
      Renderer3D* renderer3D = caller->GetGraphicsScene()->GetGraphicsSystem()->GetRenderer3D();
      map.texture->GetResource()->SetRenderer3D(renderer3D);
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent32,
      // e_PixelFormat_DepthComponent, 1024, 1024, false, false, false, true, true);
      map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent16,
                                                e_PixelFormat_DepthComponent, 2048, 2048, false,
                                                false, false, true, true);
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent16,
      // e_PixelFormat_DepthComponent, 4096, 4096, false, false, false, true, true); // filter on to
      // get hardware shadowmap AA in shader:
      // http://stackoverflow.com/questions/22419682/glsl-sampler2dshadow-and-shadow2d-clarification
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent32,
      // e_PixelFormat_DepthComponent, 4096, 4096, false, false, false, true, true);
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent16,
      // e_PixelFormat_DepthComponent, 8192, 8192, false, false, false, true, true);

      // create framebuffer for shadowmap
      boost::intrusive_ptr<Renderer3DMessage_CreateFrameBuffer> createFrameBuffer(
          new Renderer3DMessage_CreateFrameBuffer(e_TargetAttachment_Depth,
                                                  map.texture->GetResource()->GetID()));
  ...
  std::deque<boost::intrusive_ptr<Geometry>>::iterator visibleGeometryIter =
      visibleGeometry.begin();
  while (visibleGeometryIter != visibleGeometry.end()) {
    boost::intrusive_ptr<GraphicsGeometry_GeometryInterpreter> interpreter =
        boost::static_pointer_cast<GraphicsGeometry_GeometryInterpreter>(
            (*visibleGeometryIter)->GetInterpreter(e_SystemType_Graphics));
    ...
    std::deque<VertexBufferQueueEntry>::iterator visibleGeometryBufferIter =
        caller->shadowMaps.at(index).visibleGeometry.end();
  ...
  // std::sort(caller->shadowMaps.at(index).visibleGeometry.begin(),
  // caller->shadowMaps.at(index).visibleGeometry.end(), GLLI_SortVertexBufferQueueEntries);
  ...
  Vector3 pos = (caller->GetPosition().GetNormalized(Vector3(0, 0, -1))) +
                camera->GetDerivedPosition().Get2D() + Vector3(0, 70, 0);
  // Vector3 pos = caller->GetPosition().GetNormalized();
  Quaternion rot;
  rot = caller->GetPosition().GetNormalized(Vector3(0, 0, -1));
...
void GraphicsLight_LightInterpreter::OnPoke() {
  ...
  while (iter != caller->shadowMaps.end()) {
    boost::intrusive_ptr<Renderer3DMessage_RenderShadowMap> renderShadowMap(
        new Renderer3DMessage_RenderShadowMap(*iter));
    caller->GetGraphicsScene()->GetGraphicsSystem()->GetRenderer3D()->messageQueue.PushMessage(
        renderShadowMap);
