...
void GraphicsCamera::SetSize(float x_percent, float y_percent, float width_percent,
...
GraphicsCamera_CameraInterpreter::GraphicsCamera_CameraInterpreter(GraphicsCamera* caller)
    : caller(caller) {}

void GraphicsCamera_CameraInterpreter::OnLoad(const Properties& properties) {
  caller->SetSize(properties.GetReal("x_percent", 0), properties.GetReal("y_percent", 0),
                  properties.GetReal("width_percent", 100),
                  properties.GetReal("height_percent", 100));

  Renderer3D* renderer3D = caller->GetGraphicsScene()->GetGraphicsSystem()->GetRenderer3D();

  boost::intrusive_ptr<Renderer3DMessage_CreateView> createView(new Renderer3DMessage_CreateView(
      caller->x_percent, caller->y_percent, caller->width_percent, caller->height_percent));
...
void GraphicsCamera_CameraInterpreter::OnUnload() {
  ...
  boost::intrusive_ptr<Renderer3DMessage_DeleteView> deleteView(
      new Renderer3DMessage_DeleteView(caller->viewID));
...
void GraphicsCamera_CameraInterpreter::OnSpatialChange(const Vector3& position,
...
void GraphicsCamera_CameraInterpreter::EnqueueView(
    const std::string& camName, std::deque<boost::intrusive_ptr<Geometry>>& visibleGeometry,
  ...
  while (visibleLightIter != visibleLights.end()) {
    ...
    boost::intrusive_ptr<GraphicsLight_LightInterpreter> interpreter =
        boost::static_pointer_cast<GraphicsLight_LightInterpreter>(
            (*visibleLightIter)->GetInterpreter(e_SystemType_Graphics));
  ...
  while (skyboxIter != skyboxes.end()) {
    boost::intrusive_ptr<GraphicsGeometry_SkyboxInterpreter> interpreter =
        boost::static_pointer_cast<GraphicsGeometry_SkyboxInterpreter>(
            (*skyboxIter)->GetInterpreter(e_SystemType_Graphics));
  ...
  buffer->cameraMatrix.ConstructInverse(caller->GetPosition(), Vector3(1, 1, 1),
                                        caller->GetRotation());
  // buffer->projectionMatrix.ConstructProjection(caller->fov, aspect, caller->nearCap,
  // caller->farCap);
...
void GraphicsCamera_CameraInterpreter::OnPoke() {
  boost::intrusive_ptr<Renderer3DMessage_RenderView> renderView(
      new Renderer3DMessage_RenderView(caller->viewID, caller->viewBuffer));
