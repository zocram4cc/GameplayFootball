...
boost::intrusive_ptr<Interpreter> GraphicsGeometry::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_Geometry) {
    boost::intrusive_ptr<GraphicsGeometry_GeometryInterpreter> geometryInterpreter(
        new GraphicsGeometry_GeometryInterpreter(this));
...
GraphicsGeometry_GeometryInterpreter::GraphicsGeometry_GeometryInterpreter(GraphicsGeometry* caller)
...
void LoadMaterials(Renderer3D* renderer3D, const Material* material,
  ...
  if (material->diffuseTexture) {
    ...
    diffuseTexture = ResourceManagerPool::GetInstance()
    ...
    if (!texAlreadyThere) {
      ...
      diffuseTexture->GetResource()->CreateTexture(
          alpha ? e_InternalPixelFormat_SRGBA8 : e_InternalPixelFormat_SRGB8,
          alpha ? e_PixelFormat_RGBA : e_PixelFormat_RGB, image->w, image->h, alpha, repeat,
          mipmaps, bilinear);
  ...
  if (material->normalTexture) {
    ...
    bool texAlreadyThere = false;
    normalTexture = ResourceManagerPool::GetInstance()
                        .GetManager<Texture>(e_ResourceType_Texture)
                        ->Fetch(surface->GetIdentString(), false, texAlreadyThere,
                                true);  // false == don't try to use loader
    ...
    if (!texAlreadyThere) {
      ...
      normalTexture->GetResource()->CreateTexture(e_InternalPixelFormat_RGB8, e_PixelFormat_RGB,
                                                  image->w, image->h, false, true, true, true);
  ...
  if (material->specularTexture) {
    ...
    bool texAlreadyThere = false;
    specularTexture = ResourceManagerPool::GetInstance()
                          .GetManager<Texture>(e_ResourceType_Texture)
                          ->Fetch(surface->GetIdentString(), false, texAlreadyThere,
                                  true);  // false == don't try to use loader
    ...
    if (!texAlreadyThere) {
      ...
      specularTexture->GetResource()->CreateTexture(e_InternalPixelFormat_RGB8, e_PixelFormat_RGB,
                                                    image->w, image->h, false, true, true, true);
  ...
  if (material->illuminationTexture) {
    ...
    bool texAlreadyThere = false;
    illuminationTexture = ResourceManagerPool::GetInstance()
                              .GetManager<Texture>(e_ResourceType_Texture)
                              ->Fetch(surface->GetIdentString(), false, texAlreadyThere,
                                      true);  // false == don't try to use loader
    ...
    if (!texAlreadyThere) {
      ...
      illuminationTexture->GetResource()->CreateTexture(e_InternalPixelFormat_RGB8,
                                                        e_PixelFormat_RGB, image->w, image->h,
  ...
  if (normalTexture)
    r3dMaterial.normalTexture = normalTexture;
  if (specularTexture)
...
void GraphicsGeometry_GeometryInterpreter::OnLoad(boost::intrusive_ptr<Geometry> geometry) {
  ...
  caller->vertexBuffer = ResourceManagerPool::GetInstance()
                             .GetManager<VertexBuffer>(e_ResourceType_VertexBuffer)
                             ->Fetch(resource->GetIdentString(), false, alreadyThere,
                                     true);  // false == don't try to use loader
  ...
  for (unsigned int i = 0; i < triangleMeshes.size(); i++) {
    ...
    if (!alreadyThere) {
      ...
      for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
        memcpy(&triangleMesh[e * (tmeshsize / GetTriangleMeshElementCount()) + startIndex * 3 * 3],
      &triangleMeshes[i].triangleMesh[e * (triangleMeshes[i].triangleMeshSize /
      GetTriangleMeshElementCount())], triangleMeshes[i].triangleMeshSize /
      GetTriangleMeshElementCount() * sizeof(float));
      }
      */
      ...
      for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
        ...
        memcpy(&vertices[e * (verticesDataSize / GetTriangleMeshElementCount()) + startIndex],
               &triangleMeshes[i].vertices[e * (triangleMeshes[i].verticesDataSize /
                                                GetTriangleMeshElementCount())],
               triangleMeshes[i].verticesDataSize / GetTriangleMeshElementCount() * sizeof(float));
      }

      // indices.insert(indices.end(), triangleMeshes[i].indices.begin(),
      // triangleMeshes[i].indices.end());
...
void GraphicsGeometry_GeometryInterpreter::OnUpdateGeometry(boost::intrusive_ptr<Geometry> geometry,
  ...
  // todo: right now, you can only go from using no indices to using indices. you cannot go back to
  // not using indices anymore. fix this. todo: rewrite that indices/usesIndices thing either way,
  ...
  if (verticesDataSize == currentVerticesDataSize) {
    ...
    newFloatData = false;
    updateIndices =
        updateMaterials;  // todo: can we decouple indices and materials in this function? now we
                          // need to update indices on material update and the other way around
  } else {
    vertices = new float[verticesDataSize];
    newFloatData = true;
    updateIndices = true;
  ...
  for (unsigned int i = 0; i < triangleMeshes.size(); i++) {
    ...
    for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
      ...
      memcpy(
          ...
          &triangleMeshes[i]
               .vertices[e * (triangleMeshes[i].verticesDataSize / GetTriangleMeshElementCount())],
          triangleMeshes[i].verticesDataSize / GetTriangleMeshElementCount() * sizeof(float));
    }

    if (!usesIndices) {  // can only set indices once (todo: make OnUpdateIndices function, or
                         // something like that)
      for (unsigned int index = 0; index < triangleMeshes[i].indices.size(); index++) {
        indices.push_back(startIndex / 3 + triangleMeshes[i].indices.at(index));
        // printf("vertex index: %i + %i = %i\n", startIndex * 3,
        // triangleMeshes[i].indices.at(index), startIndex * 3 +
        // triangleMeshes[i].indices.at(index));
      }
    }
    ...
    if (updateIndices) {
      ...
      // this code makes this version only work on meshes that are the same size/materialorder as
      // their previous version
      vbIndex.material = r3dMaterial;

      if ((!usesIndices && indices.size() > 0) ||
          usesIndices) {  // include the first time using indices
      ...
      // printf("st %i VS %i\n", (indices.size() - triangleMeshes[i].indices.size()), startIndex /
...
void GraphicsGeometry_GeometryInterpreter::GetVertexBufferQueue(
  ...
  queueEntry.vertexBufferIndices.insert(queueEntry.vertexBufferIndices.end(),
                                        caller->vertexBufferIndices.begin(),
                                        caller->vertexBufferIndices.end());
...
GraphicsGeometry_SkyboxInterpreter::GraphicsGeometry_SkyboxInterpreter(GraphicsGeometry* caller)
