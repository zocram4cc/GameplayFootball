...
boost::intrusive_ptr<Interpreter> GraphicsOverlay2D::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_Image2D) {
    boost::intrusive_ptr<GraphicsOverlay2D_Image2DInterpreter> image2DInterpreter(
        new GraphicsOverlay2D_Image2DInterpreter(this));
...
void GraphicsOverlay2D_Image2DInterpreter::OnLoad(boost::intrusive_ptr<Resource<Surface>> surface) {
  ...
  if (!alreadyThere) {
    ...
    caller->texture->GetResource()->CreateTexture(e_InternalPixelFormat_RGBA8,
                                                  alpha ? e_PixelFormat_RGBA : e_PixelFormat_RGB,
                                                  image->w, image->h, alpha, false, false, false);
...
void GraphicsOverlay2D_Image2DInterpreter::OnChange(
  ...
  if (caller->size[0] != image->w || caller->size[1] != image->h) {
    ...
    caller->size[0] = image->w;
    caller->size[1] = image->h;
    caller->texture->GetResource()->ResizeTexture(image, e_InternalPixelFormat_RGBA8,
                                                  alpha ? e_PixelFormat_RGBA : e_PixelFormat_RGB,
                                                  alpha, false);
  } else {
