...
void GraphicsTask::operator()() {
  ...
  if (messageQueue.GetPending() > 0)
    Log(e_Error, "GraphicsTask", "operator()()",
        messageQueue.GetPending() + " messages left on quit!");
}
...
void GraphicsTask::GetPhase() {
  ...
  boost::intrusive_ptr<GraphicsTaskCommand_RenderImage2D> renderImage2D(
      new GraphicsTaskCommand_RenderImage2D());
  ...
  bool success;
  std::shared_ptr<IScene> scene = SceneManager::GetInstance().GetScene("scene3D", success);
  if (success) {
    std::list<boost::intrusive_ptr<Camera>> cameras;
    std::static_pointer_cast<Scene3D>(scene)->GetObjects<Camera>(e_ObjectType_Camera, cameras);
    // printf("cameras.size: %i\n", (signed int)cameras.size());

    std::vector<boost::intrusive_ptr<GraphicsTaskCommand_EnqueueView>>
        enqueueView;  // [cameras.size()]
    ...
    while (cameraIter != cameras.end()) {
      if ((*cameraIter)->IsEnabled()) {
        enqueueView.push_back(boost::intrusive_ptr<GraphicsTaskCommand_EnqueueView>(
            new GraphicsTaskCommand_EnqueueView((*cameraIter), shadowSkipFrameCounter)));
...
void GraphicsTask::ProcessPhase() {
  ...
  if (shadowSkipFrameCounter == 0) {
    boost::intrusive_ptr<GraphicsTaskCommand_RenderShadowMaps> renderShadowMaps(
        new GraphicsTaskCommand_RenderShadowMaps());
  ...
  // poke camera
  boost::intrusive_ptr<GraphicsTaskCommand_RenderCamera> renderCamera(
      new GraphicsTaskCommand_RenderCamera());
  ...
  boost::intrusive_ptr<Renderer3DMessage_RenderOverlay2D> renderOverlay2D(
      new Renderer3DMessage_RenderOverlay2D(graphicsSystem->GetOverlay2DQueue()));
...
void GraphicsTask::PutPhase() {
  ...
  // opengl thread has only one message processor, so don't wait for swap (might take long to
  // vsync). messages will be processed serially anyway todo: maybe still should be on. have to
  // check. seems that the lazy wait causes time problems: the next frame is drawn for situation
  // time = x, but since the gfx thread has to wait for the vsync, the situation drawn should be
  // that of time = x + vsync_delay. this makes for choppy gfx. update: with high enough fps, this
  // might be ok update: last frame time thing obviously doesn't work when only setting lastswaptime
  // at next frame time, so waiting here for retrace again.
  ...
  if (swapBuffers != boost::intrusive_ptr<Renderer3DMessage_SwapBuffers>()) {
    ...
    if (frameTimes_ms->size() > 100)
      frameTimes_ms->pop_front();
    frameTimes_ms.Unlock();
    lastSwapTime_ms.SetData(readyTime_ms);
  }

  shadowSkipFrameCounter++;
  if (shadowSkipFrameCounter > 1)
    shadowSkipFrameCounter = 0;
}
...
bool GraphicsTaskCommand_RenderCamera::Execute(void* caller) {
  ...
  std::shared_ptr<IScene> scene = SceneManager::GetInstance().GetScene("scene3D", success);
  if (success)
    scene->PokeObjects(e_ObjectType_Camera, e_SystemType_Graphics);
...
bool GraphicsTaskCommand_EnqueueView::Execute(void* caller) {
  ...
  if (success) {
    // test camera->SetPosition(Vector3(sin((float)EnvironmentManager::GetInstance().GetTime_ms() *
    // 0.001f) * 60, 0, 0), true);
    ...
    plane.Set(
        cameraPos,
        cameraRot * Vector3(0, (3.6f * wideScreenMultiplier) / (fov / 24.0f), -1).GetNormalized());
    bounding.push_back(plane);
    plane.Set(
        cameraPos,
        cameraRot * Vector3(0, (-3.6f * wideScreenMultiplier) / (fov / 24.0f), -1).GetNormalized());
    ...
    std::static_pointer_cast<Scene3D>(scene)->GetObjects<Light>(e_ObjectType_Light, visibleLights,
    bounding);
    //printf("lights.size: %i\n", visibleLights.size());

    std::list < boost::intrusive_ptr<Skybox> > skyboxes;
    std::static_pointer_cast<Scene3D>(scene)->GetObjects<Skybox>(e_ObjectType_Skybox, skyboxes);
    ...
    */
    ...
    std::static_pointer_cast<Scene3D>(scene)->GetObjects(visibleObjects, bounding);
    ...
    while (objectIter != visibleObjects.end()) {
      if ((*objectIter)->IsEnabled()) {
        ...
        if (objectType == e_ObjectType_Geometry)
          visibleGeometry.push_back(boost::static_pointer_cast<Geometry>(*objectIter));
...
bool GraphicsTaskCommand_RenderImage2D::Execute(void* caller) {
  bool success;
  std::shared_ptr<IScene> scene = SceneManager::GetInstance().GetScene("scene2D", success);
  if (success) {
