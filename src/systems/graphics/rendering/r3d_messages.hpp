...
public:
  Renderer3DMessage_CreateContext(int width, int height, int bpp, bool fullscreen)
      : Command("r3dmsg_CreateContext"),
        width(width),
        height(height),
        bpp(bpp),
...
public:
  Renderer3DMessage_CreateTexture(e_InternalPixelFormat internalPixelFormat,
                                  e_PixelFormat pixelFormat, int width, int height,
                                  bool alpha = false, bool repeat = true, bool mipmaps = true,
                                  bool filter = true, bool compareDepth = false)
      : Command("r3dmsg_CreateTexture"),
...
protected:
  virtual bool Execute(void* caller = nullptr) {
    textureID = static_cast<Renderer3D*>(caller)->CreateTexture(
        internalPixelFormat, pixelFormat, width, height, alpha, repeat, mipmaps, filter, false,
        compareDepth);  // false == multisample
...
public:
  Renderer3DMessage_DeleteTexture(int textureID)
      : Command("r3dmsg_DeleteTexture"), textureID(textureID) {};
...
public:
  Renderer3DMessage_ResizeTexture(int textureID, SDL_Surface* source,
                                  e_InternalPixelFormat internalPixelFormat,
                                  e_PixelFormat pixelFormat, bool alpha = false,
                                  bool mipmaps = true)
      : Command("r3dmsg_ResizeTexture"),
        textureID(textureID),
        internalPixelFormat(internalPixelFormat),
        pixelFormat(pixelFormat),
        alpha(alpha),
    ...
    // DAMN YOU SDL! this function won't actually copy right surface, just make a shallow copy
    // instead. that explains a crash i got. fuuufuuuuuu
    // this->source = SDL_CreateRGBSurfaceFrom(source->pixels, source->w, source->h, 0,
    // source->pitch, 0, 0, 0, 0);
...
protected:
  virtual bool Execute(void* caller = nullptr) {
    static_cast<Renderer3D*>(caller)->ResizeTexture(textureID, source, internalPixelFormat,
                                                    pixelFormat, alpha, mipmaps);
...
public:
  Renderer3DMessage_UpdateTexture(int textureID, SDL_Surface* source, bool alpha = false,
                                  bool mipmaps = true)
      : Command("r3dmsg_UpdateTexture"), textureID(textureID), alpha(alpha), mipmaps(mipmaps) {
    // copy image so caller doesn't have to wait for update to complete
    // DAMN YOU SDL! this function won't actually copy right surface, just make a shallow copy
    // instead. that explains a crash i got. fuuufuuuuuu
    // this->source = SDL_CreateRGBSurfaceFrom(source->pixels, source->w, source->h, 0,
...
protected:
  virtual bool Execute(void* caller = nullptr) {
    vertexBufferID = static_cast<Renderer3D*>(caller)->CreateVertexBuffer(
        vertices, verticesDataSize, indices, usage);
...
public:
  Renderer3DMessage_DeleteVertexBuffer(VertexBufferID vertexBufferID)
      : Command("r3dmsg_DeleteVertexBuffer"), vertexBufferID(vertexBufferID) {};
...
public:
  Renderer3DMessage_RenderOverlay2D(MessageQueue<Overlay2DQueueEntry>& overlay2DQueue)
      : Command("r3dmsg_RenderOverlay2D") {
    bool isMessage = true;
    while (isMessage) {
      Overlay2DQueueEntry queueEntry = overlay2DQueue.GetMessage(isMessage);
      if (isMessage)
        this->overlay2DQueue.push_back(queueEntry);
    }
  };
...
// todo: it's a bit lame that we have to wait for the OpenGL thread on the context size, even though
// we could cache this
...
public:
  Renderer3DMessage_CreateView(float x_percent, float y_percent, float width_percent,
                               float height_percent)
      : Command("r3dmsg_CreateView"),
        x_percent(x_percent),
        y_percent(y_percent),
...
protected:
  virtual bool Execute(void* caller = nullptr) {
    viewID = static_cast<Renderer3D*>(caller)->CreateView(x_percent, y_percent, width_percent,
                                                          height_percent);
...
public:
  // note 7/20/2010: made buffer param into a copy instead of a ref, so caller does not have to wait
  // to clean up the mess thought: could also leave cleaning to this class itself? but that would
  // destroy acquisition == ownership note 6/29/2011: too slow to copy, using a reference again.
  // breaks acq = ownership though :|
  Renderer3DMessage_RenderView(int viewID, ViewBuffer& buffer)
      : Command("r3dmsg_RenderView"), viewID(viewID), buffer(buffer) {};
...
public:
  Renderer3DMessage_RenderShadowMap(const ShadowMap& map)
      : Command("r3dmsg_RenderShadowMap"), map(map) {};
...
public:
  Renderer3DMessage_CreateFrameBuffer(
      e_TargetAttachment target1 = e_TargetAttachment_None, int texID1 = 0,
      ...
      e_TargetAttachment target5 = e_TargetAttachment_None, int texID5 = 0)
      : Command("r3dmsg_CreateFrameBuffer"),
        target1(target1),
        texID1(texID1),
        target2(target2),
        texID2(texID2),
...
public:
  Renderer3DMessage_DeleteFrameBuffer(int frameBufferID,
                                      ...
                                      e_TargetAttachment target5 = e_TargetAttachment_None)
      : Command("r3dmsg_DeleteFrameBuffer"),
