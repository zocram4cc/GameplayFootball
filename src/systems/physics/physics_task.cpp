...
void PhysicsTask::operator()() {
  ...
  if (messageQueue.GetPending() > 0)
    Log(e_Error, "PhysicsTask", "operator()()",
...
void PhysicsTask::ProcessPhase() {
  ...
  boost::intrusive_ptr<PhysicsTaskCommand_StepTime> stepTime(new PhysicsTaskCommand_StepTime(
      physicsSystem->GetPhysicsWrapper(), timeDiff_ms + remainder_ms, timeStep_ms));
...
void PhysicsTask::PutPhase() {
  ...
  boost::intrusive_ptr<PhysicsTaskCommand_UpdateGeometry> updateGeometry(
      new PhysicsTaskCommand_UpdateGeometry());
