...
boost::intrusive_ptr<Interpreter> PhysicsJoint::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_Joint) {
    boost::intrusive_ptr<PhysicsJoint_JointInterpreter> jointInterpreter(
        new PhysicsJoint_JointInterpreter(this));
    return jointInterpreter;
  }
  Log(e_FatalError, "PhysicsJoint", "GetInterpreter",
...
PhysicsJoint_JointInterpreter::PhysicsJoint_JointInterpreter(PhysicsJoint* caller)
    : caller(caller) {
  physics = caller->GetPhysicsScene()->GetPhysicsSystem()->GetPhysicsWrapper();
}

void PhysicsJoint_JointInterpreter::OnLoad(boost::intrusive_ptr<Joint> joint,
                                           boost::intrusive_ptr<Geometry> object1,
                                           boost::intrusive_ptr<Geometry> object2,
                                           const Vector3& anchor, const Vector3& axis1,
                                           const Vector3& axis2) {
  // find actors
  boost::intrusive_ptr<PhysicsGeometry_GeometryInterpreter> interpreter =
      boost::static_pointer_cast<PhysicsGeometry_GeometryInterpreter>(
          object1->GetInterpreter(e_SystemType_Physics));
  object1->LockSubject();
  ...
  // hmm... well probably the game programmer is responsible for not deleting his objects while
  // joint'ing them

  interpreter = boost::static_pointer_cast<PhysicsGeometry_GeometryInterpreter>(
      object2->GetInterpreter(e_SystemType_Physics));
  object2->LockSubject();
  ...
  if (jointTypeStr == "hinge")
    caller->jointType = e_JointType_Hinge;
  else if (jointTypeStr == "hinge2")
    caller->jointType = e_JointType_Hinge2;
  else
    Log(e_FatalError, "PhysicsJoint_JointInterpreter", "OnLoad",
        "JointType " + int_to_str(caller->jointType) + " not supported");

  caller->joint = physics->CreateJoint(caller->GetPhysicsScene()->worldID, caller->jointType,
                                       actor1, actor2, anchor, axis1, axis2);
}

void PhysicsJoint_JointInterpreter::OnUnload() {
  if (caller->joint)
    physics->DeleteJoint(caller->joint);
  delete caller;
  caller = 0;
}
  ...
  if (caller->joint) {
    ...
    if (subject->RequestPropertyExists("angle1_rate")) {
      subject->SetRequestProperty(
          "angle1_rate", real_to_str(physics->JointGetAngleRate(caller->joint, 1)).c_str());
    }
    if (subject->RequestPropertyExists("angle2_rate")) {
      subject->SetRequestProperty(
          "angle2_rate", real_to_str(physics->JointGetAngleRate(caller->joint, 2)).c_str());
    }
  }
}
