...
PhysicsGeometry::PhysicsGeometry(PhysicsScene* physicsScene)
    : PhysicsObject(physicsScene), actor(0), mesh(0) {}
...
boost::intrusive_ptr<Interpreter> PhysicsGeometry::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_Geometry) {
    boost::intrusive_ptr<PhysicsGeometry_GeometryInterpreter> geometryInterpreter(
        new PhysicsGeometry_GeometryInterpreter(this));
    return geometryInterpreter;
  }
  Log(e_FatalError, "PhysicsGeometry", "GetInterpreter",
...
PhysicsGeometry_GeometryInterpreter::PhysicsGeometry_GeometryInterpreter(PhysicsGeometry* caller)
    : caller(caller) {}
...
void PhysicsGeometry_GeometryInterpreter::OnLoad(boost::intrusive_ptr<Geometry> geometry) {
  ...
  if (geometry->GetProperty("physics_collision_type") == "box") {
    ...
    caller->mesh =
        physics->CreateCollisionBox(caller->GetPhysicsScene()->spaceID, Vector3(x, y, z));

    if (movable == true)
      physics->ActorSetMassBox(caller->actor, Vector3(x, y, z),
                               geometry->GetProperties().GetReal("mass"));
  }
    ...
    if (movable == true)
      physics->ActorSetMassSphere(caller->actor, radius, geometry->GetProperties().GetReal("mass"));
  }
    ...
    caller->mesh =
        physics->CreateCollisionPlane(caller->GetPhysicsScene()->spaceID, Vector3(x, y, z), d);

    // planes don't move, so no actor
    if (caller->actor)
      Log(e_FatalError, "PhysicsGeometry_GeometryInterpreter", "OnLoad",
          "Planes are unable to move (no pun intended :p)");
  }

  else if (geometry->GetProperty("physics_collision_type") == "mesh") {
    /* TODO
          std::vector < MaterializedTriangleMesh > triangleMeshes =
       resource->GetResource()->GetTriangleMeshes(); std::vector<Triangle*> triangles;
          ...
          caller->mesh = physics->CreateCollisionTriMesh(caller->GetPhysicsScene()->spaceID,
       vertices, triangles.size() * 3);

          delete [] vertices;

          //if (movable == true) physics->ActorSetMassTriMesh(caller->actor, radius,
       geometry->GetProperties().GetReal("weight"));
    */
  }

  else {
    Log(e_FatalError, "PhysicsGeometry_GeometryInterpreter", "OnLoad",
        "You need to set the physics_collision_type in object '" + geometry->GetName() +
            "'s properties");
  }
...
void PhysicsGeometry_GeometryInterpreter::OnUnload() {
  ...
  if (caller->mesh)
    physics->DeleteCollisionMesh(caller->mesh);
  if (caller->actor)
    physics->DeleteActor(caller->actor);
  delete caller;
  caller = 0;
}
...
void PhysicsGeometry_GeometryInterpreter::OnMove(const Vector3& position) {
  ...
  if (caller->movable == true) {
    if (caller->actor)
      physics->ActorSetPosition(caller->actor, position);
  } else {
    if (caller->mesh)
      physics->CollisionMeshSetPosition(caller->mesh, position);
  }
}
...
void PhysicsGeometry_GeometryInterpreter::OnRotate(const Quaternion& rotation) {
  ...
  if (caller->movable == true) {
    ...
    if (caller->mesh)
      physics->CollisionMeshSetRotation(caller->mesh, rotation);
  }
}
...
void PhysicsGeometry_GeometryInterpreter::OnPoke() {
  ...
  if (caller->actor) {
    ...
    // todo: locking the physics engine by querying it might have to block on the physics' steptime
    // update so cache the positions/rotations after steptime edit: maybe they are serial anyway, so
    ...
    subject->SetPosition(
        pos, false);  // false == don't update spatialdata to system objects; would cause a deadlock
    ...
    if (subject->RequestPropertyExists("velocity")) {
      subject->SetRequestProperty("velocity",
                                  real_to_str(physics->ActorGetVelocity(caller->actor)).c_str());
    }
    if (subject->RequestPropertyExists("movement")) {
      Vector3 movement = physics->ActorGetMovement(caller->actor);
      subject->SetRequestProperty(
          "movement", (real_to_str(movement.coords[0]) + "," + real_to_str(movement.coords[1]) +
                       "," + real_to_str(movement.coords[2]))
                          .c_str());
    }
  }
}

void PhysicsGeometry_GeometryInterpreter::ApplyForceAtRelativePosition(float force,
