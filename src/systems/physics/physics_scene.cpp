...
ISystemObject* PhysicsScene::CreateSystemObject(boost::intrusive_ptr<Object> object) {
  ...
  if (object->GetObjectType() == e_ObjectType_Geometry) {
    if (boost::static_pointer_cast<Geometry>(object)->GetProperties().GetBool("physicable") ==
        true) {
...
boost::intrusive_ptr<ISceneInterpreter> PhysicsScene::GetInterpreter(e_SceneType sceneType) {
  if (sceneType == e_SceneType_Scene3D) {
    boost::intrusive_ptr<PhysicsScene_Scene3DInterpreter> scene3DInterpreter(
        new PhysicsScene_Scene3DInterpreter(this));
    return scene3DInterpreter;
  }
  Log(e_FatalError, "PhysicsScene", "GetInterpreter",
...
PhysicsScene_Scene3DInterpreter::PhysicsScene_Scene3DInterpreter(PhysicsScene* caller)
    : caller(caller) {}
...
ISystemObject* PhysicsScene_Scene3DInterpreter::CreateSystemObject(
    boost::intrusive_ptr<Object> object) {
