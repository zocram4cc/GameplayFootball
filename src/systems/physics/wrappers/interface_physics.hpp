...
public:
  ...
  virtual void ActorApplyForceAtRelativePosition(IPhysicsActor* actor, float force,
                                                 const Vector3& direction,
                                                 const Vector3& position) = 0;

  // collision functions
  virtual IPhysicsCollisionMesh* CreateCollisionPlane(int spaceID, const Vector3& normal,
                                                      float d) = 0;
  ...
  virtual IPhysicsCollisionMesh* CreateCollisionTriMesh(int spaceID, const float* vertices,
                                                        int vertexCount) = 0;
  ...
  // joint functions
  virtual IPhysicsJoint* CreateJoint(int worldID, e_JointType jointType, IPhysicsActor* actor1,
                                     IPhysicsActor* actor2, const Vector3& anchor,
                                     const Vector3& axis1, const Vector3& axis2) = 0;
  virtual void DeleteJoint(IPhysicsJoint* joint) = 0;

  virtual void JointSetStops(IPhysicsJoint* joint, radian lowStop, radian highStop,
                             int paramNum = 1) = 0;
  virtual void JointSetVelocity(IPhysicsJoint* joint, float velocity,
                                int paramNum = 1) = 0;  // desired velocity
  virtual void JointSetMaxForce(
      IPhysicsJoint* joint, float force,
      int paramNum = 1) = 0;  // maximum force to reach the desired velocity
  virtual void JointSetConstraintForceMixing(IPhysicsJoint* joint, float value,
                                             int paramNum = 1) = 0;
  virtual void JointSetErrorCorrection(IPhysicsJoint* joint, float value, int paramNum = 1) = 0;
  virtual void JointSetSuspensionConstraintForceMixing(IPhysicsJoint* joint,
                                                       float value) = 0;  // only on hinge2 joints
  virtual void JointSetSuspensionErrorReduction(IPhysicsJoint* joint,
                                                float value) = 0;  // only on hinge2 joints
