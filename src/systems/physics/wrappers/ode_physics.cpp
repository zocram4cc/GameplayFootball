...
static void OdePhysicsNearCallback(void* data, dGeomID o1, dGeomID o2) {
  if (dGeomIsSpace(o1) || dGeomIsSpace(o2)) {
    ...
    if (dGeomIsSpace(o1))
      dSpaceCollide((dSpaceID)o1, data, &OdePhysicsNearCallback);
    if (dGeomIsSpace(o2))
      dSpaceCollide((dSpaceID)o2, data, &OdePhysicsNearCallback);
    ...
    if (bodies[0] != nullptr || bodies[1] != nullptr) {
      ...
      if (bodies[0] && bodies[1] && dAreConnectedExcluding(bodies[0], bodies[1], dJointTypeContact))
        return;
      if (bodies[0] && bodies[1] && dAreConnected(bodies[0], bodies[1]))
        return;
      ...
      // when larger, collisions near seams between trimesh triangles causes 2 contacts (with both
      // triangles) to be created. this causes bumpy movement on seams. max 1 contact probably won't
      // work with non-convex objects, so they are not supported (yet)
      int MAX_CONTACTS = 12;
      ...
      while (possibleNeighbours || firstTime) {
        ...
        while (subjectIter != contacts.end()) {
          ...
          Vector3 pos((*subjectIter).geom.pos[0], (*subjectIter).geom.pos[1],
                      (*subjectIter).geom.pos[2]);
          Vector3 normal((*subjectIter).geom.normal[0], (*subjectIter).geom.normal[1],
                         (*subjectIter).geom.normal[2]);

          std::vector<dContact>::iterator checkIter = subjectIter + 1;
          while (checkIter != contacts.end()) {
            Vector3 pos2((*checkIter).geom.pos[0], (*checkIter).geom.pos[1],
                         (*checkIter).geom.pos[2]);
            Vector3 normal2((*checkIter).geom.normal[0], (*checkIter).geom.normal[1],
                            (*checkIter).geom.normal[2]);
            ...
            if (pos.GetDistance(pos2) < mergeThreshold) {
              ...
              if (newNormal.GetLength() == 0) {
                newNormal.Set(0, 0, 1);
                Log(e_Warning, "ode_physics.cpp", "OdePhysicsNearCallback",
                    "Unable to normalize Vector3");
              }
              newNormal.Normalize();
      ...
      while (subjectIter != contacts.end()) {
        (*subjectIter).surface.mode =
            dContactMu2 | dContactSoftCFM | dContactSoftERP | dContactApprox1;
        (*subjectIter).surface.mu =
            properties[0]->GetReal("friction_1", 1) * properties[1]->GetReal("friction_1", 1);
        (*subjectIter).surface.mu2 =
            properties[0]->GetReal("friction_2", 1) * properties[1]->GetReal("friction_2", 1);
        ...
        if (properties[dynamicBody]->Exists("bounce")) {
          ...
          (*subjectIter).surface.bounce_vel =
              properties[dynamicBody]->GetReal("bounce_minimum_velocity", 0.1);
        }
        ...
        if (properties[dynamicBody]->Exists("friction_direction_1")) {
          ...
          */
          ...
          if (fdir1.GetLength() == 0) {
            Log(e_Warning, "ode_physics", "OdePhysicsCollisionCallback",
                "Unable to normalize fdir1");
            fdir1.Set(1, 0, 0);
          }
          fdir1.Normalize();
        ...
        dJointID c =
            dJointCreateContact(static_cast<dWorldID>(data), collisionGroup, &*subjectIter);
        dJointAttach(c, bodies[0], bodies[1]);
        // printf("%f %f %f\n", contact[i].geom.normal[0], contact[i].geom.normal[1],
...
int OdePhysics::CreateWorld() {
  ...
  std::pair<std::map<int, dWorldID>::iterator, bool> result =
      worldMap.insert(std::map<int, dWorldID>::value_type(entry, worldID));
...
int OdePhysics::CreateSpace() {
  ...
  std::pair<std::map<int, dSpaceID>::iterator, bool> result =
      spaceMap.insert(std::map<int, dSpaceID>::value_type(entry, spaceID));
  if (result.second == false) {
...
void OdePhysics::ActorSetMassPosition(IPhysicsActor* actor, const Vector3& pos) {
  ...
  dBodySetPosition(static_cast<OdePhysicsActor*>(actor)->id, bla[0] + pos.coords[0],
                   bla[1] + pos.coords[1], bla[2] + pos.coords[2]);
...
void OdePhysics::ActorSetPosition(IPhysicsActor* actor, const Vector3& pos) {
  mutex.lock();
  dBodySetPosition(static_cast<OdePhysicsActor*>(actor)->id, pos.coords[0], pos.coords[1],
                   pos.coords[2]);
  mutex.unlock();
}
...
void OdePhysics::ActorApplyForceAtRelativePosition(IPhysicsActor* actor, float force,
                                                   const Vector3& direction,
                                                   const Vector3& position) {
  mutex.lock();
  dBodyAddForceAtRelPos(static_cast<OdePhysicsActor*>(actor)->id, direction.coords[0] * force,
                        direction.coords[1] * force, direction.coords[2] * force,
...
IPhysicsCollisionMesh* OdePhysics::CreateCollisionPlane(int spaceID, const Vector3& normal,
                                                        float d) {
  mutex.lock();

  dGeomID id =
      dCreatePlane(GetOdeSpaceID(spaceID), normal.coords[0], normal.coords[1], normal.coords[2], d);
...
IPhysicsCollisionMesh* OdePhysics::CreateCollisionBox(int spaceID, const Vector3& sides) {
  ...
  dGeomID id =
      dCreateBox(GetOdeSpaceID(spaceID), sides.coords[0], sides.coords[1], sides.coords[2]);
...
IPhysicsCollisionMesh* OdePhysics::CreateCollisionTriMesh(int spaceID, const float* vertices,
                                                          int vertexCount) {
  mutex.lock();
  ...
  dGeomTriMeshDataBuildSingle(trimesh_id, dvertices, sizeof(dReal) * 3, vertexCount, indices,
                              vertexCount, sizeof(unsigned int) * 3);
  ...
  for (int i = 0; i < vertexCount; i++) {
    ...
    std::vector<Vector3>::iterator found = std::find(uniqueVertices.begin(), uniqueVertices.end(),
  pos); if (found == uniqueVertices.end()) { uniqueVertices.push_back(pos); indices[i] =
  uniqueVertices.size() - 1; } else {
...
void OdePhysics::CollisionMeshSetActor(IPhysicsCollisionMesh* mesh, IPhysicsActor* actor) {
  mutex.lock();
  dGeomSetBody(static_cast<OdePhysicsCollisionMesh*>(mesh)->id,
               static_cast<OdePhysicsActor*>(actor)->id);
  mutex.unlock();
}

void OdePhysics::CollisionMeshSetPosition(IPhysicsCollisionMesh* mesh, const Vector3& pos) {
  mutex.lock();
  dGeomSetPosition(static_cast<OdePhysicsCollisionMesh*>(mesh)->id, pos.coords[0], pos.coords[1],
                   pos.coords[2]);
  mutex.unlock();
}
...
IPhysicsJoint* OdePhysics::CreateJoint(int worldID, e_JointType jointType, IPhysicsActor* actor1,
                                       IPhysicsActor* actor2, const Vector3& anchor,
  ...
  switch (jointType) {
    case e_JointType_Hinge:
      ...
      dJointAttach(joint->id, static_cast<OdePhysicsActor*>(actor1)->id,
                   static_cast<OdePhysicsActor*>(actor2)->id);
    ...
    default:
      Log(e_FatalError, "OdePhysics", "CreateJoint",
          "Joint type " + int_to_str(jointType) + " not yet supported ;)");
      break;
  }
...
void OdePhysics::JointSetStops(IPhysicsJoint* joint, radian lowStop, radian highStop,
                               int paramNum) {
  mutex.lock();
  // hack for ode 'flaw':
  // http://opende.sourceforge.net/wiki/index.php/FAQ#My_wheels_are_sloppy.3B_they_don.27t_stay_straight_when_I_turn_them.
...
void OdePhysics::JointSetVelocity(IPhysicsJoint* joint, float velocity,
                                  int paramNum) {  // desired velocity
  mutex.lock();
  JointSetParameter(joint, dParamVel, velocity, paramNum);
  mutex.unlock();
}

void OdePhysics::JointSetMaxForce(IPhysicsJoint* joint, float force,
                                  int paramNum) {  // maximum force to reach the desired velocity
  mutex.lock();
...
void OdePhysics::JointSetSuspensionConstraintForceMixing(IPhysicsJoint* joint,
                                                         float value) {  // only on hinge2 joints
  mutex.lock();
  JointSetParameter(joint, dParamSuspensionCFM, value);
  mutex.unlock();
}

void OdePhysics::JointSetSuspensionErrorReduction(IPhysicsJoint* joint,
                                                  float value) {  // only on hinge2 joints
  mutex.lock();
...
float OdePhysics::JointGetAngleRate(IPhysicsJoint* joint, int paramNum) {
  ...
  switch (static_cast<OdePhysicsJoint*>(joint)->jointType) {
    ...
    case e_JointType_Hinge2:
      if (paramNum == 1)
        value = dJointGetHinge2Angle1Rate(id);
      if (paramNum == 2)
        value = dJointGetHinge2Angle2Rate(id);
      break;
...
dSpaceID OdePhysics::GetOdeSpaceID(int spaceID) {
  if (spaceMap.find(spaceID) == spaceMap.end())
    Log(e_FatalError, "OdePhysics", "GetOdeSpaceID", "spaceID non-existant");
  return spaceMap.find(spaceID)->second;
}

dWorldID OdePhysics::GetOdeWorldID(int worldID) {
  if (worldMap.find(worldID) == worldMap.end())
    Log(e_FatalError, "OdePhysics", "GetOdeWorldID", "worldID non-existant");
