...
public:
  ...
  boost::intrusive_ptr<Resource<T>> Fetch(const std::string& filename, bool load = true,
                                          bool useExisting = true) {
    bool foo;
    return Fetch(filename, load, foo, useExisting);
  }

  boost::intrusive_ptr<Resource<T>> Fetch(const std::string& filename, bool load,
                                          bool& alreadyThere, bool useExisting) {
  ...
  boost::intrusive_ptr<Resource<T>> FetchCopy(const std::string& filename,
  ...
  boost::intrusive_ptr<Resource<T>> FetchCopy(const std::string& filename,
                                              const std::string& newName, bool& alreadyThere) {
    boost::intrusive_ptr<Resource<T>> resourceCopy;
    if (resources.data.find(newName) != resources.data.end()) {
      // Log(e_Warning, "ResourceManager", "FetchCopy", "Duplicate key '" + newName + "' - returning
      // existing resource instead of copy (maybe just use Fetch() instead?)");
  ...
  void RemoveUnused() {
    ...
    typename std::map<std::string, boost::intrusive_ptr<Resource<T>>>::iterator resIter =
        resources.data.begin();
    while (resIter != resources.data.end()) {
      if (resIter->second->GetRefCount() == 1) {
        // printf("removing unused %s resource '%s'\n", typeDescription.c_str(),
        // resIter->second->GetIdentString().c_str());
...
protected:
  boost::intrusive_ptr<Resource<T>> Find(const std::string& identString, bool& success) {
    resources.Lock();
    typename std::map<std::string, boost::intrusive_ptr<Resource<T>>>::iterator resIter =
        resources.data.find(identString);
  ...
  void Register(boost::intrusive_ptr<Resource<T>> resource) {
    ...
    if (resources.data.find(resource->GetIdentString()) != resources.data.end()) {
      ...
      resources.Lock();
      if (resources.data.find(resource->GetIdentString()) != resources.data.end()) {
        Log(e_FatalError, "ResourceManager", "Register",
            "Duplicate key '" + resource->GetIdentString() + "'");
      }
    }
