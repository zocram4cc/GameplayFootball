...
template <>
ObjectFactory* Singleton<ObjectFactory>::singleton = 0;
...
void ObjectFactory::Exit() {
  std::map<e_ObjectType, boost::intrusive_ptr<Object>>::iterator prototype_iter =
      prototypes.begin();
...
boost::intrusive_ptr<Object> ObjectFactory::CreateObject(const std::string& name,
                                                         std::string objectTypeStr,
                                                         std::string postfix) {
  e_ObjectType objectType = e_ObjectType_Geometry;
  if (objectTypeStr == "Camera")
    objectType = e_ObjectType_Camera;
  else if (objectTypeStr == "Image2D")
    objectType = e_ObjectType_Image2D;
  else if (objectTypeStr == "Geometry")
    objectType = e_ObjectType_Geometry;
  else if (objectTypeStr == "Skybox")
    objectType = e_ObjectType_Skybox;
  else if (objectTypeStr == "Light")
    objectType = e_ObjectType_Light;
  else if (objectTypeStr == "Joint")
    objectType = e_ObjectType_Joint;
  else if (objectTypeStr == "AudioReceiver")
    objectType = e_ObjectType_AudioReceiver;
  else if (objectTypeStr == "Sound")
    objectType = e_ObjectType_Sound;
...
boost::intrusive_ptr<Object> ObjectFactory::CreateObject(const std::string& name,
                                                         ...
                                                         std::string postfix) {
  std::map<e_ObjectType, boost::intrusive_ptr<Object>>::iterator prototype_iter =
      prototypes.find(objectType);
  if (prototype_iter != prototypes.end()) {
    boost::intrusive_ptr<Object> bla = CopyObject((*prototype_iter).second, postfix);
    bla->SetName(name);
    return bla;
  } else {
    Log(e_FatalError, "ObjectFactory", "CreateObject",
        "Object type " + int_to_str(objectType) + " not found in prototype registry");
    return 0;
  }
}

boost::intrusive_ptr<Object> ObjectFactory::CopyObject(boost::intrusive_ptr<Object> source,
                                                       std::string postfix) {
...
void ObjectFactory::RegisterPrototype(e_ObjectType objectType,
                                      boost::intrusive_ptr<Object> prototype) {
