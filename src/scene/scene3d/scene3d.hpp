...
public:
  ...
  void GetObjects(std::list<boost::intrusive_ptr<Object>>& gatherObjects,
                  const vector_Planes& bounding) const {
  ...
  void GetObjects(e_ObjectType targetObjectType,
                  std::list<boost::intrusive_ptr<T>>& gatherObjects) const {
    if (!SupportedObjectType(targetObjectType)) {
      Log(e_Error, "Scene3D", "GetObjects",
          "targetObjectType " + int_to_str(targetObjectType) + " is not supported by this scene");
      return;
    }
  ...
  template <class T>
  void GetObjects(e_ObjectType targetObjectType, std::list<boost::intrusive_ptr<T>>& gatherObjects,
                  const vector_Planes& bounding) const {
    if (!SupportedObjectType(targetObjectType)) {
      Log(e_Error, "Scene3D", "GetObjects",
          "targetObjectType " + int_to_str(targetObjectType) + " is not supported by this scene");
  ...
  template <class T>
  void GetObjects(e_ObjectType targetObjectType,
                  std::deque<boost::intrusive_ptr<T>>& gatherObjects) const {
    if (!SupportedObjectType(targetObjectType)) {
      Log(e_Error, "Scene3D", "GetObjects",
          "targetObjectType " + int_to_str(targetObjectType) + " is not supported by this scene");
      return;
  ...
  template <class T>
  void GetObjects(e_ObjectType targetObjectType, std::deque<boost::intrusive_ptr<T>>& gatherObjects,
                  const vector_Planes& bounding) const {
    if (!SupportedObjectType(targetObjectType)) {
      Log(e_Error, "Scene3D", "GetObjects",
          "targetObjectType " + int_to_str(targetObjectType) + " is not supported by this scene");
