...
public:
  ...
  void GetSpatials(std::list<boost::intrusive_ptr<Spatial>>& gatherSpatials, bool recurse = true,
                   int depth = 0) const;

  void GetObjects(std::list<boost::intrusive_ptr<Object>>& gatherObjects, bool recurse = true,
                  int depth = 0) const;
  void GetObjects(std::list<boost::intrusive_ptr<Object>>& gatherObjects,
                  const vector_Planes& bounding, bool recurse = true, int depth = 0) const;

  void GetObjects(std::deque<boost::intrusive_ptr<Object>>& gatherObjects,
                  const vector_Planes& bounding, bool recurse = true, int depth = 0) const;

  template <class T>
  inline void GetObjects(e_ObjectType targetObjectType,
  ...
  template <class T>
  inline void GetObjects(e_ObjectType targetObjectType,
                         std::list<boost::intrusive_ptr<T>>& gatherObjects,
                         const vector_Planes& bounding, bool recurse = true, int depth = 0) const {
    objects.Lock();
    int objectsSize = objects.data.size();
    for (int i = 0; i < objectsSize; i++) {
    ...
    if (recurse) {
      ...
      for (int i = 0; i < nodesSize; i++) {
        if (nodes.data.at(i)->GetAABB().Intersects(bounding))
          nodes.data.at(i)->GetObjects<T>(targetObjectType, gatherObjects, bounding, recurse,
                                          depth + 1);
      }
      nodes.Unlock();
    }
  ...
  template <class T>
  inline void GetObjects(e_ObjectType targetObjectType,
                         std::deque<boost::intrusive_ptr<T>>& gatherObjects, bool recurse = true,
                         int depth = 0) const {
    objects.Lock();
    int objectsSize = objects.data.size();
    for (int i = 0; i < objectsSize; i++) {
  ...
  template <class T>
  inline void GetObjects(e_ObjectType targetObjectType,
                         std::deque<boost::intrusive_ptr<T>>& gatherObjects,
                         const vector_Planes& bounding, bool recurse = true, int depth = 0) const {
    objects.Lock();
    int objectsSize = objects.data.size();
    for (int i = 0; i < objectsSize; i++) {
