...
Node::Node(const Node& source, const std::string& postfix, std::shared_ptr<Scene3D> scene3D)
    : Spatial(source) {
  ...
  while (objectIter != gatherObjects.end()) {
    boost::intrusive_ptr<Object> objCopy =
        ObjectFactory::GetInstance().CopyObject((*objectIter), postfix);
...
void Node::DeleteNode(boost::intrusive_ptr<Node> node) {
  ...
  std::vector<boost::intrusive_ptr<Node>>::iterator nodeIter =
      find(nodes.data.begin(), nodes.data.end(), node);
...
void Node::GetNodes(std::vector<boost::intrusive_ptr<Node>>& gatherNodes, bool recurse) const {
  ...
  for (int i = 0; i < nodesSize; i++) {
    ...
    if (recurse)
      nodes.data.at(i)->GetNodes(gatherNodes, recurse);
  }
  nodes.Unlock();
}
...
void Node::DeleteObject(boost::intrusive_ptr<Object> object, bool exitObject) {
  ...
  std::vector<boost::intrusive_ptr<Object>>::iterator objIter =
      find(objects.data.begin(), objects.data.end(), object);
  if (objIter != objects.data.end()) {
    ...
    Log(e_Error, "Node", "DeleteObject",
        "Object " + object->GetName() + " not found among node " + GetName() + "'s children!");
  objects.Unlock();
...
void Node::GetSpatials(std::list<boost::intrusive_ptr<Spatial>>& gatherSpatials, bool recurse,
                       int depth) const {
  objects.Lock();
...
void Node::GetObjects(std::list<boost::intrusive_ptr<Object>>& gatherObjects, bool recurse,
                      int depth) const {
  objects.Lock();
  int objectsSize = objects.data.size();
  for (int i = 0; i < objectsSize; i++) {
...
void Node::GetObjects(std::list<boost::intrusive_ptr<Object>>& gatherObjects,
                      const vector_Planes& bounding, bool recurse, int depth) const {
  objects.Lock();
  int objectsSize = objects.data.size();
  for (int i = 0; i < objectsSize; i++) {
    if (objects.data.at(i)->GetAABB().Intersects(bounding))
      gatherObjects.push_back(objects.data.at(i));
  }
  objects.Unlock();
  ...
  if (recurse) {
    ...
    int nodesSize = nodes.data.size();
    for (int i = 0; i < nodesSize; i++) {
      if (nodes.data.at(i)->GetAABB().Intersects(bounding))
        nodes.data.at(i)->GetObjects(gatherObjects, bounding, recurse, depth + 1);
    }
    nodes.Unlock();
  }
}

void Node::GetObjects(std::deque<boost::intrusive_ptr<Object>>& gatherObjects,
                      const vector_Planes& bounding, bool recurse, int depth) const {
  objects.Lock();
  int objectsSize = objects.data.size();
  for (int i = 0; i < objectsSize; i++) {
...
void Node::PokeObjects(e_ObjectType targetObjectType, e_SystemType targetSystemType) {
  ...
  int objectsSize = objects.data.size();
  for (int i = 0; i < objectsSize; i++) {
    if (objects.data.at(i)->IsEnabled())
      if (objects.data.at(i)->GetObjectType() == targetObjectType)
        objects.data.at(i)->Poke(targetSystemType);
  }
  objects.Unlock();
...
void Node::RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
  ...
  int nodesSize = nodes.data.size();
  for (int i = 0; i < nodesSize; i++) {
    // todo: not for localmode == absolute? doesn't seem to work, but why not?! commented out for
    // now (maybe because it won't validate spatialdata/bounding volume of parents of these?)
...
void Node::PrintTree(int recursionDepth) {
  ...
  int nodesSize = nodes.data.size();
  for (int i = 0; i < nodesSize; i++) {
    for (int space = 0; space < recursionDepth; space++)
      printf("|     ");
