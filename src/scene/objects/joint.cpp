...
void Joint::Connect(boost::intrusive_ptr<Geometry> object1, boost::intrusive_ptr<Geometry> object2,
                    const Vector3& anchor, const Vector3& axis1, const Vector3& axis2) {
  subjectMutex.lock();
...
void Joint::Poke(e_SystemType targetSystemType) {
  ...
  for (int i = 0; i < observersSize; i++) {
    IJointInterpreter* jointInterpreter = static_cast<IJointInterpreter*>(observers.at(i).get());
    if (jointInterpreter->GetSystemType() == targetSystemType)
      jointInterpreter->OnPoke();
  }

  subjectMutex.unlock();
}

void Joint::RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
  ...
  /*
      ...
      int observersSize = observers.size();
      for (int i = 0; i < observersSize; i++) {
        IJointInterpreter *jointInterpreter =
     static_cast<IJointInterpreter*>(observers.at(i).get());
