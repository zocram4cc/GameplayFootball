...
Geometry::Geometry(const Geometry& src, const std::string& postfix) : Object(src) {
  if (src.geometryData != boost::intrusive_ptr<Resource<GeometryData>>()) {
    ...
    geometryData = (ResourceManagerPool::GetInstance()
                        .GetManager<GeometryData>(e_ResourceType_GeometryData)
                        ->FetchCopy(srcName, srcName + postfix, alreadyThere));
    // geometryData =
    // (ResourceManagerPool::GetInstance().GetManager<GeometryData>(e_ResourceType_GeometryData)->Fetch(srcName,
...
void Geometry::Exit() {  // ATOMIC
  ...
  for (int i = 0; i < observersSize; i++) {
    IGeometryInterpreter* geometryInterpreter =
        static_cast<IGeometryInterpreter*>(observers.at(i).get());
...
void Geometry::Poke(e_SystemType targetSystemType) {
  ...
  // todo: does this work when multiple systems want to exclude themselves? i guess not.. make it
  // (excludes) an array?
  if (updateSpatialDataAfterPoke.data.haveTo == true) {
    RecursiveUpdateSpatialData(e_SpatialDataType_Both,
                               updateSpatialDataAfterPoke.data.excludeSystem);
...
void Geometry::ApplyForceAtRelativePosition(float force, const Vector3& direction,
