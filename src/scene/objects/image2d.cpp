...
void Image2D::Exit() {  // ATOMIC
  ...
  for (int i = 0; i < observersSize; i++) {
    IImage2DInterpreter* image2DInterpreter =
        static_cast<IImage2DInterpreter*>(observers.at(i).get());
...
boost::intrusive_ptr<Resource<Surface>> Image2D::GetImage() {
  // todo: this mutex lock makes little sense or so it seems. will have to force users to lock from
...
void Image2D::PutPixel(int x, int y, const Vector3& color, int alpha) {
  ...
  if (SDL_ISPIXELFORMAT_ALPHA(surface->format->format))
    color32 = SDL_MapRGBA(surface->format, int(floor(color.coords[0])), int(floor(color.coords[1])),
                          int(floor(color.coords[2])), alpha);
  else
    color32 = SDL_MapRGB(surface->format, int(floor(color.coords[0])), int(floor(color.coords[1])),
                         int(floor(color.coords[2])));
    ...
    for (int y = 0; y < surface->h; y++) {
      ...
      for (int xsub = -radius; xsub < radius + 1; xsub++) {
        for (int ysub = -radius; ysub < radius + 1; ysub++) {
          if (x + xsub >= 0 && x + xsub < surface->w && y + ysub >= 0 && y + ysub < surface->h) {
      ...
      Uint32 color32;
      if (SDL_ISPIXELFORMAT_ALPHA(surface->format->format))
        color32 = SDL_MapRGBA(surface->format, int(floor(color.coords[0])),
                              int(floor(color.coords[1])), int(floor(color.coords[2])), 255);
      else
        color32 = SDL_MapRGB(surface->format, int(floor(color.coords[0])),
                             int(floor(color.coords[1])), int(floor(color.coords[2])));
...
void Image2D::DrawLine(const Line& line, const Vector3& color, int alpha) {
  ...
  sdl_line(surface, line.GetVertex(0).coords[0], line.GetVertex(0).coords[1],
           line.GetVertex(1).coords[0], line.GetVertex(1).coords[1], color32);
...
void Image2D::DrawTriangle(const Triangle& triangle, const Vector3& color, int alpha) {
  ...
  sdl_triangle_filled(surface, triangle, int(floor(color.coords[0])), int(floor(color.coords[1])),
                      int(floor(color.coords[2])));
...
void Image2D::DrawSimpleText(const std::string& caption, int x, int y, TTF_Font* font,
                             const Vector3& color,
                             int alpha) {  // todo: alpha doesn't work properly yet
...
void Image2D::Poke(e_SystemType targetSystemType) {
  ...
  for (int i = 0; i < observersSize; i++) {
    IImage2DInterpreter* image2DInterpreter =
        static_cast<IImage2DInterpreter*>(observers.at(i).get());
    if (image2DInterpreter->GetSystemType() == targetSystemType)
      image2DInterpreter->OnPoke();
  }
