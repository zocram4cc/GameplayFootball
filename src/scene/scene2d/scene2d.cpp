...
Scene2D::Scene2D(const std::string& name, const Properties& config)
    : Scene(name, e_SceneType_Scene2D) {
...
void Scene2D::Exit() {  // ATOMIC
  ...
  for (int i = 0; i < observersSize; i++) {
    IScene2DInterpreter* scene2DInterpreter =
        static_cast<IScene2DInterpreter*>(observers.at(i).get());
...
void Scene2D::GetObjects(e_ObjectType targetObjectType,
