...
Object::~Object() {
  if (observers.size() != 0) {
    Log(e_FatalError, "Object", "~Object",
        "Observer(s) still present at destruction time (spatial named: " + GetName() + ")");
  }
}
...
void Object::Synchronize() {
  ...
  for (int i = 0; i < observersSize; i++) {
    boost::intrusive_ptr<Interpreter> interpreter =
        boost::static_pointer_cast<Interpreter>(observers.at(i));
...
inline void Object::RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
...
boost::intrusive_ptr<Interpreter> Object::GetInterpreter(e_SystemType targetSystemType) {
  ...
  for (int i = 0; i < observersSize; i++) {
    boost::intrusive_ptr<Interpreter> interpreter =
        boost::static_pointer_cast<Interpreter>(observers.at(i));
    if (interpreter->GetSystemType() == targetSystemType)
      result = interpreter;
  }
  subjectMutex.unlock();
