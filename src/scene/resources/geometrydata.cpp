...
GeometryData::GeometryData(const GeometryData& src) {
  for (unsigned int i = 0; i < src.triangleMeshes.size(); i++) {
    ...
    memcpy(mesh.vertices, src.triangleMeshes.at(i).vertices,
           src.triangleMeshes.at(i).verticesDataSize * sizeof(float));
...
bool SortTriangleMeshes(const MaterializedTriangleMesh& tm1, const MaterializedTriangleMesh& tm2) {
  return tm1.material.diffuseTexture->GetIdentString() <
         tm2.material.diffuseTexture->GetIdentString();
}
...
AABB GeometryData::GetAABB() const {
  if (aabb.dirty) {
    ...
    for (int i = 0; i < (signed int)triangleMeshes.size(); i++) {
      aabb.aabb +=
          GetTriangleMeshAABB(triangleMeshes.at(i).vertices, triangleMeshes.at(i).verticesDataSize,
                              triangleMeshes.at(i).indices);
    }
    aabb.dirty = false;
  }
  return aabb.aabb;
