...
TaskSequenceEntryLockThread::TaskSequenceEntryLockThread(std::mutex& sequenceLock)
...
TaskSequenceEntry_Lock::TaskSequenceEntry_Lock(std::mutex& sequenceLock)
    : sequenceLock(sequenceLock) {
...
bool TaskSequenceEntry_Lock::Execute() {
  lockThread = std::thread(
      std::ref(*lockThreadObject));  // do not make copy; the contained Lockable is not copyable
  return true;
}
...
TaskSequenceEntry_Unlock::TaskSequenceEntry_Unlock(std::mutex& sequenceLock)
    : sequenceLock(sequenceLock) {}
...
TaskSequence::TaskSequence(const std::string& name, int sequenceTime_ms, bool skipOnTooLate)
    : name(name), sequenceTime_ms(sequenceTime_ms), skipOnTooLate(skipOnTooLate) {}
...
void TaskSequence::AddSystemTaskEntry(ISystem* system, e_TaskPhase taskPhase) {
  ...
  switch (taskPhase) {
    case e_TaskPhase_Get:
      name.append("get");
      message = boost::intrusive_ptr<ISystemTaskMessage>(
          new SystemTaskMessage_GetPhase(name, system->GetTask()));
      break;
    case e_TaskPhase_Process:
      name.append("process");
      message = boost::intrusive_ptr<ISystemTaskMessage>(
          new SystemTaskMessage_ProcessPhase(name, system->GetTask()));
      break;
    case e_TaskPhase_Put:
      name.append("put");
      message = boost::intrusive_ptr<ISystemTaskMessage>(
          new SystemTaskMessage_PutPhase(name, system->GetTask()));
      break;
  }
  std::shared_ptr<TaskSequenceEntry_SystemTaskMessage> taskSequenceEntry(
      new TaskSequenceEntry_SystemTaskMessage(message));
...
void TaskSequence::AddUserTaskEntry(std::shared_ptr<IUserTask> userTask, e_TaskPhase taskPhase) {
  ...
  switch (taskPhase) {
    case e_TaskPhase_Get:
      name.append("get");
      message =
          boost::intrusive_ptr<IUserTaskMessage>(new UserTaskMessage_GetPhase(name, userTask));
      break;
    case e_TaskPhase_Process:
      name.append("process");
      message =
          boost::intrusive_ptr<IUserTaskMessage>(new UserTaskMessage_ProcessPhase(name, userTask));
      break;
    case e_TaskPhase_Put:
      name.append("put");
      message =
          boost::intrusive_ptr<IUserTaskMessage>(new UserTaskMessage_PutPhase(name, userTask));
      break;
  }
  std::shared_ptr<TaskSequenceEntry_UserTaskMessage> taskSequenceEntry(
      new TaskSequenceEntry_UserTaskMessage(message));
