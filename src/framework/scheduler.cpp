  ...
  if (GetSequenceCount() != 0) {  // shouldn't happen
    ...
    for (unsigned int i = 0; i < sequences.data.size(); i++) {
      printf("sequence '%s' is stuck on entry #%i!\n",
             sequences.data.at(i)->taskSequence->GetName().c_str(),
             sequences.data.at(i)->programCounter);
    }
    sequences.Unlock();
  }
...
void Scheduler::RegisterTaskSequence(std::shared_ptr<TaskSequence> sequence) {
  if (sequence->GetEntryCount() == 0)
    Log(e_FatalError, "Scheduler", "RegisterTaskSequence",
        "Trying to add a sequence without entries");
  ...
  for (unsigned int i = 0; i < sequences.data.size(); i++) {
    ...
    if (program->taskSequence->GetName() == name) {
      if (program->taskSequence->GetSkippable()) {
        ...
        resultTime_ms =
            program->startTime + program->taskSequence->GetSequenceTime() * program->timesRan;
      }
...
bool Scheduler::Run() {
  ...
  while (EnvironmentManager::GetInstance().GetQuit() == false || GetSequenceCount() > 0) {
    ...
    for (unsigned int i = 0; i < sequences.data.size(); i++) {
      ...
      if (verbose)
        printf("sequence %i, previous program counter %i, program counter %i\n", i,
               program->previousProgramCounter, program->programCounter);

      // check if previous entry is ready
      bool previousEntryIsReady = true;
      if (program->previousProgramCounter != -1) {
        previousEntryIsReady =
            program->taskSequence->GetEntry(program->previousProgramCounter)->IsReady();
      ...
      if (verbose) {
        if (previousEntryIsReady) {
          printf("sequence %i counter %i is ready\n", programIndex,
                 program->previousProgramCounter);
        } else {
      ...
      if (previousEntryIsReady) {
        long timeUntilDueEntry_ms =
            0;  // if programCounter != 0, we just want to start the next entry ASAP
          ...
          if (program->dueQuit == true) {
            ...
            if (program->taskSequence->GetSkippable()) {
              ...
              timeUntilDueEntry_ms =
                  (program->sequenceStartTime + program->taskSequence->GetSequenceTime()) - time_ms;
              // printf("wiieee: %i .. %li .. %li\n", program->taskSequence->GetFrameTime(),
              // time_ms, startTimeRel_ms);
            } else {
              // use absolute time: if not enough iterations have been done to get to frametime *
              // timesran, start immediately
              timeUntilDueEntry_ms =
                  (program->startTime +
                   program->taskSequence->GetSequenceTime() * program->timesRan) -
                  time_ms;
              // printf("wiieee: %i .. %li .. %li\n", program->taskSequence->GetSequenceTime(),
              // time_ms, timeUntilDueEntry_ms);
            }
          }
        }

        if (!program->readyToQuit && !program->paused &&
            (timeUntilDueEntry_ms < dueEntry.timeUntilDueEntry_ms ||
             dueEntry.program == std::shared_ptr<TaskSequenceProgram>())) {
    ...
    if (someSequenceNeedsDeleting) {
      std::vector<std::shared_ptr<TaskSequenceProgram>>::iterator quiterator =
          sequences.data.begin();
      ...
      firstSequence++;
      if (firstSequence >= sequences.data.size())
        firstSequence = 0;

      long timeout_ms = 0;
      if (dueEntry.program != std::shared_ptr<TaskSequenceProgram>()) {
        // wait until time for due sequence entry (even if that is <= 0, because we need to unlock
        // the condition and sequences locks anyway)
      ...
      sequences.Unlock();
      bool isMessage = somethingIsDone.wait_for(lock, std::chrono::milliseconds(timeout_ms)) !=
                       std::cv_status::timeout;
      sequences.Lock();

      if (dueEntry.program != std::shared_ptr<TaskSequenceProgram>()) {
        if (verbose)
          printf("time until due entry: %li\n", dueEntry.timeUntilDueEntry_ms);
        ...
        if (dueEntry.timeUntilDueEntry_ms <= 0) {  // the first and/or other entries are due
          ...
          if (verbose)
            printf("executing program counter %i\n", dueEntry.program->programCounter);
