...
SDL_Surface* CreateSDLSurface(int width, int height) {
  return SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_RLEACCEL, width, height, 32, r_mask, g_mask,
                              b_mask, a_mask);
}
...
void sdl_triangle_filled(SDL_Surface* surface, const Triangle& triangle, Uint8 r, Uint8 g,
  ...
  //    sge_FilledTrigon(surface, int(triangle.GetVertex(0).coords[0]),
  //    int(triangle.GetVertex(0).coords[1]),
  //                              int(triangle.GetVertex(1).coords[0]),
  //                              int(triangle.GetVertex(1).coords[1]),
  //                              int(triangle.GetVertex(2).coords[0]),
  //                              int(triangle.GetVertex(2).coords[1]), r, g, b);
  //
  //    sge_FilledTrigon(surface, int(triangle.GetVertex(1).coords[0]),
  //    int(triangle.GetVertex(1).coords[1]),
  //                              int(triangle.GetVertex(0).coords[0]),
  //                              int(triangle.GetVertex(0).coords[1]),
  //                              int(triangle.GetVertex(2).coords[0]),
  //                              int(triangle.GetVertex(2).coords[1]), r, g, b);
}
...
void sdl_alphablit(SDL_Surface* src, SDL_Rect* srcrect, SDL_Surface* dst, SDL_Rect* dstrect) {
  ...
  for (y = 0; y < (srect.h < drect.h ? srect.h : drect.h); y++) {
    for (x = 0; x < (srect.w < drect.w ? srect.w : drect.w); x++) {
      psrc = (Uint8*)src->pixels + (y + srect.y) * src->pitch +
             (x + srect.x) * src->format->BytesPerPixel;

      /* copy value from source to destination */
      pdst = (Uint8*)dst->pixels + (y + drect.y) * dst->pitch +
...
void sdl_flipsurface(SDL_Surface* surface) {
  ...
  SDL_Surface* result =
      SDL_CreateRGBSurface(surface->flags, surface->w, surface->h,
...
void sdl_setsurfacealpha(SDL_Surface* surface, int alpha) {
  ...
  // NOTE: since we are only modifying the alpha bytes we could simply read only that byte and then
  // add 4 to get to the next alpha byte. That approach depends on the endianess of the alpha byte
  // and also the 32 bit pixel depth so if those things change then this code will need changing
  // too. but we already rely on 32 bpp
