...
AABB AABB::operator*(const Quaternion& rot) const {
  ...
  for (int i = 0; i < (signed int)vecs.size(); i++) {
    if (vecs.at(i).coords[0] < aabb.minxyz.coords[0])
      aabb.minxyz.coords[0] = vecs.at(i).coords[0];
    if (vecs.at(i).coords[0] > aabb.maxxyz.coords[0])
      aabb.maxxyz.coords[0] = vecs.at(i).coords[0];
    if (vecs.at(i).coords[1] < aabb.minxyz.coords[1])
      aabb.minxyz.coords[1] = vecs.at(i).coords[1];
    if (vecs.at(i).coords[1] > aabb.maxxyz.coords[1])
      aabb.maxxyz.coords[1] = vecs.at(i).coords[1];
    if (vecs.at(i).coords[2] < aabb.minxyz.coords[2])
      aabb.minxyz.coords[2] = vecs.at(i).coords[2];
    if (vecs.at(i).coords[2] > aabb.maxxyz.coords[2])
...
void AABB::Reset() {
  minxyz.Set(std::numeric_limits<real>::max(), std::numeric_limits<real>::max(),
             std::numeric_limits<real>::max());
  maxxyz.Set(-std::numeric_limits<real>::max(), -std::numeric_limits<real>::max(),
             -std::numeric_limits<real>::max());
  radius_needupdate = true;
  center_needupdate = true;
}
...
bool AABB::Intersects(const vector_Planes& planes) const {
  ...
  for (int p = 0; p < planes_size; p++) {
    ...
    if (aabb_intersects_planecollection == false)
      break;
  }
...
bool AABB::Intersects(const AABB& src) const {
  if ((src.minxyz.coords[0] < maxxyz.coords[0] && src.minxyz.coords[1] < maxxyz.coords[1] &&
       src.minxyz.coords[2] < maxxyz.coords[2]) &&
      (src.maxxyz.coords[0] > minxyz.coords[0] && src.maxxyz.coords[1] > minxyz.coords[1] &&
...
bool AABB::Intersects(const Line& src) const {
  ...
  if (fabsf(c.coords[0]) > e.coords[0] + ad.coords[0])
    return false;
  if (fabsf(c.coords[1]) > e.coords[1] + ad.coords[1])
    return false;
  if (fabsf(c.coords[2]) > e.coords[2] + ad.coords[2])
    return false;

  if (fabsf(d.coords[1] * c.coords[2] - d.coords[2] * c.coords[1]) >
      e.coords[1] * ad.coords[2] + e.coords[2] * ad.coords[1] +
          std::numeric_limits<float>::denorm_min())
    return false;
  if (fabsf(d.coords[2] * c.coords[0] - d.coords[0] * c.coords[2]) >
      e.coords[2] * ad.coords[0] + e.coords[0] * ad.coords[2] +
          std::numeric_limits<float>::denorm_min())
    return false;
  if (fabsf(d.coords[0] * c.coords[1] - d.coords[1] * c.coords[0]) >
      e.coords[0] * ad.coords[1] + e.coords[1] * ad.coords[0] +
          std::numeric_limits<float>::denorm_min())
    return false;
