...
bool Triangle::operator==(const Triangle& triangle) const {
  if (vertices[0] == triangle.GetVertex(0) && vertices[1] == triangle.GetVertex(1) &&
...
inline void Triangle::SetTextureVertex(unsigned char pos, const real x, const real y,
                                       const real z) {
  SetTextureVertex(0, pos, x, y, z);
}

inline void Triangle::SetTextureVertex(unsigned char texture_unit, unsigned char pos,
                                       const Vector3& xyz) {
  SetTextureVertex(texture_unit, pos, xyz.coords[0], xyz.coords[1], xyz.coords[2]);
}

inline void Triangle::SetTextureVertex(unsigned char texture_unit, unsigned char pos, const real x,
...
inline const Vector3& Triangle::GetTextureVertex(unsigned char texture_unit,
...
AABB Triangle::GetAABB() {
  ...
  for (int i = 1; i < 3; i++) {
    if (vertices[i].coords[0] < aabb.minxyz.coords[0])
      aabb.minxyz.coords[0] = vertices[i].coords[0];
    if (vertices[i].coords[0] > aabb.maxxyz.coords[0])
      aabb.maxxyz.coords[0] = vertices[i].coords[0];
    if (vertices[i].coords[1] < aabb.minxyz.coords[1])
      aabb.minxyz.coords[1] = vertices[i].coords[1];
    if (vertices[i].coords[1] > aabb.maxxyz.coords[1])
      aabb.maxxyz.coords[1] = vertices[i].coords[1];
    if (vertices[i].coords[2] < aabb.minxyz.coords[2])
      aabb.minxyz.coords[2] = vertices[i].coords[2];
    if (vertices[i].coords[2] > aabb.maxxyz.coords[2])
      aabb.maxxyz.coords[2] = vertices[i].coords[2];
  }
...
void Triangle::IntersectsPlane(bool intersects[], const Vector3& pvector,
                               const Vector3& pnormal) const {
  // returns:
  ...
  // printf("base point: %f %f %f; base normal: %f %f %f\n", pvector.coords[0], pvector.coords[1],
  // pvector.coords[2], pnormal.coords[0], pnormal.coords[1], pnormal.coords[2]);
  ...
  if ((distance[0] == 0 && distance[1] == 0) || (distance[1] == 0 && distance[2] == 0) ||
  ...
  if (sign(distance[0]) != sign(distance[1]))
    intersects[0] = true;
  if (sign(distance[1]) != sign(distance[2]))
    intersects[1] = true;
  if (sign(distance[2]) != sign(distance[0]))
    intersects[2] = true;
}
...
bool Triangle::IntersectsLine(const Line& u_ray, Vector3& intersectVec) const {
  ...
  // todo: might be fixable by ensuring correctness of lumel positions in lightmap generation
...
bool Triangle::IsCoplanar(const Vector3& N, const Triangle& triangle) const {
  ...
  EDGE_AGAINST_TRI_EDGES(vertices[0], vertices[1], triangle.GetVertex(0), triangle.GetVertex(1),
                         triangle.GetVertex(2));
  EDGE_AGAINST_TRI_EDGES(vertices[1], vertices[2], triangle.GetVertex(0), triangle.GetVertex(1),
                         triangle.GetVertex(2));
  EDGE_AGAINST_TRI_EDGES(vertices[2], vertices[0], triangle.GetVertex(0), triangle.GetVertex(1),
                         triangle.GetVertex(2));
...
void Triangle::CalculateTangents() {
  ...
  if (cp != 0.0f) {
    ...
    for (int v = 0; v < 3; v++) {
      ...
      // "To check whether it must be inverted or not, the check is simple : TBN must form a
      // right-handed coordinate system, i.e. cross(n,t) must have the same orientation than b.
      //  In mathematics, “Vector A has the same orientation as Vector B” translates as dot(A,B)>0,
      //  so we need to check if dot( cross(n,t) , b ) > 0. If it’s false, just invert t : if
      //  (glm::dot(glm::cross(n, t), b) < 0.0f) {
      //    t = t * -1.0f;
      //  }"
