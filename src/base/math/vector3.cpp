...
Vector3& Vector3::operator*=(const Matrix3& mat) {
  Vector3 tmp;
  tmp.coords[0] =
      coords[0] * mat.elements[0] + coords[1] * mat.elements[3] + coords[2] * mat.elements[6];
  tmp.coords[1] =
      coords[0] * mat.elements[1] + coords[1] * mat.elements[4] + coords[2] * mat.elements[7];
  tmp.coords[2] =
      coords[0] * mat.elements[2] + coords[1] * mat.elements[5] + coords[2] * mat.elements[8];
...
void Vector3::NormalizeTo(float length) {
  if (fabs(this->coords[0]) < 0.000001f && fabs(this->coords[1]) < 0.000001f &&
      fabs(this->coords[2]) < 0.000001f)
    Log(e_Warning, "Vector3", "NormalizeTo", "Trying to normalize 0-vector");
...
Vector3 Vector3::GetNormalizedTo(float length) const {
  if (fabs(this->coords[0]) < 0.000001f && fabs(this->coords[1]) < 0.000001f &&
      fabs(this->coords[2]) < 0.000001f)
    Log(e_Warning, "Vector3", "GetNormalizedTo", "Trying to normalize 0-vector");
  Vector3 tmp(*this);
...
real Vector3::GetDistance(const Vector3& fac) const {
  ...
  float length = sqrt(std::pow(v3[0], 2) + std::pow(v3[1], 2) + std::pow(v3[2], 2));
...
bool Vector3::Compare(const Vector3& test) const {
  return test.coords[0] == coords[0] && test.coords[1] == coords[1] && test.coords[2] == coords[2];
}
...
Vector3 Vector3::EnforceMaximumDeviation(const Vector3& deviant, float maxDeviation) const {
  ...
  if (maxDeviation == 0.0f)
    assert(deviant.GetDistance(result) < 0.001f);
  return result;
}
...
Vector3 Vector3::GetClamped2D(const Vector3& v1, const Vector3& v2) const {
  ...
  if (signSide(v1_to_this) != direction ||
      signSide(v2_to_this) == direction) {  // wrong side! (what this does: make 2 cross sections
                                            // through the virtual circle, one for each parameter,
                                            // and check if we are on the wrong side of either one
