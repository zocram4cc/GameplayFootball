...
Matrix4 Matrix4::operator*(const Matrix4& multiplier) const {
  ...
  for (int r = 0; r < 4; r++) {
    for (int c = 0; c < 4; c++) {
      result.elements[r * 4 + c] = elements[r * 4 + 0] * multiplier.elements[0 + c] +
...
void Matrix4::MultiplyVec4(float x, float y, float z, float w, float& rx, float& ry, float& rz,
                           float& rw) {
...
void Matrix4::ConstructOrtho(float left, float right, float bottom, float top, float zNear,
                             float zFar) {
  // https://solarianprogrammer.com/2013/05/22/opengl-101-matrices-projection-view-model/
