...
public:
  ...
  }  // gcc bug? this sometimes only sets the last element: memcpy(coords, src.coords, 3 *
     // sizeof(real)); }
  Vector3(real xyz);
  ...
  // this function does not make much sense does it? void NormalizeMax(float length, const Vector3
  // &ifNull);
  ...
  // this function does not make much sense does it? Vector3 GetNormalizedMax(float length, const
  // Vector3 &ifNull) const;
  ...
  // extrapolates a vector by a direction & a time difference in the future (positive) or in the
  // past (negative) assume direction vector to be units per sec, assume time to be in milliseconds
...
inline bool Vector3::operator==(const Vector3& vector) const {
  if (coords[0] == vector.coords[0] && coords[1] == vector.coords[1] &&
...
inline void Vector3::operator=(const real src) {
  Set(src);
}

inline void Vector3::operator=(const Vector3& src) {
  coords[0] = src.coords[0];
  coords[1] = src.coords[1];
  coords[2] = src.coords[2];
}
...
inline Vector3 Vector3::operator*(const Vector3& scalar) const {
  return Vector3(coords[0] * scalar.coords[0], coords[1] * scalar.coords[1],
                 coords[2] * scalar.coords[2]);
}

inline Vector3& Vector3::operator*=(const real scalar) {
  coords[0] *= scalar;
  coords[1] *= scalar;
  coords[2] *= scalar;
  return *this;
...
inline Vector3& Vector3::operator*=(const Vector3& scalar) {
...
inline Vector3 Vector3::operator+(const real value) const {
...
inline bool Vector3::operator<(const Vector3& vector) const {
  if (coords[0] == vector.coords[0]) {
    ...
    return coords[0] < vector.coords[0];
}
...
inline real Vector3::GetSquaredLength() const {
  return coords[0] * coords[0] + coords[1] * coords[1] + coords[2] * coords[2];
}

inline radian Vector3::GetAngle2D() const {
...
inline radian Vector3::GetAngle2D(const Vector3& test) const {
  radian angle = -std::atan2(coords[0] * test.coords[1] - coords[1] * test.coords[0],
...
inline void Vector3::Rotate2D(const radian angle) {
...
inline Vector3 Vector3::GetRotated2D(const radian angle) const {
  Vector3 result;
  real x = (coords[0] * std::cos(angle)) - (coords[1] * std::sin(angle));
  real y = (coords[1] * std::cos(angle)) + (coords[0] * std::sin(angle));
  result.coords[0] = x;
...
inline Vector3 Vector3::Get2D() const {
