...
bool Quaternion::operator!=(const Quaternion& fac) const {
  if (fac.elements[0] != elements[0] || fac.elements[0] != elements[1] ||
...
const Quaternion Quaternion::operator*(float scale) const {
  return Quaternion(elements[0] * scale, elements[1] * scale, elements[2] * scale,
                    elements[3] * scale);
}
...
void Quaternion::operator=(const Vector3& vec) {
  ...
  Matrix3 mat(x.coords[0], x.coords[1], x.coords[2], y.coords[0], y.coords[1], y.coords[2],
...
Quaternion Quaternion::operator*(const Quaternion& fac) const {
  ...
  result.elements[0] = elements[3] * fac.elements[0] + elements[0] * fac.elements[3] +
                       elements[1] * fac.elements[2] - elements[2] * fac.elements[1];
  result.elements[1] = elements[3] * fac.elements[1] + elements[1] * fac.elements[3] +
                       elements[2] * fac.elements[0] - elements[0] * fac.elements[2];
  result.elements[2] = elements[3] * fac.elements[2] + elements[2] * fac.elements[3] +
                       elements[0] * fac.elements[1] - elements[1] * fac.elements[0];
  result.elements[3] = elements[3] * fac.elements[3] - elements[0] * fac.elements[0] -
                       elements[1] * fac.elements[1] - elements[2] * fac.elements[2];

  return result;
}

Quaternion Quaternion::operator+(const Quaternion& q2) const {
  return Quaternion(elements[0] + q2.elements[0], elements[1] + q2.elements[1],
                    elements[2] + q2.elements[2], elements[3] + q2.elements[3]);
}
...
void Quaternion::GetAngles(radian& X, radian& Y, radian& Z) const {
  ...
  Z = std::atan2(2 * elements[y] * elements[3] - 2 * elements[x] * elements[z],
                 1 - 2 * sqy - 2 * sqz);
  Y = std::asin(2 * elements[x] * elements[y] + 2 * elements[z] * elements[3]);
  X = std::atan2(2 * elements[x] * elements[3] - 2 * elements[y] * elements[z],
                 1 - 2 * sqx - 2 * sqz);
}
...
real Quaternion::GetMagnitude() const {
  if (elements[0] == 0.0f && elements[1] == 0.0f && elements[2] == 0.0f && elements[3] == 0.0f)
    return 0.0f;
  real magnitude = std::sqrt(elements[0] * elements[0] + elements[1] * elements[1] +
...
void Quaternion::Normalize() {
  ...
  double qmagsq = elements[0] * elements[0] + elements[1] * elements[1] +
                  elements[2] * elements[2] + elements[3] * elements[3];  // squared magnitude
  if (qmagsq < 0.000001f) {
...
float Quaternion::GetDotProduct(const Quaternion& subject) const {
  return (elements[0] * subject.elements[0] + elements[1] * subject.elements[1] +
          elements[2] * subject.elements[2] + elements[3] * subject.elements[3]);
}
