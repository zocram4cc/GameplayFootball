...
TEST(OneTouchPassTest, RemovesTheAccuracyPenaltyOnAQuickRelease) {
  ...
  EXPECT_FLOAT_EQ(PlayerTraits::GetQuickReleaseAccuracyPenalty(mask, PlayerTraits::oneTouchWindow_ms,
                                                               basePenalty),
                  0.0f);
}
...
TEST(OneTouchPassTest, PlayersWithoutTheCardAlwaysPayThePenalty) {
  EXPECT_FLOAT_EQ(PlayerTraits::GetQuickReleaseAccuracyPenalty(PlayerTraits::traitMaskNone, 50,
                                                               0.3f),
                  0.3f);
  EXPECT_FLOAT_EQ(
      PlayerTraits::GetQuickReleaseAccuracyPenalty(PlayerTraits::e_Trait_TargetMan, 50, 0.3f), 0.3f);
}
...
TEST(GoalPoacherTest, GluesTheForwardToTheOpponentOffsideLine) {
  ...
  const float target = PlayerTraits::GetPoacherTargetX(PlayerTraits::e_Trait_GoalPoacher, -5.0f,
                                                       offsideLineX, 1);
...
TEST(GoalPoacherTest, PullsBackAForwardThatWouldStrayOffside) {
  ...
  const float target = PlayerTraits::GetPoacherTargetX(PlayerTraits::e_Trait_GoalPoacher, -30.0f,
                                                       offsideLineX, 1);
...
TEST(GoalPoacherTest, PlayersWithoutTheCardKeepTheirFormationPosition) {
  EXPECT_FLOAT_EQ(
      PlayerTraits::GetPoacherTargetX(PlayerTraits::traitMaskNone, -5.0f, -20.0f, 1), -5.0f);
  EXPECT_FLOAT_EQ(
...
TEST(CreativePlaymakerTest, SpaceWeightStaysNormalized) {
  EXPECT_LE(PlayerTraits::GetSpaceRatingWeight(PlayerTraits::e_Trait_CreativePlaymaker, 0.9f), 1.0f);
  EXPECT_GE(PlayerTraits::GetSpaceRatingWeight(PlayerTraits::e_Trait_CreativePlaymaker, 0.0f), 0.0f);
}
